...
** Stationary
    ...
    is \(G\)-definable, \(g\) dominates \(h\) <<Problem5>>. Note that for each large enough \(M\in H\) the area
    ...
    constantly equal to 0<<Problem6>>. So we can assume that \(f\) is ultimately non-decreasing
    ...
    *Case 1*: \((M,+\infty)\subseteq A\). Then \(f\) grows "slowly" on \((M,+\infty)\):
    ...
    definable \(g:H\to H\) we have that \(f\ll g\) and \(g\ll h\) for each \(G\)-definable \(h:H\to H\)
    with \(f\ll h\). Since \(\lim_{x\to+\infty}(g(x)-f(x))=+\infty\)
    #+END_proof
