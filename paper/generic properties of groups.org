...
** Expansions of real closed fields
    ...
    \end{equation*}
    Then \(T\) is a convex subring of \(R\),
    \begin{equation*}
    T^*=\{s\in S:\frac{1}{r}<\abs{s}<r\text{ for some }r\in R\}
    \end{equation*}
    and \((T^*,\cdot)\) is a subgroup of the multiplicative group \((S^*,\cdot)\). The quotient
    group \((S^*/T^*,*,1)\) may be ordered in the following way:
    \begin{equation*}
    ...
    2. \(\nu(x+y)\le\max(\nu(x),\nu(y))\)
    3. \(\nu(x)\neq\nu(y)\Rightarrow\nu(x+y)=\max(\nu(x),\nu(y))\)
    Since \(f\) is semi-algebraic, by Lemma ref:2.5.2 in [[cite:&bochnak2013real]], there exists a
