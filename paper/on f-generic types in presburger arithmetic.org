...
** Preliminaries
    ...
    *homogeneous* \(n\)-ary \(\Q\)-linear function. By \(L_n(M)\) we mean the space of
    ...
    s.t. \(D_m(\M^n)\subseteq\dom(f)\). In particular, \((G^0)^n\subseteq\dom(f)\). By Fact ref:1.8 and Fact ref:1.12 we
    conclude that:
    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    ...
    \end{equation*}
    with \(m\in\N^m\), \(\theta(M)\) a cell, \(f_i\in L_{n-1}(M)\), and \(\Box_i\) either \(\le\) or no condition,
    s.t. \(M\vDash\forall(\psi(x)\to\phi(x))\).
    #+END_corollary
