...
separates \(E\)-classes.
\wu{
We want to define \(X_e=\pi^{-1}(e)\), but \(\pi^{-1}(e)\) is not \(\emptyset\)-definable and so we want to
simulate it.

\(\abs{\varphi_e(e,\fC)}=1\) is showed by \(\exists!\bary\varphi_e(\pi_E(\barx),\bary)\).

Now \(E(\barx,\barz)\) iff \(\pi_E(\barx)\)

\(\varphi_e(\pi_E(\barx),\fC)=\varphi_e(\pi_E(\barz),)\)
}
