...
\section{The Basics}
\label{sec:orgd6c4aa2}

\subsection{Structures}
\label{sec:orgb684af0}
\begin{definition}[]
...
\label{sec:org767d555}
\subsection{Elementary substructures}
\label{sec:org68cf43e}
...
\label{sec:org0fcb5b8}
\subsection{Preservation theorems}
\label{sec:orgc58ddcb}
...
\label{sec:org34bb86c}
\subsection{The omitting types theorem}
\label{sec:orgad2a576}
\begin{definition}[]
...
\label{sec:org4b879f0}

\subsection{Indiscernibles}
\label{sec:org7cfdab0}
\begin{definition}[]
...
\label{sec:org8446c5c}
\subsection{Dividing and forking}
\label{sec:org59e57e3}
...
\wu{
Check \href{https://ivv5hpp.uni-muenster.de/u/baysm/einfacheTheorien/vortraege/boissonneau-dividingAndForking.pdf}{this}. by adding dummy variables we have
\begin{equation*}
...
\wu{
Let \(\fC^+\) be an expansion of the monster model \(\fC\) by Skolem functions, let \(L^+\) be the
expanded language, and let \(T^+\) be \(\Th_{L^+}(\fC^+)\). Then \(T^+\) has the tree property,
witnessed by the same \(L\)-formula \(\varphi(x;y)\) and the same tree

So there is a \(\varphi\)-\(k\)-dividing sequence \((\varphi(x,b_\alpha))_{\alpha<\kappa^+}\). Let \(M=\la\{b_\alpha\mid\alpha<\kappa^+\}\ra\).
Since \(T^+\) has Skolem functions, \(M\vDash T^+\). Similarly, for all \(\beta<\kappa^+\), \(M_\beta=\la\{b_\alpha\mid\alpha<\beta\}\ra\)
is a model
}
\end{proof}
...
\wu{
as \(\bigvee_{l<d}\varphi_l(x,b)\) is consistent with \(p\).
}
...
\begin{proposition}[Kim's lemma for simple theories]
\label{prop7.2.14}
...
\wu{
Check \href{https://math.stackexchange.com/questions/2467508/morley-sequences-of-arbitrary-order-type}{this}. It's because forking is always witnessed by a formula (``finite character''), and
whether a formula \(\varphi(x,c)\) forks over 𝐴 just depends on \(\tp(c/A)\) (``invariance'').
...
have
\begin{enumerate}
\item \(\varphi(x,a_{j_1},\dots,a_{j_n},c)\) forks over \(A\)
(since \(\tp(a_{j_1},\dots,a_{j_n}c/A)=\tp(b_{i_1},\dots,b_{i_n}c/A)\)) and
\item \(\varphi(x,a_{j_1},\dots,a_{j_n},c)\in\tp(a_{k'}/Aa_{<k'})\)
...
\begin{corollary}[Monotonicity and Transitivity]
...
\section{Fields}
\label{sec:org0ac505f}
\subsection{Ordered fields}
\label{sec:org0805149}
...
\label{sec:org0d8c7ac}
\subsection{Pregeometris}
\label{sec:orgb7f2ba3}
\begin{definition}[]
