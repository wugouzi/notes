...
\section{The Basics}
\label{sec:orgd663043}

\subsection{Structures}
\label{sec:orgb203f6b}
\begin{definition}[]
...
\label{sec:org042354b}
\subsection{Elementary substructures}
\label{sec:org9ca1dd8}
...
\(\aleph_0\)-categorical and by Vaught's test complete. Let \(A=\{a_i\mid
      i\in\omega\}\), \(B=\{b_i\mid i\in\omega\}\). We inductively define
...
\label{sec:orgf3142c6}
\subsection{Preservation theorems}
\label{sec:org4fbeb17}
...
\label{sec:orgb73f6be}
\subsection{The omitting types theorem}
\label{sec:org721e25a}
\begin{definition}[]
...
\label{ex4.2.2}
\begin{enumerate}
\item Closed subsets of \(S_n(T)\) have the form \(\{p\in
...
\begin{enumerate}
\item Show that \(\{\tp(a_0,a_1,\dots)\mid\text{ the $a_i$ enumerate a model of
      }T\}\) is comeagre in \(S_\omega(T)\)
\end{enumerate}
\#+END\_\#+BEGIN\textsubscript{exercise}
\end{exercise}
\subsection{\(ℵ_0\)-categorical theories}
\label{sec:org0f9e2c4}
\begin{theorem}[]
\label{thm4.3.1}
...
type
\begin{equation*}
p(x)=\tp(a_n/A_i)=\{\varphi(x)\mid\fA\models\varphi(a_n), \varphi(x)\text{ a $L(A_i)$-formula}\}
\end{equation*}
...
\begin{equation*}
\fA\models\varphi(a_n,\bbar{a}) \Rightarrow\fB\models\varphi(b',f_i(\bbar{a}))
\end{equation*}
