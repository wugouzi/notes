...
\section{The Basics}
\label{sec:org3c7b9c7}

\subsection{Structures}
\label{sec:org74dd529}
\begin{definition}[]
...
\label{sec:org83ad84d}
\subsection{Elementary substructures}
\label{sec:orgadf4a42}
...
\label{sec:org5ccd5af}
\subsection{Preservation theorems}
\label{sec:org8d474a7}
...
\label{sec:org3a40ac3}
\subsection{The omitting types theorem}
\label{sec:org3f60d26}
\begin{definition}[]
...
\label{sec:orgd162d64}

\subsection{Indiscernibles}
\label{sec:org57073d4}
\begin{definition}[]
...
\label{sec:org84d53e1}
\subsection{Dividing and forking}
\label{sec:orga810ac7}
...
\begin{proof}
\begin{enumerate}
\item \(\tp(a/Aa)\) divides over \(A\) iff \(\varphi(x,\barb,a)\in\tp(a/Aa)\)  divides over \(A\) iff there
is a sequence \((\barb_i,a_i)_{i<\omega}\) of realisations of \(\tp(\barb,a/A)\)
s.t. \((\varphi(x,\barb_i,a_i))_{i\in\omega}\) is \(k\)-inconsistent.

Suppose \((\varphi(x,\barb_i,a_i))_{i\in\omega}\) is consistent, then it is realised by some \(c\).
\end{enumerate}
\end{proof}
}
...
\wu{
As satisfiable iff finitely satisfiable, there is a finite subset of \(\bigcup_{i<\omega}(\pi(x,b_i))\) that
is inconsistent. Take the conjunction of it, namely \(\varphi(x,b)\).
}
...
of \(\tp(b/A)\) s.t. \((\varphi(x,b_i)\mid i<\omega)\) is \(k\)-inconsistent. We may assume by Lemma
\ref{lemma7.1.1} that \((b_i)_{i<\omega}\) is indiscernible over \(A\). Clearly \(\bigcup_{i<\omega}\pi(x,b_i)\) is inconsistent
\end{proof}
...
1\#+BEGIN\textsubscript{proof}
\(2\Leftrightarrow 3\): this is clear by considering appropriate automorphisms
\wu{
Let \(f:Aba\mapsto Aba'\), then \(f\) is elementary and extends to an automorphism \(\barf\) of \(\fC\).
}

the conclusion of 2 and 3 is equivalent to

There exist \(a'\) and \(\cali'\) with \(\tp(a'/Ab)=\tp(a/Ab)\) and \(\tp(\cali'/Ab)=\tp(\cali/Ab)\)
s.t. \(\cali'\) is indiscernible over \(Aa'\)
\#+END\textsubscript{proof}

\section{Stable Theories}
\label{sec:org33feb57}
\subsection{Heirs and coheirs}
\label{sec:orgab36a44}
\subsection{Stability}
\label{sec:org1f6ddac}
\subsection{Definable types}
\label{sec:org45b314f}
\subsection{Elimination of imaginaries and \texorpdfstring{\(T^{\eq}\)}{Teq}}
\label{sec:org1884ba4}
...
\begin{proof}
\(2\Leftrightarrow 3\). \ref{P2.35}. Suppose \(X=\varphi(\fC,\bard)\) is almost \(A\)-definable,
...
\appendix
\section{Set Theory}
\label{sec:org8fa3236}
\subsection{Sets and classes}
\label{sec:org19a25fa}
...
\section{Fields}
\label{sec:org8deb85a}
\subsection{Ordered fields}
\label{sec:orgd1e6ff9}
...
\label{sec:org0829c61}
\subsection{Pregeometris}
\label{sec:orgd6e5731}
\begin{definition}[]
