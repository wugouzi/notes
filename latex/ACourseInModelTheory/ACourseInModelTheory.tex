...
\section{The Basics}
\label{sec:org447cdde}

\subsection{Structures}
\label{sec:orga90090a}
\begin{definition}[]
...
\label{sec:org64cccf5}
\subsection{Elementary substructures}
\label{sec:org3195b7f}
...
\label{sec:org0599c44}
\subsection{Preservation theorems}
\label{sec:org2ed5dab}
...
\label{sec:org3878893}
\subsection{The omitting types theorem}
\label{sec:org5b96f53}
\begin{definition}[]
...
\begin{proof}
Let \(P\) be a new unary predicate and \(c_1,\dots,c_n\) new constants. Let \(T^*\) be the theory of
all \(L\cup\{P,c_1,\dots,c_n\}\)-structures
\begin{equation*}
(\fM,N,a_1,\dots,a_n)
\end{equation*}
where \(\fM\) is a model of \(T\), \(N\) is the universe of a proper elementary
substructure, \(a_1,\dots,a_n\) elements of \(N\) and \(\varphi(\fM,\bara)\subseteq N\). Suppose that the
...
satisfiable. For otherwise there are \(\varphi(x)\in p(x)\) and
algebraic \(L(B)\)-formulas \(\psi_1(x),\dots,\psi_n(x)\) with
\begin{equation*}
...
\item The formula \(\varphi(\barx)\) is \textbf{strongly minimal} if \(\varphi(\barx)\) defines a minimal set in all
elementary extensions of \(\fM\). In this case, we also call the definable set \(\varphi(\fM)\)
strongly minimal. A non-algebraic type containing a strongly minimal formula is called
strongly minimal
...
\wu{
Suppose bijection \(f(\bara)=\barb\) iff \(\gamma(\bara,\barb)\). Then \(\psi(\barx)\) iff \(\exists\bary(\varphi(\bary)\wedge\gamma(\bary,\barx))\).
}
...
Whether \(\varphi(\barx,\bara)\) is strongly minimal depends only on the type of the parameter
...
some \(L\)-formula \(\psi(\barx,\barz)\) the set \(\Sigma_\psi(\barz,\bara)\) is realised in \(\fM\)
\wu{
(\(\Sigma_\psi(\barz,\bara)\) is consistent as it can be realized in some elementary extensions)
}
, so \(\varphi\) is not minimal.
...
\appendix
\section{Fields}
\label{sec:org4842c1d}
\subsection{Ordered fields}
\label{sec:orgfed3c90}
