...
\label{sec:org06baccb}
\subsection{Introduction}
\label{sec:orgef908f0}
\begin{definition}[]
...
result, \(p\) is stationary (and complete) \label{Problem6}: why complete?

Suppose \(p(x,y)=p'(x,y)\) for some definable function \(f:R\to R\) an a type $\backslash$
...
It \(p\) is not complete, then we could find a definable function \(f:R\to R\) s.t.
