...
\label{sec:orgb327d81}
\subsection{Introduction}
\label{sec:org33bb545}
\begin{definition}[]
...
\end{gather*}
are weak generic, which implies that \(f\ll g\ll h\)
}
...
It \(p\) is not complete
, then we could find a definable function \(f:R\to R\) s.t.
...
Without loss of generality, we can assume that it is so on the whole interval \([1,+\infty]\) since
...
\end{equation*}
is not weak generic in \((S_+,\cdot)\times(S_+,\cdot)\).
\#+END\textsubscript{proof}


\section{Problems}
\label{sec:orgcf6950e}
\begin{center}
\begin{tabular}{lllll}
