...
\label{sec:org3412a24}
\subsection{Modal Languages}
\label{sec:orgd7799b8}
\begin{definition}[]
...
\section{Models}
\label{sec:org02ff0fb}
\subsection{Invariance Results}
\label{sec:orgc906d12}
\begin{definition}[]
...
\label{sec:orgacee1ae}
\subsubsection{Generated submodels}
\label{sec:orgc44cf6a}
\begin{definition}[]
...
\label{sec:orgb0169f4}
\subsubsection{M-saturation}
\label{sec:org8906443}
...
\section{Frames}
\label{sec:orgd480059}

\subsection{Frame Definability}
\label{sec:orgfc50fb3}
...
\label{sec:org04b4961}
\subsubsection{Finite transitive frames}
\label{sec:orge9ee7a1}
...
\label{sec:org92f5581}
\subsection{Preliminaries}
\label{sec:orgf31c1d4}
...
transitive and non-branching and \(t\in T\), the nthe subframe of \((T,R)\)
generated by \(t\) is trichotomous
\end{proposition}

\begin{definition}[]
Let \(\KtQ\) be the smallest tense logic containing
4,\(text{D}_l,\text{D}_r,den,.3_l\) and \(.3_r\)
\end{definition}
...
right.
Suppose \(R^Cwv\) and \(R^Cwu\). If \(v\neq u\), then there are
\(\phi\in v,\phi\not\in u\) and \(\psi\in u,\psi\not\in v\) since \(u\) and
\(v\) are MCS. Hence \(F \phi,F \psi\in w\). As \(w\) is MCS,
\(F\phi\wedge F\psi\in w\). Hence
\(F(\phi\wedge F\psi)\vee F(\phi\wedge\psi)\vee F(\psi\wedge F\phi)\in w\).
If \(F(\phi\wedge\psi)\in w\), then there is a cu
\end{proof}
\subsection{Limitative Results}
\label{sec:org047b4d4}
