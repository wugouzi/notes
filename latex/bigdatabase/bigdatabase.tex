...
\label{sec:orga8efd19}
\subsection{Introduction}
\label{sec:orgae02a7e}
Compile time system:
\begin{enumerate}
...
\label{sec:org4204bae}
\subsubsection{Basics}
\label{sec:orgea9b440}
...
\label{sec:org6df8a63}
\begin{enumerate}
\item Introduction
\label{sec:org1bd9cc5}
...
\captionof{figure}{\label{}An example of a select-project-join query with late materialization}
\end{center}

Advantage:
\begin{enumerate}
\item selection and aggregation operators tend to render the construction of some tuples unnecessary.
...
\item if data is compressed using a column-oriented compression method (that potentially allow
...
\item cache performance is improved when operating directly on column data, since a given cache line is
not polluted with surrounding irrelevant attributes for a given operation
\item the vectorized optimizations described above have a higher impact on performance for fixed-length
attributes.
\end{enumerate}


A \textbf{multi-column block} or \textbf{vector block} contains a cache-resident, horizontal partition of some subset of
attributes from a particular relation, stored in their original compressed representation.
RETFLAG, and LINENUM columns.
        \begin{center}
\includegraphics[width=.8\textwidth]{../images/bigdatabase/15.png}
\captionof{figure}{\label{}An example multi-column block containing values for the SHIPDATE,}
\end{center}
\item Joins
\label{sec:orgea4039c}
