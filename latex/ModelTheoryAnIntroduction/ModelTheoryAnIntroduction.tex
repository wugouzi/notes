...
\label{sec:org9c614e4}
\subsection{Languages and Structures}
\label{sec:org8269cb8}
\begin{definition}[]
...
\label{sec:org6ba9d4f}
\subsection{The Compactness Theorem}
\label{sec:org7a675cb}
...
\label{sec:orga3006b2}

\subsubsection{Dense Linear Orders}
\label{sec:org27d13a2}
...
\begin{proof}
Guide from \href{https://math.stackexchange.com/questions/1134284/prove-elementary-amalgamation-theorem-in-model-theory}{stack}.
...
\label{sec:orgd8763d8}

\subsection{Quantifier Elimination}
\label{sec:org0a5b745}
...
Consider the \(\call^*\)-theory, which we call \(\Pr\) for \textbf{Presburger arithmetic}, with axioms:
\begin{enumerate}
...
\begin{enumerate}
\setcounter{enumi}{5}
\item \(P_n\) is closed under + and -
\item \(\forall x,y\;(\underbrace{y+\dots+y}_{n\text{ times}}=x)\to P_n(x)\)
\item (for \(m\) dividing \(n\)) \(\forall x(P_m(x)\to P_n(x))\)
\item \(\forall x(P_{kn}(\underbrace{x+\dots+x}_{k\text{ times}})\to P_n(x))\) for \(k,n=2,3,\dots\)
\end{enumerate}
...
\subsection{Exercises}
\label{sec:org47b1fa2}
\begin{exercise}
\label{ex3.4.3}
\begin{enumerate}
\item Show that the theory of \((\Z,s)\) has quantifier elimination where \(s(x)=x+1\). Show that
this theory is strongly minimal and that \(\acl(A)\) is the set of elements ``reachable'' from \(A\)
\item Show that the theory of \((\N,s)\) does not have quantifier elimination
\end{enumerate}
\end{exercise}

\begin{proof}
\href{https://math.stackexchange.com/questions/3203714/exercise-3-4-3-in-david-markers-model-theory}{StackExchange}
...
\label{sec:org72e9d04}

\subsection{Types}
\label{sec:org4bcf562}
...
\begin{enumerate}
\item \(\calm_0\vDash T\) with \(\abs{\calm_0}=\kappa\)
\item \(\calm_\alpha=\bigcup_{\beta<\alpha}\calm_\beta\) for \(\alpha\) a limit ordinal
\item \(\calm_\alpha\prec\calm_{\alpha+1}\) and if \(p\in S_1^{\calm_\alpha}(M_\alpha)\), then \(p\) is realized in \(\calm_{\alpha+1}\).
\end{enumerate}
...
\item If \(a,b\in\Q\), then \(\tp^{\Q}(a/\N)=\tp^{\Q}(b/\N)\) iff there is an automorphism \(\sigma\) of \(\Q\)
fixing \(N\) pointwise with \(\sigma(a)=b\)
\item Let \(A=\{1-\frac{1}{n}:n=1,2,\dots\}\cup\{2+\frac{1}{n}:n=1,2,\dots\}\). Show that \(1\) and \(2\) realize
...
\begin{exercise}[\(\aleph_1\)-saturation of Ultraproducts]
...
\label{sec:org30dd8cd}
\subsection{Partition Theorems}
\label{sec:org20f8fab}
...
\label{sec:orgcc93d84}
\emoji{😅}
\subsection{A Many-Models Theorem}
\label{sec:org676bb2a}
...
\subsection{Exercises}
\label{sec:orgb37ab99}
\begin{exercise}
\label{ex5.5.6}
We say that a theory \(T\) has the \textbf{order property} iff there is a
...
\begin{enumerate}
\item Show that if \(\phi\) has the order property in \(T\), then \(T\) is not \(\kappa\)-stable for any infinite \(\kappa\)
\item Show that \(T\) has the order property iff there is a formula \(\psi(\barv,\barw)\)
...
\begin{proof}
\begin{enumerate}
\item Let \((A,<)\) and \(B\) be as in Lemma \ref{lemma5.2.12}. By Lemma \ref{lemma5.2.5}, we can
...
\end{proof}
\subsubsection{Existence of Strongly Minimal Formulas}
\label{sec:orgffd14e4}
...
\begin{enumerate}
\item If \(\calm\vDash T\), then there is a minimal formula in \(\calm\)
\item If \(\calm\vDash T\) is \(\aleph_0\)-saturated and \(\phi(\barv,\bara)\) is a minimal formula in \(\calm\),
...
\end{corollary}
\subsubsection{The Categoricity Theorem}
\label{sec:orgf0346bc}
...
\begin{theorem}[Existence of nonforking extensions]
\label{thm6.3.2}
Suppose that \(p\in S_n(A)\) and \(A\subseteq B\)
\begin{enumerate}
\item There is \(q\in S_n(B)\) a nonforking extension of \(p\)
\item There are at most \(\deg_M(p)\) nonforking extensions of \(p\) in \(S_n(B)\), and, if \(\calm\)
is an \(\aleph_0\)-saturated model with \(A\subseteq M\), there are exactly \(\deg_M(p)\) nonforking
extensions of \(p\) in \(S_n(M)\)
\item There is at most one \(q\in S_n(B)\), a nonforking extension of \(p\)
...
\textbf{Claim} \(E\) is an \(A\)-definable equivalence relation with at most \(\deg_M(\phi)\) equivalence
classes
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.3\textwidth]{../images/ModelTheoryAnIntroduction/2.jpg}
\label{}
\end{figure}
...
\begin{definition}[]
\(p\in S_n(A)\) is \textbf{stationary} if, for all \(B\supseteq A\), there is a unique nonforking extension
of \(p\) to \(B\)
\end{definition}
...
\begin{enumerate}
\item There is an \(\call_M\)-formula \(\theta(\barv)\) s.t. \(\theta(\M)\subseteq\phi(\M)\), \(\RM(\theta)=\alpha\), and \(\theta\) has Morley
rank 1
\item There are \(\call_M\)-formulas \(\theta_1,\dots,\theta_d\) s.t. each \(\theta_i\) has Morley rank \(\alpha\) and Morley degree
1 and \(\phi(\M)\) is the disjoint union of \(\theta_1(\M),\dots,\theta_d(\M)\)
\item If \(p\in S_n(M)\), then \(p\) has Morley degree 1. In particular, \(p\) is stationary
\end{enumerate}
\end{corollary}

\begin{proof}
\begin{enumerate}
\item In our argument above, we had \(\psi(\barv,\barw)\) and \(\barb\in\M\) and an \(M\)-definable
...
\item We inductively define \(\theta_1,\dots,\theta_d\). Fr \(m<d\), if we are given \(\theta_1,\dots,\theta_{m-1}\), use 1 to
...
\end{equation*}
\item If \(p\in S_n(M)\), choose \(\phi(\barv)\in p\) s.t. \(\RM(\phi)=\RM(p)\), and let \(\theta_1,\dots,\theta_d\) be as
...
\label{sec:org62f4976}
\begin{definition}[]
We say that \(\bara\) is \textbf{independent} from \(B\) over \(A\) if \(\tp(\bara/A)\) does not fork
...
\end{proof}
\subsection{Uniqueness of Prime Model Extensions}
\label{sec:org9621571}
\(T\) is a complete theory in a countable language with infinite models
\subsubsection{Constructible Models}
\label{sec:orgbf89c0e}
\begin{definition}[]
Let \(\calm\) be an \(\call\)-structure and let \(A\subseteq M\). Let \(\delta\) be an ordinal and \((a_\alpha:\alpha<\delta)\) be a
sequence of elements from \(M\). Let \(A_\alpha=A\cup\{a_\beta:\beta<\alpha\}\) . We call \((a_\alpha:\alpha<\delta)\) a \textbf{construction}
over \(A\) if \(\tp(a_\alpha/A_\alpha)\) is isolated for all \(\alpha<\delta\)

\(B\subseteq M\) is \textbf{constructible} over \(A\) if there is a construction \((a_\alpha:\alpha<\delta)\)
...
\label{lemma6.4.2}
\begin{enumerate}
\item If \(T\) is \(\omega\)-stable, \(\calm\vDash T\), and \(A\subset M\), there is \(\caln\prec\calm\) constructible over \(A\)
\item If \(\calm\) is constructible over \(A\), then \(\calm\) is prime over \(A\) and every type realized
...
that \(C\subseteq M\) is \textbf{sufficient} if whenever \(a_\alpha\in C\), then all parameters from \(\theta_\alpha\) are in \(C\)
\end{definition}
...
\begin{enumerate}
\item Each \(A_\alpha\) is sufficient
\item If \(C_i\) is sufficient for all \(i\in I\), then \(\bigcup_{i\in I}C_i\) is sufficient
\item If \(X\subseteq M\) is  finite, then there is a finite sufficient \(C\supseteq X\)
\item If \(C\subseteq M\) is sufficient, then \(M\) is constructible over \(A\cup C\)
\end{enumerate}
\end{lemma}
...
\begin{enumerate}
\setcounter{enumi}{2}
\item Prove by induction on \(\alpha\) that if \(X\subseteq A_\alpha\) is finite, then there is a finite
sufficient \(C\supseteq X\). This is clear if \(\alpha=0\) or \(\alpha\) is a limit ordinal. Suppose \(\alpha=\beta+1\)
and \(X=X_0\cup\{a_\beta\}\) where \(X_0\subseteq A_\beta\). Let \(B\subseteq A_\alpha\) be the parameters from \(\theta_\beta\). By
induction, there is a finite sufficient \(C\supseteq X_0\cup B\). Then \(C\cup\{a_\beta\}\) is a finite sufficient
set containing \(X\)
\item We must show that \(\tp(a_\alpha/A_\alpha\cup C)\) is isolated for all \(\alpha<\delta\). If \(a_\alpha\in C\), this is
...
\textbf{Claim} For each \(i=1,\dots,m\), there is an \(\call_{A_\alpha}\)-formula
...
formula \(\theta_{\alpha_{l+1}}\) isolates \(\tp(a_\alpha/A_{\alpha_{l+1}})\). All parameters occurring
in \(\theta_{\alpha_{l+1}}\) are in \(A_\alpha\cup\{a_{\alpha_1},\dots,a_{\alpha_l}\}\). Thus \(\theta_{\alpha_{l+1}}\)
isolates \(\tp(a_{\alpha+1}/A_{\alpha+1}\cup\{a_{\alpha_1,\dots,a_{\alpha_l}}\})\)
...
\begin{theorem}[Uniqueness of Constructible Models]
...
\end{proof}
\subsubsection{Prime Models of \texorpdfstring{\(\omega\)}{ω}-Stable Theories}
\label{sec:org153e1c4}
...
\section{Reference}
\label{sec:org0044238}
\bibliographystyle{alpha}
\bibliography{../references}
\section{Index}
\label{sec:org9005b93}
