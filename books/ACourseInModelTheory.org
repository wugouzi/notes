...
** Saturated models and the monster
    ...
    *Claim 2*. \(X=\theta(\fC)\)
    ...
    1#+BEGIN_proof
    <<Problem12>>
    \(1\to 2\). Any automorphism fixing \(A\) can only permute the classes of \(E\)

    \(2\to 1\). assume \(X=\phi(\fC,b)\) and \(p(y)=\tp(b/A)\). By assumption there are some \(b_0,\dots,b_k\)
    realizing \(p\) and s.t. for any \(\sigma\in\Aut(\fC/A)\), \(\sigma(X)=\phi(\fC,b_i)\) for some \(i\le k\). Then, by
    homogeneity as before, we have \(p(y)\vdash\bigvee_{i\le k}\forall x(\phi(x,y)\leftrightarrow\phi(x,b_i))\). By compactness there is
...
** Dividing and forking
    ...
    Or, check [[https://ivv5hpp.uni-muenster.de/u/baysm/einfacheTheorien/vortraege/boissonneau-dividingAndForking.pdf][this]]. by adding dummy variables we have
    \begin{equation*}
    ...
    #+LATEX: \wu{
    #+BEGIN_proof
    1. \(\tp(a/Aa)\) divides over \(A\) iff \(\varphi(x,\barb,a)\in\tp(a/Aa)\)  divides over \(A\) iff there
    ...
    2. For any infinite sequence of \(A\)-indiscernibles \(\cali\) containing \(b\), there
