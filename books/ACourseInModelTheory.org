...
** Theories
   ...
   2. If \(T\vDash\varphi(c_1,\dots,c_n)\) and the constants \(c_1,\dots,c_n\)
      occur neither in \(T\) nor in \(\varphi(x_1,\dots,x_n)\), then \(T\vDash\forall
...
** Preservation theorems
   ...
   1. All sentences \(\varphi\in\Delta\) which are true in \(\fA\) are
      consistent with \(T\) (There is a model \(\fB\vDash\Th_\Delta(\fA_A)\cup T\) and \(\fA\Rightarrow_\Delta\fB\))
   ...
   \(\fA_2\to_\exists\fA_1'\) where \(\fA_1'\vDash T_1\)
...
** Quantifier elimination
   ...
   *quantifier free sentence \(\rho(\bbar{c})\)*. We apply the Separation Lemma
   ...
   \end{equation*}
   is equivalent to \((\fM^1,\bbar{a}^1)\vDash
