...
** Saturated models and the monster
    ...
    * A *global* type is a type \(p\) over \(\fC\); we denote this by \(p\in S(\fC)\)
    ...
    #+BEGIN_proof
    [[https://math.stackexchange.com/questions/3361635/when-are-automorphism-invariant-subsets-definable][StackExchange]]
    Let \(\D\) be defined by \varphi, defined over \(B\supset A\). Consider the maps
    \begin{equation*}
    ...
    In language \(L\), suppose there are two models \(N_1,N_2\) and \(b_1\in N_1\), \(b_2\in N_2\).
    If \(\tp(b_1)\supseteq\tp(b_2)\), let \(\varphi(x)\in\tp(b_1)\setminus\tp(b_2)\) assuming \(\tp(b_1)\neq\tp(b_2)\). Then \(\varphi(x)\notin\tp(b_2)\)
    implies \(N_2\vDash\neg\varphi(b_2)\) and \(\neg\varphi(x)\in\tp(b_2)\subseteq\tp(b_1)\).
