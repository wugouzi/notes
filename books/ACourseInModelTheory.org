...
** Saturated models and the monster
    ...
    *Claim 2*. \(X=\theta(\fC)\)
    ...
    #+BEGIN_proof
    1. \(a\in\dcl(A)\) iff \(\{a\}\) is \(A\)-definable iff \(\{a\}\) is invariant under all automorphisms
...
** Simplicity
    ...
    #+LATEX: \wu{
    Check [[https://math.stackexchange.com/questions/2467508/morley-sequences-of-arbitrary-order-type][this]]. It's because forking is always witnessed by a formula ("finite character"), and
    ...
    #+BEGIN_exercise
    label:ex7.2.1
    If \(T\) is simple, there doesn't exist an ascending chain \((p_\alpha)_{\alpha\in\abs{T}^+}\) of forking extensions
    #+END_exercise
