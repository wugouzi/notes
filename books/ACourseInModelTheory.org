...
** \omega-stable theories
   ...
   #+ATTR_LATEX: :options []
   #+BEGIN_definition
   A countable theory \(T\) is *totally transcendental* if it has no model \(\fM\) with a binary tree of
...
** Prime extensions
    ...
    2. \(B\subseteq M\) is *constructible* over \(A\) if \(B\) has an enumeration
       ...
       \end{equation*}
       where each \(b_\alpha\) is atomic over \(A\cup B_\alpha\), with \(B_\alpha=\{b_\mu\mid \mu<\alpha\}\)
    #+END_definition
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If a model \(M\) is contructible over \(A\), then \(\fM\) is prime over \(A\)
    #+END_lemma
