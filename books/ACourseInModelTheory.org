...
** Algebraic formulas
   ...
   #+LATEX: \wu{
   Suppose \(\psi\in p\) is algebraic and \(\abs{\psi(\fM)}=n\). Then add \(c_0,\dots,c_n\) and hence
   \begin{equation*}
    T_A\cup\bigcup_{i=0}^np(c_i)\vDash\bigvee_{i\neq j}c_i=c_j
   \end{equation*}
   By compactness we have a \(\phi\in p\) s.t.
   \begin{equation*}
    T_A\vDash\forall x_0\dots x_n(\bigwedge_{i=0}^n\phi(x_i)\to\bigvee_{i\neq j}x_i=x_j)
   \end{equation*}
   Now if \phi doesn't isolates \(p\), then there must be a \(\phi'\) s.t. \(\phi\wedge\phi'(\fM)\) is a proper subset
   of \(\phi(\fM)\). This process must end with \(\varphi\) with \(\abs{\varphi(\fM)}>0\)
   #+LATEX: }
...
** Dividing and forking
    ...
    #+LATEX: \wu{
    1#+BEGIN_proof
    1. \(\tp(a/Aa)\) divides over \(A\) iff there is a \(\varphi(x,a)\in\tp(a/Aa)\), a \(L(A)\)-formula,
...
* TODO Don't understand
  ...
  | [[Problem6]]:done | [[Problem7]]  | [[Problem8]]      | [[Problem9]]     | [[Problem10]]:done | [[Problem11]] |
