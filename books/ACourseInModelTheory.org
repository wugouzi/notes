...
** Prime Models
    ...
    2. A structure \(\fA\) is called *atomic* if all \(n\)-tuples \(\bara\) of elements of \(\fA\) are
       atomic. This means that the types \(\tp(\bara)\) are isolated in \(S_n^{\fA}(\emptyset)\)
    #+END_definition
...
** Indiscernibles
    ...
    depends only on the value of \(i(0)\).  Again by the pigeonhole principle there are infinitely
    ...
    possibilities) and the type \(\tp(\bara/A_0)\). Write \(\bara=a_{\bari}\) for a tuple \(\bari\)
    from \(I\). Since the \(a_i\) are indiscernible, the type depends only on
    the quantifier-free type \(\tp_{\qf}(\bari/I_0)\) in the structure \((I,<)\) since it has
    quantifier elimination. This type again depends on \(\tp_{\qf}(\bari)\) (finitely many
    possibilities) and on the types  \(p(x)=\tp_{\qf}(i/I_0)\) of the elements \(i\)
    ...
    *Claim*. \(T^*\) is equivalent to a universal theory
    ...
    quantifier-free \(L_\Skolem\)-sentence \(\varphi^*\). Therefore \(T^*\) is equivalent to the universal theory
    #+END_proof
...
** \omega-stable theories
   ...
   that all types over \(A\) are realised in \(\fM\).  Consider the restriction
   map \(\pi:S_n(A)\to S_1(A)\). By assumption the image \(S_1(A)\) has cardinality at most \kappa.
   Every \(p\in S_1(A)\) has the form \(\tp(a/A)\) for some \(a\in M\).
   #+END_proof
