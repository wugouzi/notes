...
** Elimination of imaginaries and @@latex:\texorpdfstring{\(T^{\eq}\)}{Teq}@@
    ...
    3. For each \(L\)-formula \varphi, the axiom \(\forall_{s_\varphi}y\exists_{s_=}x(f_\varphi(x)=y)\)
    #+END_definition
    ...
    4. Suppose \(M,N\vDash T\) and let \(a\in M\), \(b\in N\) with \(\tp_M(a)=\tp_N(b)\). Then \(\tp_{M^{\eq}}(a)=\tp_{N^{\eq}}(b)\)
    #+END_lemma
    ...
    3. Let \(M,N\vDash T^{\eq}\), we want to show that they are elementary equivalent. Assume the
       ...
       <<Problem13>>
    4. Let \(M,N\vDash T\), they are elementary submodels of \(\fC\). Since \(\tp_M(a)=\tp_N(b)\), there
