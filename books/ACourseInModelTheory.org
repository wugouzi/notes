...
** Prime extensions
   #+ATTR_LATEX: :options []
    ...
    #+LATEX: \wu{
    \(\varphi(x,\barc)\) isolates \(\tp(a_\alpha/A_\alpha)\).
    #+LATEX: }
    Then \(a_\alpha\) is also atomic over \(A\cup\{\barb\barc\}\). Using induction, we know
    that \(\barb\barc\) is atomic over \(A\). By Lemma ref:lemma5.3.5 applied
