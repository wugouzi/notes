...
** Strongly minimal sets
    ...
    2. The formula \(\varphi(\barx)\) is *strongly minimal* if \(\varphi(\barx)\) defines a minimal set in all
    ...
    #+LATEX: \wu{
    Suppose bijection \(f(\bara)=\barb\) iff \(\gamma(\bara,\barb)\). Then \(\psi(\barx)\)
    iff \(\exists\bary(\varphi(\bary)\wedge\gamma(\bary,\barx))\). Then for each \(\psi(\barx)\wedge\gamma(\barx)\), we can transform
    it into prenex form and analyze it.
    #+LATEX: }
    ...
    #+LATEX: \wu{
    as we can express this in first order language. Guess the power of infinitary disjunction \emoji{😅}
    #+LATEX: }
    ...
    #+LATEX: \wu{
    Whats the type of realisations? <<Problem5>>
    #+LATEX: }
    #+END_proof
    ...
    of \(\fM\)) and \(b'\) non-algebraic over \(A'\). Since all \(a'\in A'\) have the same
    type \(p(x,b')\) over \(b'\), no \(a'\) is algebraic over \(b'\). Thus also \(a\) is not
    algebraic over \(b\).
    #+END_proof
