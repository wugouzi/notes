...
* NIP groups
    [[https://ivv5hpp.uni-muenster.de/u/fjahn_01/mtf/Pierre2.pdf][Pierre's]] note.
** Connected components, revised
    ...
    Let \(p(x)\) be the type defining \(H\). We may assume \(p(x)\) is close by conjunction.
    ...
    * if \(\phi_n(x)\in p(x)\) is given, by compactness, there is \(\phi'_{n+1}(x)\in p(x)\) s.t.
    ...
    that \(G_\emptyset^{00}\subseteq G_A^{00}\))
