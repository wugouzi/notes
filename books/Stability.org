...
** Dividing and forking
    ...
    1. A formula \(\phi(x,a)\) *divides* over \(B\) if there is a sequence \((a_i)_{i\in\omega}\) and \(k\in\omega\)
       ...
       s.t. \(\{\phi(x,a_i)_{i\in\omega}\}\) is inconsistent (by compactness and ref:C2.4.8 <<Problem8>>)
       #+LATEX: \wu{
       \(\Leftarrow\): If for each \(k\in\omega\), there is \(\phi(x,a_{i_1}),\dots,\phi(x,a_{i_k})\) that are consistent. By
       indiscernibility, \(\{\phi(x,a_i)_{i\in\omega}\}\) is consistent

       \(\Rightarrow\): We can find an \(B\)-indiscernible sequence \((b_i)_{i\in\omega}\) realizing \(\EM(\bara/B)\).
       Thus \(b_i\equiv_Ba\) and \(\{\phi(x,a_i)\}_{i\in\omega}\) is inconsistent
       #+LATEX: }
...
** Special extensions of types
    ...
    * We would like to be able to choose a "generic" extension \(q\) of \(p\), s.t. it doesn't add
    ...
    #+BEGIN_proof
    1. Obvious
    2. Let \(p\) be the set of all corresponding formulas of \(\phi(x,b)\in q\)
    ...
    * \(q_2(x)=p(x)\cup\{x<b:M<b\in\M\}\)
    ...
    and \((d_r\phi(y))_{\phi(x,y)\in L}\) we must have \(d_q\phi(M)=d_r\phi(M)\). But again as \(M\prec\M\), this implies
