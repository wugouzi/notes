...
** The Ordering of \(\R\)
    ...
    1. Let \(P_1=\{a_n:n\in\N\}\) and let \(P_2=\{b_n:n\in\N\}\) be two such linearly ordered sets. We construct
       ...
       unbounded)
    2. Let \(C\) and \(C'\) be two complete dense unbounded linearly ordered sets, let \(P\)
       and \(P'\) be dense in \(C\) and \(C'\), and let \(f\) be an isomorphism of \(P\)
       onto \(P'\). Then \(f\) can be extended (uniquely) to an isomorphism \(f^*\) of \(C\)
       and \(C'\): for \(x\in C\), let \(f^*(x)=\sup\{f(p):p\in P\text{ and }p\le x\}\)
    #+END_proof
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    Let \((P,<)\) be a dense unbounded linearly ordered set. Then there is a complete unbounded
    linearly ordered set \((C,\prec)\) s.t.
