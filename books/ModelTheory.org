...
** Refinements of the method. Omitting types and interpolation theorems
   ...
   *generalized \(\omega\)-logic* we mean first order logic for the language
   ...
   A sentence \theta of \(\call_0'\) is said to *separate* \(T\) and \(U\) iff
   \begin{equation*}
   T\models\theta \quad\text{ and }\quad
   U\models\neg\theta
   \end{equation*}
   \(T\) and \(U\) are said to be *inseparable* iff no sentence \theta of \(\call_0'\)
   separates them.
   ...
   2. [@2] \(T_m\) and \(U_m\) are inseparable finite sets of sentences
   3. If \(U_m\cup\{\varphi_m\}\) and \(U_m\) are inseparable, then
      \(\varphi_m\in T_{m+1}\)

      If \(T_{m+1}\) and \(U_m\cup\{\psi_m\}\) are inseparable, then \(\psi_m\in
      U_{m+1}\)
   4. If \(\varphi_m=(\exists x)\sigma(x)\) and \(\varphi_m\in T_{m+1}\), then
   ...
   5. [@5] \(T_\omega\) is a maximal consistent theory in \(\call_1'\) and
   ...
   6. [@6] \(T_\omega\cap U_\omega\) is a maximal consistent theory in
      \(\call_0'\)
   ...
   that \(\Sigma(P)\) *defines \(P\) implicitly* iff
   \begin{equation*}
   ...
   \(R=R'\), \(\Sigma(P)\) is said to *define \(P\) explicitly* iff there exists a
   ...
   \(\fA\) to \(\call\) s.t. \(R\neq R'\). This is *Padoa's method*

   #+ATTR_LATEX: :options [Beth's Theorem]
   #+BEGIN_theorem
   ...
   \end{equation*}
   _By the compactness theorem, there exists finite subsets_
   _\(\Delta\subset\Sigma(P),\Delta'\subset\Sigma(P')\) s.t._
   \begin{equation*}
   \Delta\subset\Delta'\models P(c_1\dots c_n)\to P'(c_1\dots c_n)
   \end{equation*}
   Let \(\psi(P)\) be the conjunction of all \(\sigma(P)\in\Sigma(P)\) s.t. either
   \(\sigma(P)\in\Delta\) or \(\sigma(P')\in\Delta'\).Then
   \begin{equation*}
   \psi(P)\wedge\psi(P')\models P(c_1\dots c_n)\to P'(c_1\dots c_n)
   \end{equation*}
   Rearanging to get all symbols \(P\) on one side and all symbols \(P'\) on the
   other
   \begin{equation*}
   \psi(P)\wedge P(c_1\dots c_n)\models\psi(P')\to P'(c_1\dots c_n)
   \end{equation*}
   Then by the Craig interpolation theorem, there is a sentence \(\theta(c_1\dots
   c_n)\) of \(\call\cup\{c_1\dots c_n\}\) s.t.
   \begin{align*}
   &\psi(P)\wedge P(c_1\dots c_n)\models\theta(c_1\dots c_n)\\
   &\theta(c_1\dots c_n)\models\psi(P')\to P'(c_1\dots c_n)
   \end{align*}
   But any model \((\fA,R')\) for \(\call\cup\{P',c_1,\dots,c_n\}\) is also a
   model for \(\call\cup\{P,c_1,\dots,c_n\}\), when we interpret \(P\) by
   \(R'\). Thus
   \begin{equation*}
   \theta(\c_1\dots c_n)\models\psi(P)\to P(c_1\dots c_n)
   \end{equation*}
   Hence
   \begin{equation*}
   \psi(P)\models P(c_1\dots c_n)\leftrightarrow\theta(c_1\dots c_n)
   \end{equation*}
   Since \(c_1,\dots,c_n\) do nor occur in \(\psi(P)\), we have
   \begin{equation*}
   \psi(P)\models\forall x_1\dots x_n[P(x_1\dots x_n)\leftrightarrow\theta(x_1\dots x_n)]
   \end{equation*}
   where \(x_1,\dots,x_n\) are variables not occurring in \(\theta(c_1\dots c_n)\)
   #+END_proof

** Countable models of complete theories
   Consider a complete theory \(T\) in \(\call\). A formula \(\varphi(x_1\dots x_n)\)
   is said to be *complete (in \(T\))* iff for every formula \(\psi(x_1\dots x_n)\)
   exactly one of
   \begin{equation*}
   T\models\varphi\to\psi,\quad
   T\models\varphi\to\neg\psi
   \end{equation*}
   holds. A formula \(\theta(x_1\dots x_n)\) is said to be *completable (in \(T\))* iff
   there is a complete formula \(\varphi(x_1,\dots,x_n)\) with
   \(T\models\varphi\to\theta\). If \(\theta(x_1\dots x_n)\) is not completable it is
   said to be *incompletable*

   A theory \(T\) is said to be *atomic* iff every formula of \(\call\) which is
   consistent with \(T\) is completable in \(T\). A model \(\fA\) is said to be
   an *atomic model* iff every \(n\)-tuple \(a_1,\dots,a_n\in A\) satisfies a
   complete formula in \(\Th(\fA)\)

   #+ATTR_LATEX: :options []
   #+BEGIN_examplle
   1. Let \(T\) be a complete theory and let \(c_0,c_1,\dots\) be constant
      symbols of \(\call\). Then any formula of \(\call\) of the form
      \begin{equation*}
      x_0\equiv c_0\wedge x_1\equiv c_1\wedge\dots\wedge
      x_n\equiv c_n
      \end{equation*}
      is complete in \(T\). If \(\fA\) is a model of \(T\) s.t. every element of
      \(A\) is a constant, then \(\fA\) is an atomic model.
   #+END_examplle

   #+ATTR_LATEX: :options [Existence Theorem for Atomic Models]
   #+BEGIN_theorem
   Let \(T\) be a complete theory. Then \(T\) has a countable atomic model iff
   \(T\) is atomic
   #+END_theorem

   #+BEGIN_proof
   First assume that \(T\) has an atomic model \(\fA\). Let \(\varphi(x_1\dots x_n)\)
   be consistent with \(T\). Then, since \(T\) is complete,
   \begin{equation*}
   T\models(\exists x_1\dots x_n)\varphi(x_1\dots x_n)
   \end{equation*}
   Let \(a_1,\dots,a_n\in A\) satisfy \varphi, and let \(\psi(x_1\dots x_n)\) be a
   complete formula satisfied by \(a_1,\dots,a_n\). Then we cannot have
   \(T\models\psi\to\neg\varphi\). So we must have \(T\models\psi\to\varphi\).
   Hence \varphi is completable and \(T\) is atomic

   Now assume \(T\) is atomic. For each \(n<\omega\), let \(\Gamma_n(x_1\dots
   x_n)\) be the set of all negations of complete formulas \(\psi(x_1\dots x_n)\)
   in \(T\). Then every formula \(\varphi(x_1\dots x_n)\) which is consistent with
   \(T\) is completable, and hence \(\varphi\wedge\neg\gamma\) is consistent
   with \(T\) for some \(\gamma\in\Gamma_n\). Therefore \(T\) locally omits each
   set \(\Gamma_n(x_1\dots x_n)\). By the extended omitting types theorem, \(T\)
   has a countable model \(\fA\) which omits each \(\Gamma_n\). Then each
   \(a_1\dots a_n\in A\) satisfies a complete formula, whence \(\fA\) is an
   atomic model
   #+END_proof
