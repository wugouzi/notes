...
** Examples and Fraïssé's Theorem
   ...
   *Prove that for arbitrary \tau that \(\EVEN[\tau]\) is not axiomatizable.* <<Problem1>>
   ...
   introduced in the preceding example. For \(j\ge2\) any \(p\in I_j\) preserves \(E\) too, that is,
   \(I_j\subseteq\Part(\cala_l,\cala_k)\). If \(E^{A_l}ab\) for some \(a,b\in A_l\). Then
   \(d_j(a,b)=4\) or \(2l\) or \(2l-2\) <<Problem2>>


   Hence \((I_{j+1})_{j\le m-2}:\cala_l\cong_{m-2}\cala_k\)
   and we have
   ...
   \begin{equation*}
   \text{if }l,k\ge2^m\text{ then }\calg_l\cong\calg_k\text{ and }\calg_l\cong_m
