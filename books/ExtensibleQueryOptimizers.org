...
*** Sketches
        ...
        *Hyperloglog*. The Hyperloglog (HLL) sketch is used to estimate the number of distinct values in a
        ...
        the effect of \(m\) experiments and thereby significantly improves accuracyn, while using only a single
...
** Case Study: Cost Estimation in Microsoft SQL Server
*** Cost Model
*** Statistics
* Plan Management
** Plan Caching and Invalidation
** Improving Sub-optimal Plans with Execution Feedback
        *Handling regressions due to plan change*: 
