...
** Definitions of Relative Computability
   ...
   *independent* of the oracle \(A\)
   ...
   2. \(\{e\}^\sigma(x)\downarrow\Leftrightarrow(\exists s)[\{e\}_s^\sigma(x)=y]\)
   ...
   #+ATTR_LATEX: :options []
   #+BEGIN_definition
   1. Let \(W_e^A=\dom\{e\}^A\) and similarly define \(W_{e,s}^A\),
      \(W_e^\sigma\) and \(W_{e,s}^\sigma\)
   2. Let \(\Phi_e^A(x)=\Phi_e(A;x)=\{e\}^A(x)\) and
      \(\Phi_{e,s}(A;x)=\{e\}_s^A(x)\)
   3. Let \(\{e\}(x)\) denote \(\{e\}^\emptyset(x)\)
   #+END_definition

   #+ATTR_LATEX: :options []
   #+BEGIN_definition
   1. \(B\) is *recursive in (Turing reducible to)* \(A\), written \(B\le_T A\),
      if \(B=\{e\}^A\) for some \(e\).  (We identify sets with their
      characteristic functions.)
   2. \(B\) is *recursively enumerable* in \(A\) if \(B=W_e^A\) for some \(e\).
   3. \(B\) is in \(\Sigma_1^A\)-form (abbreviated \(B\) is \(\Sigma_1^A\)) if
   ...
   The following are equivalent
   1. \(B\) is r.e. in \(A\)
   2. \(B=\emptyset\) or \(B\) is the range of some \(A\)-recursive total function
   3. \(B\) is \(\Sigma_1^A\)
   #+END_theorem
...
*** Exercises
    ...
    #+BEGIN_exercise
    label:eq3.1.15
    1. Prove that \(\{\la e,\sigma,x,y,s\ra:\{e\}_s^\sigma(x)=y\}\) is recursive
    2. Prove that \(\{\la e,\sigma,x,y\ra:\{e\}^\sigma(x)=y\}\) is r.e., and that this set
