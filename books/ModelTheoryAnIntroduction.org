...
*** Henkin Constructions
    ...
    *Claim* $T_1$ is finitely satisfiable
    ...
    #+ATTR_LATEX: :options [stengthening of Compactness Theorem]
    #+BEGIN_theorem
    label:thm2.1.11
...
** Complete Theories
   ...
   $\cdot:F\times V\to V$ 
   1. Associativity of addition: $\bu+(\bv+\bw)=(\bu+\bv)+\bw$
   2. commutativity of addition: $\bu+\bv=\bv+\bu$
   3. Identity element of addition: zero vector $\bv+\bzero=\bv$ for all $\bv\in
      V$
   4. inverse elements of addition: \(\forall \bv\in V\exists\)-\(\bv\in
      V,\bv+(\)-\(\bv)=\bzero\)
   5. compatibility of scalar multiplication with field multiplication:
      $a(b\bv)=(ab)\bv$
   6. identity element of scalar multiplication: $1\bv=\bv$
   ...
   p\text{-times}}=\bzero
   \end{equation*}
   If $\Q$ has characteristic $0$ and $V$ is not torsion-free, then
   \begin{equation*}
