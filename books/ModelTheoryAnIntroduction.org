...
** Quantifier Elimination
   ...
   \(\psi_1,\dots,\psi_n\in\Gamma\) s.t. <<Problem2>>
   \begin{equation*}
...
*** Prime Model Extensions of \texorpdfstring{\(\omega\)}{ω}-Stable Theories
    ...
    *Claim 1* \(\calm_0\prec\calm\).
    ...
    is \(b\in M\) s.t. \(\calm\vDash\phi(b,\bara)\) and \(\tp^{\calm}(b/A_\delta)\) <<Problem4>> is isolated. By Choice of
...
***  Countably Saturated Models
    ...
    with \(\tp^{\calm_{\alpha+1}}(\barb,d)=\tp^{\calm_{\alpha+1}}(\bara,c)\). <<Problem6>>
...
*** Existence of Saturated Models
    ...
    *Claim* For any \(\calm\) there is \(\calm\prec\calm'\) s.t. \(\abs{M'}\le\abs{M}^\kappa\), and if \(A\subseteq M\), \(\abs{A}\le\kappa\)
    ...
    for \(n<\omega\), \(A\subseteq M\) with \(\abs{A}\le\kappa\).
    #+END_proof
