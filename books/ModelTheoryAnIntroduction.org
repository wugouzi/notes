...
** Forking and Independence
    ...
    #+LATEX: \wu{
    If \(M\) is \(\aleph_0\)-saturated, then if \(\varphi\) is \alpha-minimal, then it is \(\alpha\)-strongly minimal
    #+LATEX: }
    ...
    1. Let \(\calm\) be an \(\aleph_0\)-saturated model containing \(B\).
        #+LATEX: \wu{
        Only after assuming \(\calm\) is \(\aleph_0\)-saturated can we find such \(\call_M\)-formula \(\psi(\barv)\)
        #+LATEX: }
