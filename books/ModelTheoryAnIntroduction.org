...
** Exercise
    ...
    1#+BEGIN_proof
    Suppose for all finite sets \(I_0\subseteq I\),
    \begin{equation*}
    \calm\vDash\bigvee_{i\in I}\phi_i(\barv)\to\neg\bigvee_{i\in I_0}\phi_i(\barv)
    \end{equation*}
    Then there is a formula \(\phi_{I_0}\)
    \begin{equation*}
    \calm\vDash\exists\barv(\phi_{I_0}(\barv)\to\bigvee_{i\in I_0}\phi(\barv))
    \end{equation*}


    Then there is a formula \(\phi_{i_0}\) and \(\barc_{I_0}\) s.t.
    \begin{equation*}
    \calm\vDash\phi_{I_0}(\barc_{I_0})\to\neg\bigvee_{i\in I_0}\phi(\barc_{I_0})
    \end{equation*}
    Then we have
    \begin{equation*}
    \Gamma=\{\phi_{I_0}(\barc_{I_0})\to\bigvee_{i\in I_0}\phi(\barc_{I_0})\}
    \end{equation*}
    This is finitely satisfiable and hence realized in \(\)
    #+END_proof
