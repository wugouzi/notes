...
*** Morley Degree
    ...
    all \(n\) and \(\psi_1(\M),\psi_2(\M),\dots\) are disjoint subsets of \(\phi(\M)\). But then \(\RM(\phi)\ge\alpha+1\), a
    ...
    #+LATEX: \wu{
    Lemma ref:lemma5.2.5 needs compactness, the new model is elementarily embedded into \(\M\) by
    Lemma ref:lemma4.3.17.
    #+LATEX: }
...
*** Ranks of Types
    ...
    Because \(\RM(\phi)=\infty\ge\beta+2\), we can find an \(\call_{\M}\)-formula \psi s.t. \(\RM(\phi\wedge\psi)\ge\beta+1\) and \(\RM(\phi\wedge\neg\psi)\ge\beta+1\)
    #+END_proof
