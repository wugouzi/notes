...
\begin{equation*}
    ...
    2. Suppose that \(\abs{S_n^{\calm}(A)}>\aleph_0\). We claim that \(\abs{S_n^{\calm}(A)}=2^{\aleph_0}\).
       ...
       is a formula \phi s.t. \(\abs{[\phi]}>\aleph_0\)

       *Claim* if \(\abs{[\phi]}>\aleph_0\), there is an \(\call_A\)-formula \psi s.t. \(\abs{[\phi\wedge\psi]}>\aleph_0\)
       and \(\abs{[\phi\wedge\neg\psi]}>\aleph_0\)

       Suppose not. Let \(p=\{\psi(\barv):\abs{[\phi\wedge\psi]}>\aleph_0\}\). We claim that \(p\) is satisfiable.
       ...
       \end{equation*}
       and if \(f\neq g\), then \(p_f\neq p_g\). Thus \(\abs{S_n^{\calm}}=2^{\aleph_0}\)
    #+END_proof
...
*** Countable Homogeneous Models
    ...
    Thus \(tp^{\caln}(\barc)=\tp^{\caln}(\barb)\). Because \(\caln\) is homogeneous, there is \(e\in N\) s.t.
...
*** Prime Model Extensions of \omega-Stable Theories
    ...
    argument from Theorem ref:thm4.2.11
    #+END_proof
