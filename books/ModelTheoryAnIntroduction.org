...
*** The Random Graph
    ...
    *Claim* There is a graph \(G_1\supseteq G_0\) s.t. \(G_1\) is countable and if
    ...
    \(X=\{j\le m:R(\alpha_j,a_i)\}\) and let \(Y=\{j\le m:\neg R(\alpha_j,a_i)\}\). Because
    \(G_2\vDash T\), we can find \(b\in G_2\) s.t. \(G_2\vDash R(f_s(\alpha_j),b)\) for \(j\in X\) and
    ...
    The same back-and-forth shows that random graph has quantifier elimination
...
*** Zariski Closed and Constructible Sets
    ...
    we let
    \(I(Y)=\{f\in K[X_1,\dots,X_n]:f(\overbar{a})=0\text{ for all}\overbar{a}\in Y\}\).
    ...
    #+BEGIN_proof
    1. If \(X_0\supset X_1\supset X_2\supset\cdots\) is a descending sequence of Zariski closed sets,
       then \(I(X_0)\subset I(X_1)\subset I(X_2)\subset\cdots\) is an ascending sequence of prime ideals contradicting
       Hilbert's Basis Theorem
    2. Suppose not. Then we can find \(X_1,X_2,\dots\) Zariski closed sets s.t.
       \begin{equation*}
       \bigcap_{i=1}^{n+1}X_i\subset\bigcap_{i=1}^nX_i
       \end{equation*}
       for \(n=1,2,\dots\) contradicting 1
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    Let \(K\) be a field. The subsets of \(K^n\) defined by atomic formulas are exactly those of the
    form \(V(p)\) for some \(p\in K[\barX]\). A subset of \(K^n\) is quantifier-free definable iff it
    is a Boolean combination of Zariski closed sets
    #+END_lemma

    #+BEGIN_proof
    If \(\phi(\barx,\bary)\) is an atomic, then there is \(q(\barX,\barY)\in\Z[\barX,\barY]\)
    s.t. \(\phi(\barx,\bary)\) is equivalent to \(q(\barx,\bary)=0\).
    If \(X=\{\barx:\phi(\barx,\bara)\}\), then \(X=\{V(q(\barX,\bara))\}\)
    and \(q(\barX,\bara)\in K[\barX]\). On the other hand, if \(p\in K[\barX]\), there
    is \(q\in\Z[\barX,\barY]\) and \(\bara\in K^m\) s.t. \(p(\barX)=q(\barX,\bara)\). Then \(V(p)\) is
    defined by the quantifier-free formula \(q(\barX,\bara)=0\)

    If \(X\) is Zariski closed, then by Hilbert basis, then by Hilbert's Basis Theorem, there
    are \(p_1,\dots,p_l\) s.t.
    \begin{equation*}
    X=V(p_1,\dots,p_n)=V(p_1)\cap\cdots\cap V(p_n)
    \end{equation*}
    Because the quantifier-free definable sets are exactly finite Boolean combinations of atomic
    definable sets, the quantifier-free definable sets are exactly the Boolean combination of
    Zariski closed sets.
    #+END_proof

    If \(X\subseteq K^n\) is a finite Boolean combination of Zariski closed sets we call \(X\) *constructible*.
    If \(K\) is algebraically closed, the constructible sets have much stronger closure properties

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    Let \(K\) be an algebraically closed field
    1. \(X\subseteq K^n\) is constructible iff it is definable
    2. (Chevalley's Theorem) The image of a constructible set under a polynomial map is
       constructible
    #+END_corollary

    #+BEGIN_proof
    1. by quantifier elimination
    2. Let \(X\subseteq K^n\) be constructible and \(p:K^n\to K^m\) be a polynomial map. Then the image
       of \(X=\{y\in K^m:\exists x\in K^n\;p(x)=y\}\) . This set is definable and hence constructible

    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    If \(K\) is an algebraically closed field and \(X\subseteq K\) is algebraically closed, then
    either \(X\) or \(K\setminus X\) is finite. Thus, \(\ACF\) is strongly minimal
    #+END_corollary

    Lemma ref:lemma3.2.4 shows that the map \(X\mapsto I(X)\) is a lattice inverting map from Zariski
    closed subsets of \(K^n\) to radical ideals in \(K[X]\). Hilbert's Nullstellensatz says that this
    is a bijection and for \(I\) a radical ideal \(I=I(V(I))\). The model-completeness of
    algebraically closed fields can be used to give a proof of the Nullstellensatz

    #+ATTR_LATEX: :options [Primary Decomposition]
    #+BEGIN_fact
    label:lemma3.2.10
    If \(I\subset K[\barX]\) is a radical ideal, then there are prime ideals \(P_1,\dots,P_m\) containing \(I\)
    s.t. \(I=P_1\cap\cdots\cap P_m\), \(I\neq\bigcup_{j\in J}P_j\) for any proper \(J\subset\{1,\dots,m\}\) and if \(Q_1,\dots,Q_n\) is another
    set of prime ideals with these properties, then \(n=m\) and \(\{Q_1,\dots,Q_n\}=\{P_1,\dots,P_m\}\)
    #+END_fact

    #+ATTR_LATEX: :options [Hilbert's Nullstellensatz]
    #+BEGIN_theorem
    Let \(K\) be an algebraically closed field. Suppose that \(I\) and \(J\) are radical ideals
    in \(K[X_1,\dots,X_n]\) and \(I\subset J\). Then \(V(J)\subset V(I)\). Thus \(X\mapsto I(X)\) is a bijective
    correspondence between Zariski closed sets and radical ideals
    #+END_theorem

    #+BEGIN_proof
    Let \(p\in J\setminus I\). By Lemma ref:lemma3.2.10, there is a prime ideal \(P\supseteq I\) s.t. \(p\notin P\). We
    will show that there is \(x\in V(P)\subseteq V(I)\) s.t. \(p(x)\neq 0\). Thus \(V(I)\neq V(J)\). Because \(P\)
    is prime, \(K[\barX]/P\) is a domain and we can take \(F\), the algebraic closure of its
    fraction field
    #+END_proof
...
** Partition Theorems
    ...
    #+BEGIN_proof
    Induction on \(n\). For \(n=0\), \(\kappa^+\to(\kappa^+)^{1}_\kappa\) is just the Pigeonhole Principle
    ...
    \end{equation*}
    Thus we can find \(X_{\alpha+1}\supseteq X_\alpha\) s.t. \(\abs{X_{\alpha+1}}=\beth_n(\kappa)\) and if \(Y\subset X_\alpha\)
    with \(\abs{Y}=\beth_{n-1}(\kappa)\) and \(\beta\in\lambda\setminus Y\), then there is \(\gamma\in X_{\alpha+1}\setminus Y\) s.t.
    \(f_\beta|[Y]^n=f_\gamma|[Y]^n\).
    #+LATEX: \wu{
    There are \(\beth_n(\kappa)\) many such \(Y\). And the number of such functions guarantee we can find
    such \gamma.
    #+LATEX: }

    Let \(X=\bigcup_{\alpha<\beth_{n-1}(\kappa)^+}X_\alpha\). If \(Y\subset X\) with \(\abs{Y}\le\beth_{n-1}(\kappa)\), then \(Y\subset X_\alpha\) for
    some \(\alpha<\beth_{n-1}(\kappa)^+\). If \(\beta\in\lambda\setminus Y\), then there is \(\gamma\in X\setminus Y\) s.t. \(f_\beta|[Y]^n=f_\gamma|[Y]^n\)

    Fix \(\delta\in\lambda\setminus X\). Inductively construct \(Y=\{y_\alpha:\alpha<\beth_{n-1}(\kappa)^+\}\subseteq X\). Let \(y_0\in X\). Suppose that
    we have constructed \(Y_\alpha=\{y_\beta:\beta<\alpha\}\). Choose \(y_\alpha\in X\) s.t. \(f_{y_\alpha}|[Y_\alpha]^n=f_\delta|[Y_\alpha]^n\)

    By the induction hypothesis, there is \(Z\subseteq Y\) s.t. \(\abs{Z}\ge\kappa^+\) and \(Z\) is homogeneous
    for \(f_\delta\). Say \(f_\delta(B)=\gamma\) for all \(B\in[Z]^n\). We claim that \(Z\) is homogeneous
    for \(f\). Let \(A\in[Z]^{n+1}\). There are \(\alpha_1<\dots<\alpha_{n+1}\) s.t. \(A=\{y_{\alpha_1},\dots,y_{\alpha_{n+1}}\}\).
    Then
    \begin{equation*}
    f(A)=f_{y_{\alpha_{n+1}}}(\{y_{\alpha_1},\dots,y_{\alpha_n}\})=f_\delta(\{y_{\alpha_1},\dots,y_{\alpha_n}\})=\gamma
    \end{equation*}
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    \(\beth_{\alpha+n}^+\to(\beth_\alpha^+)_{\beth_\alpha}^{n+1}\)
    #+END_corollary

    #+BEGIN_proof
    \(\beth_{\alpha+n}=\beth_n(\beth_\alpha)\)
    #+END_proof
...
** Forking and Independence
    ...
    ref:lemma6.2.2, \(\RM(\varphi_i(x,\barc_i))=\RM(\varphi_i(x,\bara_i))\)
    #+END_proof
    ...
    3. Suppose \(q_1\neq q_2\) and \(\RM(p)=\RM(q_1)=\RM(q_2)\) and \(\deg(p)=\deg(q_1)=\deg(q_2)\). Then
       there is a \(\psi\in q_1\) and \(\neg\psi\in q_2\) and \phi is still the formula with minimal rank and degree
       in \(q_1\) and \(q_2\). Then \(\RM(\phi\wedge\psi)=\RM(\phi\wedge\neg\psi)\), a contradiction
...
*** Definable Types
    ...
    #+LATEX: \wu{
    Crucial point is that Morley rank is definable
    #+LATEX: }
    ...
    *Claim* For each \(\barc\in\M\) s.t. \(\RM(\phi(\barv)\wedge\psi(\barv,\barc))=\alpha\), there is a finite
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    Suppose that \(A\subseteq M\) and \(X\subseteq M^n\) is \(A\)-definable, then any definable \(Y\subseteq X^m\) is \(A\cup X\)-definable
    #+END_corollary

    #+BEGIN_proof
    Let \(\psi(\barv,\barb)\) define \(Y\). Then \(Y=\{\barc\in X^n:\psi(\barc,\barb)\in\tp(\barb/X)\}\).
    Because \(\tp(\barb/X)\) is definable over \(X\), \(Y\) is definable over \(A\cup X\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    Suppose that \(p\in S_n(A)\) and \(\deg_M(p)=1\). Then \(p\) is definable over \(A\).
    If \(B\supseteq A\), let
    \begin{equation*}
    q=\{\psi(\barv,\barb):\M\vDash d_p\psi(\barb),\barb\in B,\psi\in\call\}
    \end{equation*}
    Then \(q\) is the unique nonforking extension of \(p\) to \(B\) and \(q\) is definable
    over \(A\) - indeed, we can take \(d_q\psi=d_p\psi\) for all \(\psi\in\call\)
    #+END_proposition

    #+BEGIN_proof
    Suppose that \(p\in S_n(A)\), \(\RM(p)=\alpha\) and \(\deg_M(p)=1\). Let \(\phi(\barv)\) be
    an \(\call_A\)-formula with \(\RM(\phi)=\alpha\) and \(\deg_M(p)=1\). If \(\psi(\barv,\barw)\) is
    an \(\call\)-formula, there is an \(\call_A\)-formula \(d_p\psi\) s.t. \(\RM(\phi\wedge\psi(\barv,\bara))=\alpha\)
    iff \(\M\vDash d_p\phi(\bara)\). Suppose \(A\subseteq B\), the proof of Theorem ref:thm6.3.2 shows that
    \begin{equation*}
    q=\{\psi(\barv,\barb):\barb\in B\wedge\RM(\phi(\barv)\wedge\psi(\barv,\barb))=\alpha,\psi\in\call\}
    \end{equation*}
    is the unique nonforking extension of \(p\), but
    \begin{equation*}
    q=\{\psi(\barv,\barb):\M\vDash d_p\psi(\barb),\barb\in B,\psi\in\call\}
    \end{equation*}
    Thus \(q\) is definable over \(A\). Indeed we can use \(d_q\phi=d_p\phi\) to define \(q\)
    #+END_proof

    If \(\deg_M(p)>1\), the situation is more complicated. We will show that any nonforking
    extension of \(p\in S_n(A)\) is definable over the algebraic closure of \(A\) in \(\M^{\eq}\)

    Suppose \(\calm\) is \(\aleph_0\)-saturated, \(A\subseteq M\), \(p\in S_n(A)\), \(q\in S_n(M)\), \(p\subseteq q\). \(\RM(p)=\RM(q)=\alpha\)
    and \(\deg_M(q)=1\).  Let \(\phi(\barv)\) and \(\psi(\barv,\barw)\) be \(\call_A\)-formulas
    and \(\barb\in M\) s.t. \(\phi(\barv)\in p\) is a formula of minimal rank and degree
    and \(\psi(\barv,\barb)\in q\) has Morley rank \alpha and Morley degree 1. We may assume
    that \(\psi(\barv,\barw)\) implies \(\phi(\barv)\)

    Let \(X\) be the set of all \(\barc\) s.t. \(\RM(\psi(\barv,\barc))=\alpha\) and \(\forall\bard\) if \(\RM(\psi(\barv,\bard))=\alpha\) then
    either \(\RM(\psi(\barv,\barc)\wedge\psi(\barv,\bard))<\alpha\) or \(\RM(\psi(\barv,\barc)\wedge\neg\psi(\barv,\bard))<\alpha\). By
    theorem ref:thm6.3.5 \(X\) is definable over \(A\)
    #+LATEX: \wu{
    \(\RM(\psi(\barv,\barc))=\alpha\) iff \(\theta(\barc)\). \(\RM(\psi(\barv,\barc)\wedge\psi(\barv,\bard))=\alpha\)
    iff \(\theta'(\barc,\bard)\). Then \(\barc\in X\) iff \(\theta(\barc)\wedge\forall\barx\neg\theta'(\barc,\barx)\)
    #+LATEX: }

    Define \(E\subseteq X\times X\) by
    \begin{equation*}
    (\barc,\bard)\in E\Leftrightarrow\RM(\psi(\barv,\barc)\wedge\psi(\barv,\bard))=\alpha
    \end{equation*}

    *Claim* \(E\) is an \(A\)-definable equivalence relation with at most \(\deg_M(\phi)\) equivalence
    classes

    By Theorem ref:thm6.3.5, \(E\) is \(A\)-definable. It is clear that \(E\) is symmetric and
    reflexive. Suppose \(\barc E\bard\) and \(\neg(\barc E\bare)\).
    Then \(\RM(\psi(\barv,\bard)\wedge\neg\psi(\barv,\barc))<\alpha\) and \(\RM(\psi(\barv,\barc)\wedge\psi(\barv,\bare))<\alpha\).
    Because \(\psi(\barv,\bare)\wedge\psi(\barv,\bard)\) implies
    \begin{equation*}
    (\psi(\barv,\bard)\wedge\neg\psi(\barv,\barc))\vee(\psi(\barv,\barc)\wedge\psi(\barv,\bare))
    \end{equation*}
    #+ATTR_LATEX: :width .3\textwidth
    #+NAME:
    #+CAPTION:
    [[../images/ModelTheoryAnIntroduction/2.jpg]]
    \(\RM(\psi(\barv,\bard)\wedge\psi(\barv,\bare))<\alpha\). Thus \(\neg(\bard E\bare)\) and \(E\) is an equivalence
    relation

    If \(\barb_1,\dots,\barb_m\in X\) are \(E\)-inequivalent, then \(\psi(\barv,\barb_i)\) defines a Morley rank
    \alpha subset of \(\phi(\M)\) and \(\RM(\psi(\barv,\barb_i)\wedge\psi(\barv,\barb_j))<\alpha\) for \(i\neq j\).
    Thus \(m\le\deg_M(\phi)\)

    Suppose that \(\barc E\barb\). Because \(\deg_M(\psi(\barv,\barb))=1\), \(\RM(\psi(\barv,\barc))=\alpha\)
    and \(\RM(\psi(\barv,\barc)\wedge\neg\psi(\barv,\barb))<\alpha\), \(\psi(\barv,\barc)\) is also a formula in \(q\) of
    Morley rank \alpha and Morley degree 1. Thus \(q\) is definable over \(A\cup\barc\). In fact, for any
    formula \(\theta(\barv,\barz)\) and \(\bara\in M\), \(\theta(\barv,\bara)\in q\)
    iff \(\RM(\psi(\barv,\barc)\wedge\theta(\barv,\bara))=\alpha\). In particular, there is
    an \(\call_A\)-formula \(\theta^*(\barw,\barz)\) s.t. \(\theta(\barv,\bara)\in q\) iff \(\theta^*(\barc,\bara)\). The
    definition of \(q\) depends on \(\barb/E\) rather than the choice of \(\barc\)

    We think of \(\barb/E\) as an element of \(\M^{\eq}\). Because \(E\) is definable over \(A\) and
    has only finitely many equivalence classes \(\barb/E\) is algebraic over \(A\) in \(\M^{\eq}\).

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    Suppose that \(A\subseteq B\), \(p\in S_n(A)\), \(q\in S_n(B)\), and \(q\) does not fork over \(A\). There
    is \(\alpha\in\acl^{\eq}(A)\) s.t. \(q\) is definable over \(A\cup\{a\}\). In other words, there is
    an \(A\)-definable equivalence relation \(E\) with finitely many classes and \(\bara\in\M\) s.t.
    for any \(\call\)-formula \(\phi(\barv,\barw)\) there is an \(\call_A\)-formula \(d_p\phi(\barw,\barz)\) s.t.
    if \(\barb E\bara\) then
    \begin{equation*}
    \phi(\barv,\bard)\in p \quad\text{ iff }\quad d_p\phi(\bard,\barb)
    \end{equation*}
    for all \(\bard\in B\)

    If \(\deg_M(p)=1\), then any nonforking extension of \(p\) is definable over \(A\)
    #+END_theorem

    #+BEGIN_proof
    We choose \(\calm\), an \(\aleph_0\)-saturated model containing \(B\), and \(q^*\in S_n(M)\), a nonforking
    extension of \(q\). The argument above shows that \(q^*\), and hence \(q\) is definable over \(\acl^{eq}(A)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    If \(\calm\vDash T\), \(p\in S_n(\calm)\), \(M\subseteq B\) and \(q\in S_n(B)\) is a nonforking extension of \(p\),
    then \(q\) is definable over \(\calm\)
    #+END_corollary
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(p\in S_n(A)\) is *stationary* if, for all \(B\supseteq A\), there is a unique nonforking extension
    of \(p\) to \(B\)
    #+END_definition
    ...
    #+BEGIN_corollary
    label:cor6.3.12
    Let \(\calm\vDash T\) and let \(\phi(\barv)\) be an \(\call_M\)-formula with \(\RM(\phi)=\alpha\) and \(\deg_M(\phi)=d\)
    1. There is an \(\call_M\)-formula \(\theta(\barv)\) s.t. \(\theta(\M)\subseteq\phi(\M)\), \(\RM(\theta)=\alpha\), and \theta has Morley
       rank 1
    2. There are \(\call_M\)-formulas \(\theta_1,\dots,\theta_d\) s.t. each \(\theta_i\) has Morley rank \alpha and Morley degree
       1 and \(\phi(\M)\) is the disjoint union of \(\theta_1(\M),\dots,\theta_d(\M)\)
    3. If \(p\in S_n(M)\), then \(p\) has Morley degree 1. In particular, \(p\) is stationary
    #+END_corollary

    #+BEGIN_proof
    1. In our argument above, we had \(\psi(\barv,\barw)\) and \(\barb\in\M\) and an \(M\)-definable
       equivalence relation \(E\) with finitely many classes s.t. if \(\bara E\barb\),
       then \(\psi(\M,\bara)\) is a Morley rank \alpha, Morley degree 1 subset of \(\phi(\M)\). Because \(E\) has
       only finitely many classes, there is \(\bara\in M\) s.t. \(\bara E\barb\),
       so \(\psi(\barv,\bara)\) is the desired formula.
       #+LATEX: \wu{
       Each class is not empty since \(\calm\prec\M\)
       #+LATEX: }
    2. We inductively define \(\theta_1,\dots,\theta_d\). Fr \(m<d\), if we are given \(\theta_1,\dots,\theta_{m-1}\), use 1 to
       find \(\theta_m\) defining a Morley rank \alpha, Morley degree 1 subset of
       \begin{equation*}
       \phi(\M)\setminus\bigcup_{i=1}^{m-1}\theta_i(\M)
       \end{equation*}
       #+LATEX: \wu{
       \(\phi\leftrightarrow\bigvee_{i=1}^d\phi_i\) where \(\phi_i\in\call_M\). Since \(\deg(\theta_i)=1\), there is at most one \(\phi_j\)
       s.t. \(\RM(\theta_i\wedge\phi_j)=\alpha\). Hence for each \(i<d\), \(\RM(\phi\wedge\bigwedge_{i=1}^i\neg\theta_i)=\alpha\)
       #+LATEX: }
       Let
       \begin{equation*}
       \theta_d=\phi\wedge\bigwedge_{i=1}^{d-1}\neg\theta_i
       \end{equation*}
    3. If \(p\in S_n(M)\), choose \(\phi(\barv)\in p\) s.t. \(\RM(\phi)=\RM(p)\), and let \(\theta_1,\dots,\theta_d\) be as
       in 2. Then \(p\) must contain one of the formulas \(\theta_i\), so \(p\) has Morley rank 1. Because
       the number of nonforking extensions is at most the Morley degree, \(p\) is stationary
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    If \(p\in S_n(\acl^{\eq}(A))\), then \(p\) is stationary
    #+END_corollary
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    Suppose that \(\calm\vDash T\), \(p\in S_n(M)\), \(M\subseteq B\), \(q\in S_n(B)\), \(p\subseteq q\), and \(q\) is definable
    over \(M\). Then \(q\) is a nonforking extension of \(p\)
    #+END_lemma

    #+BEGIN_proof
    There is \(r\in S_n(B)\) a nonforking extension of \(p\) that is definable over \(M\).
    Because \(q\) and \(r\) both extends \(p\)
    \begin{equation*}
    \calm\vDash\forall\barw(d_r\phi(\barw)\leftrightarrow d_q\phi(\barw))
    \end{equation*}
    for all formulas \(\phi(\barv,\barw)\). Because \(\calm\prec\M\), \(\M\vDash\forall\barb(d_r\phi(\barb)\leftrightarrow d_q\phi(\barb))\).
    Thus \(p=q\)
    #+END_proof
*** Independence
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    We say that \(\bara\) is *independent* from \(B\) over \(A\) if \(\tp(\bara/A)\) does not fork
    over \(A\cup B\), i.e., \(\RM(\bara/A)=\RM(\bara/A\cup B)\). We write \(\bara\ind_AB\)
    #+END_definition

    #+ATTR_LATEX: :options [Monotonicity]
    #+BEGIN_lemma
    If \(\bara\ind_AB\) and \(C\subseteq B\), then \(\bara\ind_AC\)
    #+END_lemma

    #+BEGIN_proof
    \(\RM(\bara/A)\ge\RM(\bara/A\cup C)\ge\RM(\bara/A\cup B)\), if \(\RM(\bara/A)=\RM(\bara/A\cup B)\),
    then \(\RM(\bara/A)=\RM(\bara/A\cup C)\)
    #+END_proof

    #+ATTR_LATEX: :options [Transitivity]
    #+BEGIN_lemma
    \(\bara\ind_A\barb,\barc\Leftrightarrow\bara\ind_A\barb\)  and \(\bara\ind_{A,\barb}\barc\)
    #+END_lemma

    #+ATTR_LATEX: :options [Finite Basis]
    #+BEGIN_lemma
    \(\bara\ind_AB\Leftrightarrow\bara\ind_AB_0\)  for all finite \(B_0\subseteq_fB\)
    #+END_lemma

    #+BEGIN_proof
    \(\Leftarrow\). Suppose that \(\bara\nind_AB\), then there is \(\phi(\barv)\in\tp(\bara/A\cup B)\)
    with \(\RM(\phi)<\RM(\bara/A)\). Let \(B_0\) be a finite subset of \(B\) s.t. \(\phi\in\call_{A\cup B_0}\),
    then \(\bara\nind_AB_0\)
    #+END_proof

    #+ATTR_LATEX: :options [Symmetry]
    #+BEGIN_lemma
    If \(\bara\ind_A\barb\), then \(\barb\ind_A\bara\)
    #+END_lemma

    #+BEGIN_proof
    Suppose \(\RM(\bara/A\barb)=\RM(\bara/A)\). We need to show
    that \(\RM(\barb/A\bara)=\RM(\barb/A)\). Let \(\alpha=\RM(\bara/A)\) and \(\beta=\RM(\barb/A)\)

    We first assume that \(A=M\), an \(\aleph_0\)-saturated model. Let \(\phi(\barv)\in\tp(\bara/M)\)
    s.t. \(\alpha=\RM(\phi)\) and \(\deg_M(\phi)=1\). Let \(\psi(\barv)\in\tp(\barb/M)\) s.t. \(\beta=\RM(\psi)\)
    and \(\deg_M(\psi)=1\). Suppose, for purposes of contradiction, that there is a
    formula \(\theta(\barv,\barw)\in\call_A\) s.t. \(\M\vDash\theta(\bara,\barb)\) and \(\RM(\theta(\bara,\barw))<\beta\). By Theorem
    ref:thm6.3.5, there is an \(\call_M\)-formula \(\chi(\barv)\)
    defining \(\{\barx:\RM(\psi(\barw)\wedge\theta(\barx,\barw))<\beta\}\). Because \(\RM(\bara/M,\barb)=\alpha\), the
    formula
    \begin{equation*}
    \phi(\barv)\wedge\theta(\barv,\barb)\wedge\chi(\barv)
    \end{equation*}
    has rank \alpha. Thus by Lemma ref:lemma6.3.4, there is \(\bara'\in M\) s.t. \(\theta(\bara',\barb)\)
    and \(\RM(\psi(\barw)\wedge\theta(\bara',\barw))<\beta\), contradicting the fact that \(\RM(\barb/M)=\beta\)

    For the general case, let \(\calm\) be an \(\aleph_0\)-saturated model containing \(A\). Let \(\barb'\)
    realize a nonforking extension of \(\tp(\barb/A)\) to \(\calm\). Then \(\RM(\barb'/M)=\beta\).
    Because \(\M\) is saturated, there is \(\barc\) s.t. \(\tp(\bara,\barb/A)=\tp(\barc,\barb'/A)\).
    Let \(\bara'\) realize a nonforking extension of \(\tp(\barc/A,\barb')\) to \(M,\barb'\).
    Then \(\RM(\bara'/M\barb')=\RM(\barc/A\barb')=\RM(\bara/A\barb)=\alpha\)
    and \(\RM(\bara'/M=\RM(\barc/A)=\RM(\bara/A))=\alpha\). By the  first part of the
    proof, \(\RM(\barb'/M,\bara')=\RM(\barb'/M)=\beta\). Hence
    \begin{equation*}
    \RM(\barb/A,\bara)=\RM(\barb'/A,\barc)=\RM(\barb'/A,\bara')\ge\RM(\barb'/M,\bara')=\beta
    \end{equation*}
    Thus \(\RM(\barb/A,\bara)=\RM(\bara/A)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    \(\bara,\barb\ind_AC\Leftrightarrow\bara\ind_AC\) and \(\barb\ind_{A,\bara}C\)
    #+END_corollary

    #+BEGIN_proof
    Because forking occurs over a finite subset, it suffices to assume that \(C\) is a finite
    sequence \(\barc\)
    \begin{align*}
    \bara,\barb\ind_A\barc&\Leftrightarrow\barc\ind_A\bara,\barb\quad\text{symmetry}\\
    &\Leftrightarrow\barc\ind_A\bara\wedge\barc\ind_{A,\bara}\barb\quad\text{transitivity}\\
    &\Leftrightarrow\bara\ind_A\barc\wedge\barb\ind_{A,\bara}\barc\quad\text{symmetry}
    \end{align*}
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    For any \(\bara\), \(\bara\ind_A\acl(A)\)
    #+END_corollary

    #+BEGIN_proof
    Suppose \(\barb\in\acl(A)\). By Lemma ref:lemma6.2.7 (3), \(\RM(\barb/A,\bara)=\RM(\barb/A)=0\).
    Thus \(\barb\ind_A\bara\) and by symmetry, \(\bara\ind_A\barb\)
    #+END_proof
...
*** Constructible Models
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    Let \(\calm\) be an \(\call\)-structure and let \(A\subseteq M\). Let \delta be an ordinal and \((a_\alpha:\alpha<\delta)\) be a
    sequence of elements from \(M\). Let \(A_\alpha=A\cup\{a_\beta:\beta<\alpha\}\) . We call \((a_\alpha:\alpha<\delta)\) a *construction*
    over \(A\) if \(\tp(a_\alpha/A_\alpha)\) is isolated for all \(\alpha<\delta\)

    \(B\subseteq M\) is *constructible* over \(A\) if there is a construction \((a_\alpha:\alpha<\delta)\)
    s.t. \(B=A\cup\{a_\alpha:\alpha<\delta\}\). We say that a model \(\calm\) is constructible over \(A\) if \(M\) is
    constructible over \(A\)
    #+END_definition

    Our proof of Theorem ref:thm4.2.20 can be broken into the following two assertions about
    constructible models

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    label:lemma6.4.2
    1. If \(T\) is \omega-stable, \(\calm\vDash T\), and \(A\subset M\), there is \(\caln\prec\calm\) constructible over \(A\)
    2. If \(\calm\) is constructible over \(A\), then \(\calm\) is prime over \(A\) and every type realized
       in \(\calm\) is isolated over \(A\)
    #+END_lemma

    We will prove the uniqueness of prime model extension in two steps. We will first prove
    Ressayre's Theorem that constructible extensions are unique for any theory. We will then prove
    that in \omega-stable theories all prime model extensions are constructible

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    Suppose that \((a_\alpha:\alpha<\delta)\) is a construction of \(\calm\) over \(A\). For each \(\alpha<\delta\),
    let \(\theta_\alpha(v)\) be a formula with parameters from \(A_\alpha\) isolating  \(\tp(a_\alpha/A_\alpha)\). We say
    that \(C\subseteq M\) is *sufficient* if whenever \(a_\alpha\in C\), then all parameters from \(\theta_\alpha\) are in \(C\)
    #+END_definition

    #+LATEX: \wu{
    Construction of \(\calm\) over \(A\) means that \(\calm\) is constructible over \(A\)

    Since \(\theta_\alpha\in L(A_\alpha)\), actually \(\theta_\alpha\in L(C\setminus\{a_\alpha\})\)
    #+LATEX: }

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    label:lemma6.4.4
    Suppose that \((a_\alpha:\alpha<\delta)\) is a construction of \(\calm\) over \(A\)
    1. Each \(A_\alpha\) is sufficient
    2. If \(C_i\) is sufficient for all \(i\in I\), then \(\bigcup_{i\in I}C_i\) is sufficient
    3. If \(X\subseteq M\) is  finite, then there is a finite sufficient \(C\supseteq X\)
    4. If \(C\subseteq M\) is sufficient, then \(M\) is constructible over \(A\cup C\)
    #+END_lemma

    #+BEGIN_proof
    3. [@3] Prove by induction on \alpha that if \(X\subseteq A_\alpha\) is finite, then there is a finite
       sufficient \(C\supseteq X\). This is clear if \(\alpha=0\) or \alpha is a limit ordinal. Suppose \(\alpha=\beta+1\)
       and \(X=X_0\cup\{a_\beta\}\) where \(X_0\subseteq A_\beta\). Let \(B\subseteq A_\beta\) be the parameters from \(\theta_\beta\). By
       induction, there is a finite sufficient \(C\supseteq X_0\cup B\). Then \(C\cup\{a_\beta\}\) is a finite sufficient
       set containing \(X\)
    4. We must show that \(\tp(a_\alpha/A_\alpha\cup C)\) is isolated for all \(\alpha<\delta\). If \(a_\alpha\in C\), this is
       trivial so we will assume that \(a_\alpha\notin C\). In this case, we claim that \(\theta_\alpha\)
       isolates \(\tp(a_\alpha/A_\alpha\cup C)\). Suppose not. Then there is an \(\call_A\)-formula \(\psi(v,\barw)\)
       and \(\barb\in C\) s.t. \(\psi(a_\alpha,\barb)\) but
       \begin{equation*}
       \Th_{A_\alpha\cup C}(\calm)\not\vDash\theta_\alpha(v)\to\psi(v,\barb)
       \end{equation*}
       Thus \(\theta_\alpha\) doesn't isolate \(\tp(a_\alpha/A_\alpha,\barb)\). By 2 and 3, we may assume
       that \(A_\alpha,\barb\) is sufficient.
       #+LATEX: \wu{
       Since \(\barb\) is finite, there is a finite sufficient \(C\supseteq\barb\). But again, \(\theta_\alpha\)
       doesn't isolate \(\tp(a_\alpha/A_\alpha,C)\). What's more, \(A\) is isolated and hence \(A_\alpha\cup C\) is isolated.
       #+LATEX: }
       We may assume that \(\barb=(a_{\alpha_1},\dots,a_{\alpha_n})\)
       where \(\alpha<\alpha_1<\dots<\alpha_n\). Note that \(A_\alpha\cup\{a_{\alpha_1},\dots,a_{\alpha_m}\}\) is sufficient for \(m=1,\dots,n\)

       *Claim* For each \(i=1,\dots,m\), there is an \(\call_{A_\alpha}\)-formula
       isolating \(\tp(a_{\alpha_1},\dots,a_{\alpha_m}/A_{\alpha+1})\)

       We prove this by induction on \(m\). Suppose that the claim holds for \(l<m\). The
       formula \(\theta_{\alpha_{l+1}}\) isolates \(\tp(a_{l+1}/A_{\alpha_{l+1}})\). All parameters occurring
       in \(\theta_{\alpha_{l+1}}\) are in \(A_\alpha\cup\{a_{\alpha_1},\dots,a_{\alpha_l}\}\). Thus \(\theta_{\alpha_{l+1}}\)
       isolates \(\tp(a_{\alpha_{l+1}}/A_{\alpha+1}\cup\{a_{\alpha_1},\dots,a_{\alpha_l}\})\)

       Let \(\theta_{\alpha_{l+1}}\) be \(\psi(a_{\alpha_1},\dots,a_{\alpha_l},v_{l+1})\) where \(\psi(v_1,\dots,v_{l+1})\) is
       an \(\call_{A_\alpha}\)-formula. By induction there is an \(\call_{A_\alpha}\)-formula \(\phi(v_1,\dots,v_l)\)
       isolating \(\tp(a_{\alpha_1},\dots,a_{\alpha_l}/A_{\alpha+1})\). Thus \(\phi(v_1,\dots,v_l)\wedge\psi(v_1,\dots,v_{l+1})\)
       isolates \(\tp(a_{\alpha_1},\dots,a_{\alpha_{l+1}}/A_{\alpha+1})\) by Lemma ref:lemma4.2.21

       Let \(\psi(\barw)\) be an \(\call_{A_\alpha}\)-formula isolating \(\tp(\barb/A_{\alpha+1})\).
       Then \(\theta_\alpha(v)\wedge\psi(w)\) isolates \(\tp(a_\alpha,\barb/A_\alpha)\).
    #+END_proof

    #+ATTR_LATEX: :options [Uniqueness of Constructible Models]
    #+BEGIN_theorem
    Suppose that \(A\subseteq\M\), \(\calm\prec\M\), \(\caln\prec\M\), and \(\calm\) and \(\caln\) are constructible over \(A\). The
    identity map on \(A\) extends to an isomorphism between \(\calm\) and \(\caln\)
    #+END_theorem

    #+BEGIN_proof
    Let \((a_\alpha:\alpha<\delta)\) and \((b_\alpha:\alpha<\gamma)\) be the constructions of \(\calm\) and \(\caln\) over \(A\).
    Let \(\kappa=\abs{M}\). Let \(I\) be the set of partial elementary \(f:A\subset X\to N\) s.t. \(X\) is
    sufficient in \(\calm\), \(f\uhr A\) is the identity and \(\im(f)\) is sufficient in \(\caln\). Because the
    identity map on \(A\) is in \(I\) and the union of a chain of elements of \(I\) is an element
    of \(I\), we may apply Zorn's Lemma to get a maximal \(f:X\to N\) in \(I\). We claim that \(f\) is
    an isomorphism between \(\calm\) and \(\caln\).

    Suppose that \(a\in M\setminus X\). Let \(C_0\) be the finite sufficient subset of \(M\) with \(a\in C_0\).
    By Lemma ref:lemma6.4.4 (4) and ref:lemma6.4.2 (2) \(\tp(C_0/X)\) is isolated. Thus we can
    extend \(f\) to a partial elementary \(f_0:X\cup C_0\to N\). Let \(D_0\supseteq f_0(C_0)\) be a finite
    sufficient subset of \(N\). Because \(\tp(D_0/f(X))\) is isolated, by Exercise
    ref:ex4.5.11, \(\tp(D_0/f(X\cup C_0))\) is isolated since \(\tp(D,f(C_0)/f(X))=\tp(D_0/f(X))\) is
    isolated. Thus we can find a finite \(C_0'\supseteq C_0\) and a surjective partial
    elementary \(f_0':X\cup C_0'\to f(X)\cup D_0\). Let \(C_1\) be a finite sufficient set
    containing \(C_0'\). By the same argument, \(\tp(C_1/X\cup C_0')\) is isolated and we may
    extend \(f_0'\) to a partial elementary \(f_1:C_1\to N\).

    Continuing in this way, we have \(f_0:X\cup C_i\to N\) is partial elementary and \(f_i(C_i)\) is
    contained in a sufficient subset of \(f_{i+1}(C_{i+1})\).  Then \(g=\bigcup f_i\in I\), a contradiction
    #+END_proof
*** Prime Models of \texorpdfstring{\(\omega\)}{ω}-Stable Theories
    It remains to show that if \(T\) is \omega-stable, then any prime model extension is constructible

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    Let \(T\) be \omega-stable. If \(A\subseteq B\), \(p\in S_n(A)\), \(p'\in S_n(B)\), \(p'\) is a nonforking
    extension of \(p\), and \(p'\) is isolated, then \(p\) is isolated
    #+END_lemma
...
* Problems :ignore:
    [[https://people.math.wisc.edu/~lempp/teach/Marker-errata.html][more errata]]
