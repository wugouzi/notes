...
** Lagrange's Theorem
   ...
   *generated by* $X$
   ...
   multiplication. If $(x,m)=1$, then $rs+sm=1$. Therefore $(r,m)=1$. Each of
   them have inverse.
   #+END_proof
...
** Quotient group
   ...
   #+BEGIN_proof
   Define \(\Phi:Sub(G;K)\to Sub(G/K)\) by \(\Phi:S\mapsto S/K\)

   To see that \Phi is injective, we begin by showing that if \(K\le S\le G\), then \(\pi^{-1}\pi(S)=S\).
   Apparently \(S\subseteq\pi^{-1}\pi(S)\). Let \(a\in\pi^{-1}\pi(S)\), so that \(\pi(a)=\pi(s)\) for some \(s\in S\). It
   follows taht \(as^{-1}\in\ker\pi=K\), so that \(a=sk\) for some \(k\in K\). But \(K\le S\), and
   so \(a=sk\in S\).

   Assume now that \(\pi(S)=\pi(S')\), where \(S\) and \(S'\) are subgroups of \(G\) containing \(K\).
   Then \(\pi^{-1}\pi(S)=\pi^{-1}\pi(S')\), and so \(S=S'\) as we have just proved in the preceding. Hence
   \Phi is injective.

   To see that \Phi is surjective, let \(U\) be a subgroup of \(G/K\). Now \(\pi^{-1}(U)\) is a subgroup
   of \(G\) containing \(K=\pi^{-1}(\{1\})\) and hence \(\pi(\pi^{-1}(U))=U\) as \pi is surjective

   \(T\le S\le G\) implies \(T/K=\pi(T)\le\pi(S)\le S/K\). Conversely, assume that \(T/K\le S/K\) . If \(t\in T\),
   then \(tK\in T/K\le S/K\) and so \(tK=sK\) for some \(s\in S\). Hence \(t=sk\) for some \(k\in K\le S\),
   and so \(t\in S\).
...
** First Properties
   ...
   *Notation*. The tradition in ring theory is to write \(S\subseteq R\) for a
   ...
   Moreover, $F$ can be chosen so that for each $f\in  F$, there are \(a,b\in R\) with
   \(b\neq 0\) and \(f=ab^{-1}\) 
   #+END_theorem
...
** Homomorphisms
   ...
   1. Let $R$ be a domain and let \(F=\Frac(R)\). 
      \(R'=\{[a,1]:a\in R\}\subseteq F\), then the function \(f:R\to R'\) given by \(f(a) =[a,1]\), is an isomorphism
   ...
   \(0=\deg(2)=\deg(d)+\deg(f)\). The candidates for $d(x)$ are $\pm 1$ and
...
** Euclidean Rings
   ...
   3. The Gaussian integers \(\Z[i]\) form a euclidean ring whose degree
      ...
      \(\alpha/\beta=\alpha\overline{\beta}/\beta\overline{\beta}=\alpha\overline{\beta}/\partial{\beta}\), so that
      \begin{equation*}
      a/\beta=x+yi
      \end{equation*}
...
*** Vector Spaces
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_examplle
    Let \(V=\R^2\), let \(e_1=(1,0)\) and let \(e_2=(0,1)\). then \(V=\la e_1,e_2\ra\)
    #+END_examplle
    ...
    *Notation*. If \(v_1,\dots,v_m\) is a list, then
    \(v_1,\dots,\hatv_i,\dots,v_m\) is the shorter list with \(v_i\) deleted
    ...
    2. some \(v_i\) is in the subspace spanned by the others; that is
       \begin{equation*}
       v_i\in\la v_1,\dots,\hatv_i,\dots,v_m\ra
       \end{equation*}
    ...
    *dimension* denoted by \(\dim_k(V)\) or \(\dim(V)\), is the number of elements
    ...
    \begin{equation*}
    X'=y_n,x_1,\dots,\hatx_i,\dots,x_m
    \end{equation*}

    Now repeat this argument for the spanning list 
    \(y_{n-1},y_n,x_1,\dots,\hatx_i,\dots,x_m\). It follows that the
...
** Insolvability of the Quintic
   ...
   *separable* if each of its irreducible factor has no repeated roots.
   ...
   1. Let \(f(x)=x^3-1\in\Q[x]\). Now \(f(x)=(x-1)(x^2+x+1)\). Suppose roots of
      \(x^2+x+1\) is \(\omega\) and \(\bar{\omega}\). The splitting field of
      \(f(x)\) is \(\Q(\omega)\), for \(\omega^2=\bar{\omega}\), and so \([\Q(\omega),\Q]=2\).
...
*** Basis Theorem
    ...
    #+END_lemma
    #+BEGIN_proof
    1. By the Correspondence Theorem, \(p(G/S)=(pG+S)/S\), so that
       \begin{equation*}
       ...
       \end{equation*}
    2. We now analyze \((pG+S)/pG\), the kernel of \(G/pG\to G/(pG+S)\). By the
    ...
    #+ATTR_LATEX: :options [Basis Theorem]
    #+BEGIN_theorem
...
** Application of Zorn's Lemma
   ...
   *Zorn's lemma*. \hspace{0.1cm} /If $X$ is a nonempty partially ordered set in/
   ...
   #+ATTR_LATEX: :options []
   #+BEGIN_theorem
   Given a field $k$ , there exists an algebraical closure \(\bar{k}\) of $k$
   #+END_theorem
   ...
   #+BEGIN_lemma
   label:nlemma5.60
   Let $k$ be a field and let \(\bar{k}/k\) be an algebraic closure. If
   ...
   ordered pair \((E,f)\) an *approximation* if \(f:E\to\bar{k}\) is a \(k\)-map.
   ...
   irreducible \(p(x)\in E_0[x]\) having $a$ as a root. Since \(\bar{k}/k\) is
   algebraic and \(\bar{k}\) is algebraically closed, we have a factorization
   in \(\bar{k}[x]\):
   \begin{equation*}
   f_0^*(p(x))=\displaystyle\prod_{i=1}^n(x-b_i)
   \end{equation*}
   where \(f_0^*:E_0[x]\to\bar{k}[x]\) is the map
   \(f_0^*:e_0+\dots+e_nx^n\mapsto f_0(e_0)+\dots+f_0(e_n)x^n\). If all the
   \(b_i\) lie in \(f_0(E_0)\subseteq\bar{k}\), then
   ...
   ref:nthm2.144, we may define \(f_1:E_0(a)\to\bar{k}\) by
   \begin{equation*}
   ...
   *algebraically dependent* over $k$ if there exists a finite subset 
   ...
   isomorphism \(\widetilde{\varphi}:k(x_1,\dots,x_n)\to k(u_1,\dots,u_n)\subseteq E\), where
