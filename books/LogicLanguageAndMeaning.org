...
#+LATEX_HEADER: \DeclareMathOperator{\bTtwo}{\bT\btwo}
...
** Temporal Operators
   IL is a straightforward extension of our formulasim in which the temporal
   operators \(\bP,\bF,\bH,\bG\) are added to the syntax. The semantics is
   adapted by adding to the models a set of temporal moments \(T\) linearly
   ordered by a relation <.
   5. [@5] \(\bD_{\la s,a\ra,D,W,T}=\bD^{W\times T}_{a,D,W,T}\)


   \(W\times T\) is the set of all ordered pairs \(\la w,t\ra\) in which \(w\)
   is a world and \(t\) a moment in time.

   7. [@7] If \(\phi\in\WE^L_t\), then \(\llbracket\Box\phi\rrbracket_{\bM,\la
      w,t,\ra,g}=1\) iff for all \(w'\in W\) and all \(t'\in T\):
      \(\llbracket\phi\rrbracket_{\bM,\la w',t'\ra,g}=1\)
   8. If \(\alpha\in\WE_a^L\), then \(\llbracket^\wedge\alpha\rrbracket_{\bM,\la
      w,t\ra,g}\) is that function \(h\in\bD^{W\times T}_a\) s.t. for all
      \(w'\in W\) and all \(t'\in T\): \(h(\la
      w',t'\ra)=\llbracket\alpha\rrbracket_{\bM,\la w',t'\ra,g}\)
   9. If \(\alpha\in\WE_{\la s,a\ra}^L\), then
      \(\llbracket^\vee\alpha\rrbracket_{\bM,\la
      w,t\ra,g}=\llbracket\alpha\rrbracket_{\bM,\la w,t\ra,g}(\la w,t\ra)\)


   Now \(\Box\) means *necessarily at all times*

** Two-Sorted Type Theory
   #+ATTR_LATEX: :options []
   #+BEGIN_definition
   \(\bT\btwo\) is the minimal set s.t.
   1. \(e,t,s\in\T\btwo\)
   2. If \(a,b\in\bTtwo\), then \(\la a,b\ra\in\bTtwo\)
   #+END_definition

   The set \(\bT\) of types for intensional type theory is a proper subset of \(\bTtwo\).
   ...
   2. \(\bD_{t,D,W}=\{0,1\}\)
   3. \(\bD_{s,D,W}=W\)
   4. \(\bD_{\la a,b\ra,D,W}=\bD_{b,D,W}^{\bD_{a,D,W}}\)
   #+END_definition
