...
** A Consistency Proof of \(\PA\)
   ...
   #+END_remark
      ...
      #+BEGIN_remark
      As an application of Gentzen's reduction method, one can easily show the following

      The consistency of arithmetic in which the induction formula are restricted to those which
      have at most \(k\) quantifiers can be proved by transfinite induction on \(\omega_{k+1}\). The
      outline of the proof is as follows. Suppose there is a proof of \(\to\) in this system.
      1. We assume that the induction formulas are in prenex normal form
      2. A formula \(A\) in a proof (in this system) will be temporarily called free if either is
         has no ancestor but a logical
   #+END_remark
