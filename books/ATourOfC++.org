...
Vector f() {
  ...
  return z; // we get a move
    ...
    [[https://stackoverflow.com/questions/9779079/why-does-c11-have-implicit-moves-for-value-parameters-but-not-for-rvalue-para][Implicit Move]]: The only time a && variable will be implicitly moved from (ie: without ~std::move~)
...
** Resource Management
    ...
    Consider a standard-library ~thread~ representing a concurrent activity and a ~Vector~ of
...
auto v = init(1'000'000);             // start heartbeat and initialize v
    ...
    maintainable code, without added overhead. In particular, we can achieve *strong resource safety*;
...
*** Comparisons
    To give identical treatment to both operands of a binary operator, such as ~==~, it is best
...
*** User-Defined Literals
    ...
    * ~"Surpriese"~ is a ~std::string~
    * ~123s~ is \texttt{second}s
    * ~12.7i~ is ~imaginary~ so that ~12.7i+47~ is a ~complex number~
    ...
    literals with user-defined suffixes are called *user-defined literals* or *UDLs*. Such literals
    are defined using *literal operators*. A literal operator converts a literal of its argument type,
    followed by a subscript, into its return type. For example, the ~i~ for ~imaginary~ suffix might be
...
** Advice
    ...
    3. Far large operands, use ~const~ reference argument types
* Template
** Parameterized Types
...
class Vector {
 ...
 public:
  ...
  int size() const { return sz; }
    ...
    The ~template<typename T>~ prefix makes ~T~ a parameter of the declaration it prefixes. It is C++'s
...
void f2(Vector<string>& vs)    // Vector of some strings
{
  ...
  in the compilation process, at *instantiation time*, code is generated for each instantiation used
  in a program
*** Contrained Template Arguments (C++20)
    ...
    just a ~typename~ but an ~Element~ where "~Element~" specifies the requirements of a type that can
    be an element:
    #+begin_src c++
template<Element T>
class Vector {
 private:
  ...
  // ...
    ...
    #+end_src
    This ~template<Element T>~ prefix is C++'s version of mathematic's "for all T such that
    ~Element(T)~"; that is, ~Element~ is a predicate that checks whether T has all the properties that
    a ~Vector~ requires. Such a predicate is called a *concept*. A template argument for which a concept
    is specified is called a *constrained argument* and a template for which an argument is
    constrained is called a *constrained template*.
...
struct Buffer
{
  ...
  // ...
    ...
    #+end_src
    The alias (~value_type~) and the ~constexpr~ function are provided to allow users access to the
    template arguments.
...
pair<int,double> p = {1,5.2};
    ...
    offers a function, ~make_pair()~, that deduces the template arguments of the ~pair~ it returns from
...
Vector vs3 {"Hello"s, "World"};        // error: the initializer list is not homogenous
    #+end_src
    The type of a C-style string literal is ~const char*~. If that was not what was intended, use the
    ~s~ suffix to make it a proper ~string~.
...
void f(const Vector<int>& vec, const list<string>& lst, int x, const string& s)
{
  ...
  cout << "number of values less than " << s << ": " << count(lst,Less_than{s}) << '\n';
    ...
    3. for a simple function object like ~Less_than~, inlining is simple, so a call of ~Less_than~ is
    ...
    referred to as *policy objects*.
*** Lambda Expression
    The notation ~[&](int a){ return a<x; }~ is called a lambda expression. It generates a function
    object exactly like ~Less_than<int>{x}~. The ~[&]~ is a *capture list* specifying that all local names
    used in the lambda body (such as ~x~) will be accessed through references.

    Had we wanted to "capture" only ~x~, we could have said so: ~[&x]~. Had we wanted to give the
    generated object a copy of ~x~, we could have said so: ~[=x]~. Capturing nothing is ~[]~, capture all
    local names by reference is ~[&]~, and capture all local nmaes used by value is ~[=]~.
...
void rotate_and_draw(vector<S>& v, int r) {
  for_all(v,[](auto& s){ s−>rotate(r); s−>draw(); });
    ...
    #+end_src
    Here, like in variable declarations, ~auto~ means that any type is accepted as an initializer (an
    argument is considered to initialize the formal parameter in a call). This makes a lambda with
    an ~auto~ parameter a template, a ~generic lambda~. For reasons lost in standards committee
    politics, this use of ~auto~ is not currently allowed for function arguments.
** Template Mechanisms
*** Variable Templates
    ...
    of type ~T~ and other types depending on ~T~.
...
String_map<int> m; // m is a Map<string,int>
    #+end_src
*** Compile-Time ~if~
    Consider writing an operation that can use one of two operations ~slow_and_safe(T)~ or
    ~simple_and_fast(T)~. The traditional solution is to write a pair of overloaded functions and
    select the most appropriate based on a trait (ref:trait), such as the standard-library ~is_pod~.
    If a class hierarchy is involved, a base class can provide the ~slow_and_safe~ general operation
    and a derived class can override with a ~simple_and_fast~ implementation.
...
** Advice
    1. There is no separate compilation of templates: ~#include~ template definitions in every
...
Num sum(Seq s, Num v)
{
  ...
  return v;
    ...
    ~sum()~ requires that its first template argument is some kind of sequence and its second template
    argument is some kind of number. We call such requirements *concepts*.
*** Use of Concepts
...
Num sum(Seq s, Num v)
{
  ...
  return v;
    ...
    Once we have defined what the concepts ~Sequence~ and ~Number~ mean, the compiler can reject bad
    calls by looking at ~sum()~'s interface only, rather than looking at its implementation. This
    improves error reporting.

    However, the specification of ~sum()~'s interface is not complete: we should be able to add
    elements of a ~Sequence~ to a ~Number~. We can do that
    #+begin_src c++
template<Sequence Seq, Number Num>
...
Num sum(Seq s, Num n);
    ...
    The ~Value_type~ of a sequence is the type of the elements in the sequence. ~Arithmetic<X,Y>~ is a
    concept specifying that we can do arithmetic with numbers of types ~X~ and ~Y~. This saves us from
    accidentally trying to calculate the ~sum()~ of a ~vector<string>~ or a ~vector<int∗>~ while still
    accepting ~vector<int>~ and ~vector<complex<double>>~.
...
** Strings
    ~string~ is a ~Regular~ type for owning and manipulating a sequence of characters of various
    character types.
*** ~string~ Implementation
    ~string~ is usually implemented using the *short-string optimization*. That is, short string values
    are kept in the ~string~ object itself and only longer strings are placed on free store. Consider
    #+begin_src c++
...
** String Views
    ...
    To address this, the standard library offers ~string_view~; a ~string_view~ is basically a
    (pointer,length) pair denoting a sequence of characters:

    #+ATTR_LATEX: :width .8\textwidth
    #+NAME:
    #+CAPTION:
    [[../images/ATourOfC++/5.png]]

    A ~string_view~ gives access to a contiguous sequence of characters. The characters can be stored
    in many possible ways, including in a ~string~ and in a C-style string. A ~string_view~ is like a
...
std::literals::string_view_literals;
    ...
    The behavior of out-of-range access to a ~string_view~ is unspecified. If you want guaranteed range
    checking, use ~at()~, which throws ~out_of_range~ for attempted out-of-range access, use a
    ~gsl::string_span~, or ‘‘just be careful.’’
** Regular Expressions
    In ~<regex>~, the standard library provides support for regular expressions in the form of the
...
for (string line; getline(cin,line); ) {   // read into line buffer
  ...
  if (regex_search(line,matches,pat))      // search for pat in line
    ...
    expression stored in ~pat~ and if it finds any matches, it stores them in matches. If no match was
    found, ~regex_search(line,matches,pat)~ returns false. The matches variable is of type ~smatch~. The
    ‘‘s’’ stands for ‘‘sub’’ or ‘‘string,’’ and an ~smatch~ is a vector of submatches of type string.
    The first element, here ~matches[0]~, is the complete match. The result of a ~regex_search()~ is a
    collection of matches, typically represented as an ~smatch~:
    #+begin_src c++
void use() {
...
vector<int> read_ints(istream& is)
{
  ...
  return res;
    ...
    What is happening here is that the operation ~is>>i~ returns a reference to is, and testing an
...
struct Entry {
  ...
  int number;
    ...
    #+end_src
    We can define a simple output operator to write an ~Entry~ using a {"name",number} format similar
    ...
    // read { "name" , number } pair. Note: formatted with { " " , and }
{
  ...
  is.setstate(ios_base::failbit); // register the failure in the stream return is;
    ...
    The ~is>>c~ skips whitespace by default, but ~is.get(c)~ does not, so this ~Entry~-input operator
    ignores (skips) whitespace outside the name string, but not within it. For example:
** Formatting
    The ~iostream~ library provides a large set of operations for controlling the format of input and
    out- put. The simplest formatting controls are called ~manipulators~ and are found in ~<ios>~,
    ~<istream>~, ~<ostream>~, and ~<iomanip>~ (for manipulators that take arguments). For example, we can
...
// print 1234, 4d2, 2322
    ...
    * The *scientific* format (~scientific~) presents a value with one digit before a decimal point and
    ...
    • The *fixed* format (~fixed~) presents a value as an integer part followed by a decimal point and
...
** String Streams
    ...
    * \texttt{istringstream}s for reading from a ~string~
    * \texttt{ostringstream}s for writing to a ~string~.
    * \texttt{stringstream}s for reading from and writing to a ~string~.
** C-style I/O
...
void f(int i, int j) // X* vs. unique_ptr<X>
{
  ...
  delete p;                 // destroy *p
    ...
    #+end_src
    Here, we "forgot" to delete ~p~ if ~i<99~ or if ~j<77~. On the other hand, ~unique_ptr~ ensures that its
    object is properly destroyed whichever way we exit ~f()~
    ...
    // make an X and immediately give it to a unique_ptr
{
  ...
  return unique_ptr<X>{new X{i}};
    ...
    A ~unique_ptr~ is a handle to an individual object (or an array) in much the same way that a
    ~vector~ is a handle to a sequence of objects. Both control the lifetime of other objects (using
    RAII) and both rely on move semantics to make ~return~ simple and efficient.

    The ~shared_ptr~ is similar to ~unique_ptr~ except that \texttt{shared\_ptr}s are copied rather than
    moved.
...
void user(const string& name, ios_base::openmode mode)
{
  ...
  // ...
    ...
    Now, the file opened by ~fp~'s constructor will be closed by the last function to (explicitly or
    implicitly) destroy a copy of ~fp~. Note that ~f()~ or ~g()~ may spawn a task holding a copy of ~fp~
    or in some other way store a copy that outlives ~user()~. Thus, ~shared_ptr~ provides a form of
    garbage collection that respects the destructor-based resource management of the memory-managed
    objects.

    The standard library (in ~<memory>~) provides functions for constructing an object and returning
    an appropriate smart pointer, ~make_shared()~ and ~make_unique()~. For example:
    #+begin_src c++
struct S {
  int i;
  string s;
...
auto p2 = make_unique<S>(2,"Oz",7.62);
    ...
    * A shared polymorphic object typically requires \texttt{shared\_ptr}s.
*** ~move()~ and ~forward()~
    The choice between moving and copying is mostly implicit. A compiler will prefer to move
    when an object is about to be destroyed (as in a ~return~) because that’s assumed to be the
    simpler and more efficient operation. However, sometimes we must be explicit. For example, a
    ~unique_ptr~ is the sole owner of an object. Consequently, it cannot be copied:
    #+begin_src c++
void f1() {
...
void f1() {
  ...
  // ...
    ...
    Confusingly, ~std::move()~ doesn't move anything. Instead, it casts its argument to an rvalue
    refer- ence, thereby saying that its argument will not be used again and therefore may be moved
    (§5.2.2). It should have been called something like ~rvalue_cast~. Like other casts, it's
...
v.push_back(move(s2)); // use a move constructor
    ...
    the ~push_back()~ of ~s2~ cheaper. The problem is that a moved-from object is left behind. If we use
    ~s2~ again, we have a problem:
    #+begin_src c++
cout << s1[2]; // write ’l’
cout << s2[2]; // crash?
    #+end_src
...
*** ~array~
    An ~array~, defined in ~<array>~, is a fixed-size sequence of elements of a given type where the
...
void h() {
  ...
  p1[3].draw();    //disaster
    ...
    a ~Shape∗~ gives a wrong offset. All standard containers provide this advantage over built-in
    arrays.
** Type Functions
    A *type function* is a function that is evaluated at compile time given a type as its argument or
    returning a type.

    For numerical types, numeric_limits from <limits> presents a variety of useful information. For
    example:
    #+begin_src c++
...
constexpr int szi = sizeof(int); // the number of bytes in an int
    ...
    features is often called ~metaprogramming~ or (when templates are involved) template
    ~metaprogramming~ .
*** ~iterator_traits~
    The standard-library ~sort()~ takes a pair of iterators supposed to define a sequence. Furthermore, those iterators must offer random access to that sequence, that is, they must
    be *random-access iterators*. Some containers, such as ~forward_list~, do not offer that. In
    particular, a ~forward_list~ is a singly-linked list so subscripting would be expensive and
    there is no reasonable way to refer back to a previous element. However, like most containers,
    ~forward_list~ offers forward iterators that can be used to traverse the sequence by algorithms
    and ~for~-statements .

    The standard library provides a mechanism, ~iterator_traits~, that allows us to check which kind
    of iterator is provided. Given that, we can improve the range ~sort()~ from ref:12.8 to accept
    either a ~vector~ or a ~forward_list~. For example:
    #+begin_src c++
...
void sort(C& c)
{
  ...
  sort_helper(c.begin(),c.end(),Iterator_category<Iter>{});
    ...
    #+end_src
    Here, I use two type functions: ~Iterator_type<C>~ returns the iterator type of ~C~ (that is,
    ~C::iterator~) and then ~Iterator_category<Iter>{}~ constructs a "tag" value indicating the kind of
    iterator provided:
    ...
    * ~std::forward_iterator_tag~ if ~C~'s iterator supports forward iteration
    ...
    called *tag dispatch*, is one of several used in the standard library and elsewhere to improve
...
using Iterator_type = typename C::iterator
    ...
    standard-library support for tag dispatch comes in the form of a class template ~iterator_traits~
    from ~<iterator>~. The specialization for pointers looks like this:
    #+begin_src c++
template<class T>
...
// Iter’s category
    ...
    Now an ~int∗~ can be used as a random-access iterator despite not having a member type;
    ~Iterator_category<int∗>~ is ~random_access_iterator_tag~.

    Many traits and traits-based techniques will be made redundant by concepts (§7.2). Consider the
    concepts version of the ~sort()~ example:
    #+begin_src c++
...
*** ~enable_if~
    ...
    and \texttt{enable\_if}s. The standard-library ~enable_if~ is a widely used mechanism for conditonally
...
class Smart_pointer {
  ...
  // -> is defined if and only if T is a class
    ...
    If ~is_class<T>()~ is true, the return type of ~operator−>()~ is ~T&~; otherwise, the definition of
    ~operator−>()~ is ignored.
* Concurrency
** Waiting for Events
