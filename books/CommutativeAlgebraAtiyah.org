...
:END:
    ...
    Or, since \(x_1,\dots,x_n\) is a nontrivial solution, \(\det(B)=0\).
...
\begin{center}\begin{tikzpicture}[>=angle 90,scale=2.2,text height=1.5ex, text depth=0.25ex]
  ...
  \end{tikzpicture}\end{center}
    ...
    [[https://math.mit.edu/~phaine/files/Diagram_Chases.pdf][nice illustration]]
...
** Exercises
    ...
    *Claim* The module \(M\otimes_AN\) is canonically isomorphic to \(\bigoplus_{i\in I}(M_i\otimes_AN)\) for regardless of
    ...
    Or, define \(\phi:M[x]\to A[x]\otimes_AM\) by \(\sum m_ix^i\mapsto\sum(x^i\otimes m_i)\)
    #+END_proof
    ...
    #+BEGIN_proof
    If \(\fp\) is prime, then \((A/\fp)[x]\) is a domain. By Exercise ref:ex2.6, this is isomorphic
    to \(A[x]\otimes_A(A/\fp)\) as an \(A[x]\)-module. Exercise ref:ex2.2 shows that this is isomorphic
    to \(A[x]/\fp[x]\) as an \(A\)-module. The composite map \(f:(A/\fp)[x]\to A[x]/\fp[x]\) is
    an \(A\)-linear map s.t. \(f(\sum\bara_ix^n)=\bbar{\sum a_ix^i}\). Thus \(f\) is an \(A[x]\)-algebra
    isomorphism, \(A[x]/\fp[x]\) is a domain

    \(\Z[x]/(2\Z)[x]\cong \F_2[x]\) is not maximal as \(x\) is not a unit
    #+END_proof
    ...
    #+BEGIN_proof
    \(M\twoheadrightarrow M/\fa M\twoheadrightarrow N/\fa N\), we must have \(u(M)+\fa N=N\). Then by
    ref:2.7, \(\fa N=N\)

    Consider the diagram
    \begin{center}\begin{tikzcd}
    0\ar[r]&\fa M\ar[r]\ar[d,"v"]&M\ar[r]\ar[d,"u"]&M/\fa M\ar[r]\ar[d,"w"]&0\\
    0\ar[r]&\fa N\ar[r]&N\ar[r]&N/\fa N\ar[r]&0
    \end{tikzcd}\end{center}
    Applying the snake lemma we have
    \begin{equation*}
    \coker v\xrightarrow{f}\coker u\to\coker w
    \end{equation*}
    Note that \(\im f=\fa\coker u=\fa N/M\).

    Suppose \(w\) is surjective, then \(f\) is surjective
    \begin{equation*}
    \coker u=\im f=\fa\coker u
    \end{equation*}
    and \(\coker u\) is finite generated because it is a quotient of \(N\). By Nakayama's
    Lemma, \(\coker u=0\)
    #+END_proof
    ...
    #+BEGIN_proof
    Let \(\fm\) be a maximal ideal of \(A\) and let \(\phi:A^m\to A^n\) be a isomorphism.
    Then \(1\otimes\phi:(A/\fm)\otimes A^m\to(A/\fm)\otimes A^n\) is an isomorphism between vector spaces of dimensions \(m\)
    and \(n\) over the field \(k=A/\fm\). Hence \(m=n\)

    Nope, since \(\N\times\N\to\N\) is injective
    #+END_proof

    #+BEGIN_exercise
    label:ex2.12
    Let \(M\) be a finitely generated \(A\)-module and \(\phi:M\to A^n\) a surjective homomorphism. Show
    that \(\ker\phi\) is finitely generated
    #+END_exercise

    #+BEGIN_proof
    Let \(e_1,\dots,e_n\) be a basis of \(A^n\) and choose \(u_i\in M\) s.t. \(\phi(u_i)=e_i\).
    Then \(M=\ker(\phi)\oplus(u_1,\dots,u_n)\)

    For any \(m\in M\), \(\phi(m)=\sum a_ie_i\) and hence \(m\in\phi^{-1}(\sum a_ie_i)=\sum a_iu_i+\ker(\phi)\). Thus there
    is \(m'\in\ker\phi\) s.t. \(m=\sum a_iu_i+m'\) and we define \(h(m)=(\sum a_iu_i,m')\)
    #+END_proof

    #+BEGIN_exercise
    label:ex2.13
    Let \(f:A\to B\) be a ring homomorphism, and let \(N\) be a \(B\)-module. Regarding \(N\) as
    an \(A\)-module by restriction of scalars, form the \(B\)-module \(N_B=B\otimes_AN\). Show that the
    homomorphism \(g:N\to N_B\) with \(y\mapsto 1\otimes y\) is injective and that \(g(N)\) is a direct summand of \(N_B\)
    #+END_exercise

    #+BEGIN_proof
    Suppose \(1\otimes y=1\otimes y'\) where \(y,y'\in N\). Then \(1\otimes(y-y')=0\) and so \(y-y'=0\).

    Define \(p:N_B\to N\) by \(p(b\otimes y)=by\) and show that \(N_B=\im(g)\oplus\ker(p)\)
    #+END_proof
* TODO Problems
