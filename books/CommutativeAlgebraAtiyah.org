...
** Exercise
    ...
    3. Let \(\fq\) be a prime ideal containing \(\fb\)and define \(\fp=\phi^{-1}(\fq)\). Then \(\fb^c\) is
       contained in \(\fp\). Thus \(\phi^*(V(\fb))\subseteq V(\fb^c)\).

       W.L.O.G., \(\fb=\sqrt{\fb}\). Let \(\fp\in V(\fb^c)\) and suppose \(f\notin\fp\). By the transitivity of
       containment, \(\phi(f)\notin\fb=\sqrt{\fb}\). By Proposition ref:1.14, there  exists \(\fq\in V(\fb)\)
       s.t. \(\phi(f)\in\fq\). Therefore
       \begin{equation*}
       \phi^*(\fq)\in\phi^*(Y_{\phi(f)}\cap V(\fb))\subseteq X_f\cap\phi^*(V(\fb))
       \end{equation*}
       Hence \(V(\fb^c)\subseteq\bbar{\phi^*(V(\fb))}\)
