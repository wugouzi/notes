...
** 可定义性与哥德尔运算
    ...
    - 在\(\ZF\)下，该“2阶”表达式可形式化为一个一阶表达式
      ...
      \(Form(x)\)使得\(\forall n\in\N\), \(Form(n)\Leftrightarrow\) \(n\)是某公式\(\varphi\)的编码，于是\(\exists\varphi\Leftrightarrow\exists xForm(x)\)
    ...
    #+BEGIN_proof
    \(\subseteq\): 设\(X\in\Def(M)\)形如\(\{x\in M\mid\psi^M(x,y_1,\dots,y_m)\}\)，\psi公式，\(\bary\in M\)
    ...
    由于\(M\)是传递模型，受囿量词关于\(M\)的相对化不改变，故\(\psi^M=\varphi\)

    即\(X=\{u\in M\mid\psi^M(u,M,\bary)\}\in\Def(M)\)
    #+END_proof

    * \(u\in G(X_1,\dots,X_n)\)是\(\Delta_0\)-公式
    * 若\(\psi\)是\(\Delta_0\)公式，则\(\forall u\in G(X_1,\dots,X_n)\varphi\)也是\(\Delta_0\)公式
...
** 哥德尔的\(\bL\)
    ...
    * 并集公理
      ...
      \(y=\bigcup x\)\(\Delta_0\)\(\psi(x,y)\)定义，相对化为\(\forall x\in\bL\exists y\in\bL\psi(x,y)\)，只需验证\(x\in\bL\Rightarrow\bigcup x\in\bL\)
...
** 可构成公理和相对一致性
    ...
    * 相对化为\(\forall x\in\bL\exists\alpha\in\On^{\bL}(x\in L_\alpha)^{\bL}\)等价于\(\forall x\in\bL\exists\alpha\in\On(x\in L_\alpha)^{\bL}\)
    ...
    括号里都是绝对的，若\(\On,\bL\subseteq\M\)，则\(\forall x(\bL(x)\leftrightarrow\bL^{\bM}(x))\)
    ...
    由于\(\alpha\mapsto L_\alpha\)是绝对的，故\(\alpha\in\bM\Rightarrow L_\alpha\in\bM\Rightarrow L_\alpha\subseteq\bM\)，因此\(\bL\Rightarrow\bM\)
    #+END_proof
    ...
    \end{equation*}
    同时\(\Delta^M\vdash\forall\alpha\in\On^M\exists\beta\in\On^M(\beta>\alpha)\)，于是\(\alpha^M\)中没有最大元，\(\alpha^M\)是极限序数，即
    ...
    * 令\(<_{\bL}=\bigcup_{\alpha\in\On}<_\alpha\)则\(<_{\bL}\)是良序
    ...
    则\(\ZFC+(\bV=\bL)\vdash\varphi\) （练习，反映定理）
...
** 力迫法的基本思想
    ...
    为了破坏\(\CH\)，可以在\(M\)中添加\(\calp(\omega)\)的子集得到\(M\)的扩张\(N\)使得\(N\vDash\calp(\omega_1)=\omega_2\)
...
** 脱殊扩张
    设\(M\vDash\ZFC\)是可数传递模型，令\(\P\in M\)是一个偏序集，\(\P\)的脱殊滤\(G\)一般不属于\(M\)，讲\(G\)添
    加到\(M\)得到\(M\)的扩张\(M[G]\)，\(M[G]\)的性质由\(\P\)绝顶

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    设\((\P,\le,\bbone)\)是偏序集，其中\(\le\)是其上的偏序，而\(\bbone\)是\(\P\)最大元，
    1. \(\bP\)称为 *力迫偏序* 或 *力迫* ，\(\P\)的元素称为 *力迫条件* ，\(\forall p,q\in\P\)，\(p\le q\)称为力迫条
       件\(p\)强于\(q\)
    ...
    若\(G\)是\(M\)上的脱殊滤，\(G\notin M\)，则\(G\cap D_x\)，\(G\cap E_y\)均非空，故\(f_G=\bigcup G\)是\(I\)到\(G\)的
    满射
    ...
    * 如\(f_G=\bigcup G\in M[G]\)，\(M\)可以看到\(f_G\)的“任意多”的部分信息，但\(f_G\notin M\)
    ...
    #+BEGIN_proof
    1. 以证明，若\(x\in N^{\P}\)且\(G\in N\)，则\(x_G\in N\)，由于\(M^{\P}\subseteq N^{\P}\)，故\(M[G]\subseteq N\)
...
** 力迫
    ...
    2. \(\Rightarrow\)：由1
       ...
       *断言* ：\(D_p\)在\(p\)下稠密
       ...
       若\(q\Vdash^*\sigma\in\tau_1\)，由稠密性，取\(r\le q\)使得\(r\Vdash^*\tau_1=\tau_2\)
    ...
    殊滤，则
    1. 若\(p\in G\)且\((p\Vdash^*\theta)^M\)，则\(M[G]\Vdash\theta(\sigma_G,\tau_G)\)
