...
** 脱殊扩张
    ...
    1. \(\bP\)称为 *力迫偏序* 或 *力迫* ，\(\P\)的元素称为 *力迫条件* ，\(\forall p,q\in\P\)，\(p\le q\)称为力迫条
       件\(p\)强于\(q\)
    2. \(D\subseteq\P\)是稠密的指\(\forall p\in\P\exists q\in D(q\le p)\)
    ...
    对任意力迫\(\P\)，对任意\(x\)递归定义
    \begin{equation*}
    \smile{x}=\{(\smile{x},\bbone)\mid y\in x\}
    \end{equation*}
    #+END_definition

    直观上\(\bV^{\P}\)比\(\bV\)丰富，\(x\mapsto\smile{x}\)是\(\bV\)到\(\bV^{\P}\)的嵌入，同时\(\bV^{\P}\subseteq\bV\)
    ...
    \(\forall x(\smile{x})_G=x\) (练习，用rank归纳)
    ...
    \end{equation*}
    只要证明\(\forall y\in M\exists x\in M\theta(x,y)\)，容易验证\(y\in M\Rightarrow\smile{y}\in M\)（替换公理），而\(y=(\smile{y})_G\)，因
    此\(\theta(\smile{y},y)\)
    #+END_proof
