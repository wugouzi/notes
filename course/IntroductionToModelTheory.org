...
** Definable closure
    ...
    *\(A\)-definable* if \(\Gamma(f)=\{(\bara,b):b=f(\bara)\}\) is definable
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_examplle
    In a field, \(a/b\in\dcl(\{a,b\})\) because \(\{a/b\}\) is \(\varphi(\M)\), \(\varphi(x):=bx=a\)
    #+END_examplle

    _Note_: if \(\barb\in\M^n\), \(\barb\in\dcl(A)^n\Leftrightarrow\{\barb\}\subseteq\M^n\) is \(A\)-definable \(\Leftrightarrow\)
    ...
    #+BEGIN_proof
    3. take \(b\in\dcl(\dcl(A))\), \(\{b\}\) is \(\dcl(A)\)-definable, \(\{b\}\) is \(A\)-definable, \(b\in\dcl(A)\)
    4. [@4] If \(D\) is \(\dcl(A)\)-definable. If \(\sigma\in\Aut(\M/A)\), if \(b\in\dcl(A)\), \(\sigma(b)=b\) by
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(\bara\), \(\barb\) are *interdefinable* if \(\dcl(\bara)=\dcl(\barb)\) iff \(\bara\in\dcl(\barb)\)
    ...
    #+BEGIN_proof
    By Proposition ref:4.7.10, \(\bara\in\dcl(\barb)\Leftrightarrow\Aut(\M/\barb)\subseteq\Aut(\M/\bara)\)
    #+END_proof
...
** Acl
    ...
    1. \(\barb\in\acl(A)\), there is \(D\) finite, \(A\)-definable, \(\barb\in D\). \(\sigma(\barb)\in\sigma(D)=D\)
       for \(\sigma\in\Aut(\M/A)\), so \(S\subseteq D\), \(S\) is finite. Also \(S\) is \(A\)-invariant
...
** Imaginaries
    ...
    #+BEGIN_fact
    label:4.7.22
    1. If \(X\subseteq M^n\), \(X\) is 0-definable in \(M\) \(\Leftrightarrow\) \(X\) is 0-definable in \(M^{\eq}\)
    ...
    4. If \(X\) is definable in \(M^{\eq}\), it's \(M\)-definable in \(M^{\eq}\)

       If \(X\subseteq M^n\), \(X\) is definable in \(M^{\eq}\) \(\Leftrightarrow\) \(X\) is definable in \(M\)
    ...
    8. If \(D/E\) is 0-interpretable in \(M\), then \(D/E\) is 0-definition in \(M^{\eq}\)
...
** Elimination of imaginaries
    ...
    If \(e\in D/E\), there is \(Y\) definable, bijection \(f:D/E\to Y\), \(e\) interpretable
    with \(f(e)\in\M^n\)
...
** Codes
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(e\) is a *code* for \(D\) ( \(e\) *codes* \(D\)) if \(\{\sigma\in\Aut(\M):\sigma(D)=D\}=\Aut(\M/e)\)
    #+END_definition
