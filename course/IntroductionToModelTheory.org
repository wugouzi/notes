...
** Density
    Recall \(p\in S_n(A)\) is *isolated* if \(\{p\}\) is clopen, \(\{p\}=[\varphi]\), \(\varphi\in L(A)\), \(a\vDash p\Leftrightarrow\vDash\varphi(a)\)
...
** Vaught pairs
    ...
    1. Let \(R(-)\) be the Cantor-Bendixson rank on \(S_1(M)\). \(R(D)>0\Leftrightarrow\abs{D}=\infty\)
       ...
       Let \(E_\alpha=\{p\in S_1(M):R(p)\ge\alpha\}\), \(R(S_1(M))=R(\M)>0\)

       If \(E_1=E_2=E_1'\), then \(E_1\) is perfect nonempty, \(S_1(M)\) is not empty, \(T\) is not t.t..
       So \(E_1\supsetneq E_2\), take \(p\in E_1\setminus E_2\), \(R(p)=1\), \(p\notin E_2=E_1'\) means \(p\) is isolated
       in \(E_1\)
    #+END_proof

    #+ATTR_LATEX: :options [continued]
    #+BEGIN_proof
    By Theorem ref:5.12.1,12, there is a prime model \(M_0\), \(\abs{M_0}=\aleph_0\).
    #+END_proof
...
** Orthogonality
    ...
    * if \(a\in M\), then \(p(y)\vdash\neg\psi(a,y,\barc)\)
    ...
    and \((M,dp)\preceq(M',dp')\) (\(p'\) has same definitions as \(p\))
    #+END_proof
    ...
    1. \(\forall M'\succeq M\) there is \(N'\succ M'\) with \(\varphi(N')=\varphi(M')\)
    ...
    1. Take \(b\in N\setminus M\), \(p=\tp(b/M)\), \(p\bot\varphi\), \(p'\in S_1(M')\) be the heir. By Lemma
       ...
       \(p\) not constant \(\Rightarrow\) \(p'\) not constant

    2. Take \(\abs{M_\alpha}\le\abs{\alpha}+\abs{L}+\abs{M}\)
    #+END_proof
