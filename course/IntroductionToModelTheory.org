...
** Cantor-Bendixson rank
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(R(p)=\max\{\alpha\mid\alpha\in E_\alpha\}\) (possibly \(\infty\)) *Cantor-Bendixson rank*
    ...
    1. If \(C_1\subseteq C_2\) then \(R(C_1)\)
    ...
    4. [@4] \(R(\varphi\cup\psi)=\max(R(\varphi),R(\psi))\)
    ...
    1. If \(C=\bigcap_{i\in I}U_i\) filtered intersection (\(\forall i,j\in I\exists k U_k\subseteq U_i\cap U_j\))
    #+END_proposition
...
** When is \texorpdfstring{\(S_n(A)\)}{S_n(A)} scattered
    ...
    #+BEGIN_proof
    1. If \(R(S_n(A))=\infty\) ref:5.5.5.2
    #+END_proof
