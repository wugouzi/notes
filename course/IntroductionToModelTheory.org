...
#+LATEX_HEADER: \DeclareMathOperator{\tbd}{bd}
#+LATEX_HEADER: \DeclareMathOperator{\RF}{RF}
...
** Heirs and strong heirs
    ...
    The first axiom schema expresses finite satisfiablity of \(p\), and the second expresses
    completeness of \(p\)
    #+END_remark
...
* Coheirs and invariant types
** Conclusion of the last lecture
    Let \lambda be an infinite cardinal. \(T\) is \lambda-stable if the following equivalent conditions hold
    1. \(\forall A\subseteq\M\), \(\abs{A}\le\lambda\Rightarrow\abs{S_1(A)}\le\lambda\)
    2. \(\forall A\subseteq\M\), \(\abs{A}\le\lambda\Rightarrow\abs{S_n(A)}\le\lambda\)


    If \(\lambda\ge\abs{L}\), then 1 and 2 are equivalent to the following
    3. [@3] \(\forall M\preceq\M\), \(\abs{M}\le\lambda\Rightarrow\abs{S_1(M)}\le\lambda\)
    4. \(\forall M\preceq\M\), \(\abs{M}\le\lambda\Rightarrow\abs{S_n(M)}\le\lambda\)


    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    Suppose for every model \(M\) and every \(p\in S_1(M)\), \(p\) is definable. Then \(M\)  is
    \lambda-stable for some \lambda
    #+END_lemma

    #+BEGIN_proof
    Take \(\lambda=2^{\abs{L}}\). It suffices to show that if \(M\preceq\M\) and \(\abs{M}\le\lambda\),
    then \(\abs{S_1(M)}\le\lambda\). Every type in \(S_1(M)\) is definable. A definable type is determined by
    the map \(\varphi\mapsto d\varphi\). Therefore are \(\abs{L}\)-many possibilities for \varphi, and \(\abs{L(M)}\)-many
    possibilities for \(d\varphi\). So the number of types is at most \(\lambda^{\abs{L}}=\lambda\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    TFAE for a theory with monster model \(\M\)
    1. All types over models are definable
    2. All 1-types over models are definable
    3. No formula \(\varphi(\barx;\bary)\) has the dichotomy property
    4. No formula \(\varphi(\barx;\bary)\) has the dichotomy property
    5. \(T\) is \lambda-stable for at least one \lambda
    #+END_theorem
...
** Coheirs
    ...
    *Claim* \(\Sigma(\barx)\) fsat in \(M\) \(\Rightarrow\) \(\Sigma\cup\Psi\) is fsat \(\Rightarrow\) \(q\in S_n(N)\), \(q\supseteq\Sigma\cup\Psi\)
    ...
    If \(q\) isn't fsat. in \(M\) then \(\varphi(\barx)\in q(\barx)\), \(\varphi(\barx)\) not sat. in \(M\), then \(\neg\varphi\in\Psi\)
    #+END_proof
...
** Invariant types
    ...
    3. If \(p\) is definable then \(p\) is \(A\)-definable for some small \(A\)
    ...
    #+END_proof
** Coheirs and invariant types in stable theories
    In stable theories:
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    ...
    #+BEGIN_corollary
    label:3.10.21
    In a stable theory, "coheir" is transitive, that is, a coheir of a coheir is a coheir
    #+END_corollary
...
** Invariant types clarification
*** Morley products via fixed monster model
    #+BEGIN_remark
    ...
    #+END_warning
*** Morley products by varing the monster
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    label:itc10
    Suppose \(M\) is a model and \(A\subseteq M\). Suppose every \(n\)-type over \(A\) is realized in \(M\)
    for every \(n<\omega\). We say that \(p\in S_n(M)\) is *\(A\)-invariant* if the following property holds:
    * If \(\barb,\barc\in M\), \(\barb\equiv_A\barc\) and \(\varphi(\barx,\bary)\) is a formula, then
      \begin{equation*}
    ...
    1. If \(\alpha\in\R\) is irrational, then the constant type \(\tp(\alpha/\R)\) has no \(\Q\)-invariant
       extension to \(N\) (The only extension is \(\tp(\alpha/N)\), which is not \(\Q\)-invariant)
    2. Suppose \(\beta\in N\) is greater than every element of \(\R\). Then \(\tp(\beta/\R)\) is the type
    ...
    *Claim*: \(\tp(\bara,\barb/M)=\tp(\bara',\barb'/M)\)
...
* Morley sequences and the order property
** Morley sequence
    ...
    #+BEGIN_proof
    If \(i_1<\dots<i_n\), then \(\barb_{i_j}\vDash p\uhr A\barb_{i_1}\dots\barb_{i_{j-1}}\) and so
    \(\tp(\barb_{i_1},\dots,\barb_{i_n}/A)=\underbrace{p\otimes\dots\otimes p}_{n\text{ times}}\uhr A\), doesn't depend
    on the choice of \(i\)
    #+END_proof
...
** Order Property
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    Let \(\varphi(\barx,\bary)\) be a formula. Then \(\varphi(\barx,\bary)\) has the *order property* if there
    are \((\bara_i:i\in\Z)\) and \((\barb_i:i\in\Z)\) s.t.
    \begin{equation*}
    \M\vDash\varphi(\bara_i,\barb_j)\Leftrightarrow i<j
    \end{equation*}
    #+END_definition
    ...
    #+END_remark
** Instability from the order property
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    ...
    *Claim* \(I\setminus S\to S_n(C)\) , \(i\mapsto\tp(\bara_i/C)\) is an injection
    ...
    #+END_proof
** The order property from instability
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(\varphi(\barx;\bary)\) doesn't have O.P., then there is \(n_\varphi\) s.t. there don't
    exist \((\bara_i:i<n_\varphi)\) and \((\barb_i:i<n_\varphi)\) s.t.
    \begin{equation*}
    \M\vDash\varphi(\bara_i;\barb_j)\Leftrightarrow i<j
    \end{equation*}
    #+END_lemma
    ...
    \begin{align*}
    &\M\vDash\varphi(\bara,\barb_i)\text{ for }0\le i<n_\varphi\\
    &\M\vDash\neg\varphi(\bara,\barb_i)\text{ for }n_\varphi\le i<2n_\varphi
    \end{align*}
    #+END_lemma
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    Suppose \(\varphi(x_1,\dots,x_n;\bary)\) doesn't have the order property. If \(M\) is a small model
    and \(p\in S_n(M)\) then the relation \(d_p\varphi(\bary)\) is definable
    #+END_proposition

    #+BEGIN_proof
    Take \(q\in S_n(\M)\) a global coheir of \(p\). Then \(q\) is \(M\) is \(M\)-invariant
    (ref:3.10.17). Let \((\bara_i:i<\omega)\) be a Morley sequence of \(q\) over \(M\). By Lemma
    ref:3.17.11 \(d_q\varphi(\bary)\) is definable from the Morley sequence by majority voting
    \begin{equation*}
    \varphi(\barx;\barb)\in q(\barx)\Leftrightarrow\M\vDash\bigvee_S\bigwedge_{i\in S}\varphi(\bara_i;\barb)
    \end{equation*}
    where \(S\) ranges over \(\{S\subseteq 2N:\abs{S}>N\}\). Now \(d_q\varphi\) is definable and \(q\)
    is \(M\)-invariant and hence \(M\)-invariant. Then \(d_p\varphi\) is \(M\)-definable: it's the
    restriction of \(d_q\varphi\) to \(M\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    Fix \(n\). Suppose no formula \(\varphi(x_1,\dots,x_n;\bary)\) has the OP. Then for any \(M\vDash T\)
    and \(p\in S_n(M)\), \(p\) is definable
    #+END_theorem
    ...
    1. All types over models are definable
    2. All 1-types over models are definable
    3. No formula \(\varphi(\barx;\bary)\) has the OP
    4. No formula \(\varphi(x;\bary)\) has the OP
    5. \(T\) is \lambda-stable for at least one \lambda
    #+END_corollary
...
** Commuting types
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_examplle
    Suppose \(T\) is strongly minimal, and \(p,q\in S_1(\M)\)  are both the transcendental 1-type. By
    Theorem ref:3.17.16, \(p\otimes q=q\otimes p\). This means that for \(a,b\in\M\) and \(C\subseteq\M\)
    \begin{equation*}
    a\notin\acl(C)\wedge b\notin\acl(Ca)\Leftrightarrow b\notin\acl(C)\wedge a\notin\acl(Cb)
    \end{equation*}
    This implies that \(\acl(-)\) satisfies the "Steinitz exchange property"
    \begin{equation*}
    a\in\acl(Cb)\setminus\acl(C)\Rightarrow b\in\acl(Ca)
    \end{equation*}
    #+END_examplle
** Indiscernible sequences exist
    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    label:3.17.17
    Suppose \(\M\) is infinite, then there is a non-constant indiscernible sequence \((b_i:i<\omega)\)
    #+END_theorem

    #+BEGIN_proof
    Because \(\M\) is infinite, it is not small, by saturation. Take a small model \(M\) and
    take \(a\in\M\setminus M\). Let \(p=\tp(a/M)\in S_1(M)\). Let \(q\in S_1(\M)\) be a coheir of \(p\). Then \(q\)
    is \(M\)-invariant. Let \((b_i:i<\omega)\) be a Morley sequence of \(q\) over \(M\). Then \((b_i:i<\omega)\)
    is \(M\)-indiscernible, hence indiscernible, by Theorem ref:3.17.4. It remains to show that the
    sequence is non-constant

    By indiscernibility it suffices to show that \(b_2\neq b_1\). Suppose \(b_2=b_1\).
    Since \(b_1\vDash q\uhr M\) and \(b_2\vDash q\uhr Mb_1\), \(x=b_1\in\tp(b_2/Mb_1)\), so \(x=b_1\in q\). As \(q\) is
    finitely satisfiable in \(M\), there is some \(a_0\in M\) satisfying \(a_0=b_1\).
    Now \(x=a_0\in\tp(b_1/M)=p\), and so the formula \(a=a_0\in M\), a contradiction
    #+END_proof
...
** Extracting indiscernibles
    ...
    #+ATTR_LATEX: :options [Extracting indiscernible sequences]
    #+BEGIN_theorem
    ...
    #+BEGIN_proof
    Apply Lemma ref:3.24.9 with \(\Delta=\{\text{all the }L(B)\text{-formulas}\}\)
    #+END_proof
    ...
    #+ATTR_LATEX: :options [Stretching indiscernible sequences]
    #+BEGIN_theorem
...
** The order property and indiscernible sequences
...
** Total indiscernibility
    ...
    #+BEGIN_proof
    \(3\to 2\): trivial, \(2\to 1\) ref:3.24.18

    \(1\to 3\): Suppose \(T\) stable but \((\bara_i:i\in I)\) \(B\)-indiscernible not
    totally \(B\)-indiscernible

    We may assume \(I=\Z\), let \(C=\{\bara_i:i\in\Z\setminus\{0,1\}\}\). By Lemma
    ref:2.24.23, \(\bara_0\bara_1\not\equiv_{BC}\bara_1\bara_0\). Take
    an \(L(BC)\)-formula \(\varphi(\barx,\bary)\) s.t.
    \begin{equation*}
    \M\vDash\varphi(\bara_0,\bara_1)\wedge\neg\varphi(\bara_1,\bara_0)
    \end{equation*}
    By Theorem ref:3.24.14 we can extend \(\{\bara_i:i\in\Z\}\) to a \(B\)-indiscernible
    sequence \(\{\bara_i:i\in\R\}\). Now \(\{\bara_i:i\in\R,-1<i<2\}\) is \(BC\)-indiscernible. Therefore
    \begin{gather*}
    -1<i<j<2\Rightarrow\bara_i\barb_j\equiv_{BC}\bara_0\bara_1\\
    -1<i<j<2\Rightarrow\M\vDash\varphi(\bara_i,\bara_j)\wedge\neg\varphi(\bara_j,\bara_i)
    \end{gather*}
    Therefore \varphi has the order property, witnessed by \(\bara_i,\bara_j\) for \(i,j\) in the
    interval \([-0.9,1.9]\). By Proposition ref:3.24.24, \(T\) is unstable
    #+END_proof
...
** Ehrenfeucht-Mostowski models
    ...
    #+BEGIN_theorem
    label:3.24.31
    Let \(T\) be a complete theory and \kappa be an infinite cardinal. There is a model \(M\vDash T\) of size
    \kappa with the following property: for any countable \(A\subseteq M\) only countably many types over \(A\)
    are realized in \(M\)
    \begin{equation*}
    \abs{A}\le\aleph_0\Rightarrow\abs{\{\tp(c/A):c\in M\}}\le\aleph_0\tag{\star}
    \end{equation*}
    #+END_theorem

    #+BEGIN_proof
    We can replace \(T\) with an expansion \(T'\) that has Skolem functions. Property (\(\star\)) is
    preserved in reducts, so we can go back from \(T'\) to \(T\). Assume \(T\) has Skolem functions

    Take a non-constant indiscernible sequence \((b_i:i<\kappa)\) in the monster model \(\M\) and
    let \(M=\la\{b_i:i<\kappa\}\ra\) . Then \(M\preceq\M\) so \(M\vDash T\). Also \(\abs{M}=\kappa\)

    Fix countable \(A\subseteq\aleph_0\), for each \(c\in A\) there is finite \(I_c\subseteq\kappa\) s.t. \(c\in\la\{b_i:i\in I_c\}\ra\).
    Let \(I_0=\bigcup_{c\in A}I_c\) and let \(A_0=\{b_i:i\in I_0\}\). Then \(I_0\) and \(A_0\) are countable
    and \(A\subseteq\la A_0\ra\)

    Because \(A\subseteq\la A_0\ra\) every \(L(A)\)-formula is equivalent to an \(L(A_0)\)-formula.
    Therefore \(\tp(c/A_0)\) determines \(\tp(c/A)\). It suffices to show that countably many types
    over \(A_0\) are realized in \(M\). Replacing \(A\) with \(A_0\), we may
    assume \(A=A_0=\{b_i\in I_0\}\).

    Suppose \(c\in M\). Then \(c=t(b_{i_1},\dots,b_{i_n})\) for some term \(t\) and some \(i_1,\dots,i_n\in\kappa\). By
    indiscernibility, \(\tp(c/A)\) is determined by the following data:
    * the term \(t(x_1,\dots,x_n)\)
    * \(\tp(b_{i_1},\dots,b_{i_n}/A)\) which is in turn determined by the following data
      * the relative order of \(i_1,\dots,i_n\)
      * the cut in \(I_0\) defined by each \(i_j\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    label:3.24.32
    Let \(T\) be a complete theory (in a countable language). If \(T\) is \kappa-categorical for some
    uncountable \kappa, then \(T\) is \(\aleph_0\)-stable
    #+END_theorem

    #+BEGIN_proof
    Suppose not. Take a monster model \(\M\vDash T\) and a countable \(A\subseteq\M\) with \(\abs{S_1(A)}>\aleph_0\).
    Take \((b_i:i<\aleph_1)\) realizing distinct types over \(A\). Let \(N\) be a model of size \(\kappa\ge\aleph_1\)
    containing \(A\cup\{b_i:i<\aleph_1\}\). Then \(N\) has a countable subset \(A\) over which uncountablly many
    types are realized. Therefore \(N\) is not isomorphic to the model of size \kappa from ref:3.24.31
    #+END_proof
...
** Definable closure
    ...
    "infinite boolean combination" of \(\calf\), i.e., if \(\bara,\barb\in\M^n\)
...
** EI in PA and ACF
    ...
    *Claim*: If \(\sigma\in\Aut(\M)\), \(\sigma(S)=S\Leftrightarrow\sigma\in\Aut(\M/A)\)
    ...
    #+END_fact
** Stability and @@latex:\texorpdfstring{${\M}^{\eq}$}{Meq}@@
    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
...
** Theorem of the bound
    ...
    2. If \([q]=\beta\), then \(q\) is \(A\)-definable by Lemma ref:4.21.4.1.
...
** Stationary types
    ...
    2. If \(A=\acl^{\eq}(A)\) and \(p\in S_n(A)\), then \(p\) is stationary. (It has
       an \(A\)-definable extension by Corollary ref:4.21.4.2) In the language of Remark
       ref:4.21.4.6, strong types are stationary
    3. If \(p\in S_n(A)\) is stationary and \(q\in S_n(B)\) is a non-forking extension, then \(q\) is
    ...
    4. In a strongly minimal theory, the transcendental type over \(A\) is stationary (The global
       transcendental type is \(\emptyset\)-definable and extends it)
    5. In \(\C\vDash\ACF\), \(\tp(i/\Q)=\tp(-i/\Q)\) is /not/ stationary, because it has two extensions
...
** Stability spectra
    ...
    *Claim* \(M\preceq\M\)
    ...
    #+BEGIN_remark
    Theorem ref:4.28.8.6 shows that \(\kappa(T)\) and \(\lambda_0(T)\) determine the *stability spectrum*
...
** Forking calculus
    ...
    4. (left Transitivity) \(A\ind_CB\) \(A'\ind_{CA}B\) \(\Rightarrow\) \(AA'\ind_CB\)
    ...
    10. if \(C_1\subseteq C_2\subseteq C_3\), \(A\ind_{C_1}C_3\Leftrightarrow(A\ind_{C_1}C_2\text{ and }A\ind_{C_2}C_3)\)
    #+END_proposition
...
** Examples
    ...
    #+BEGIN_proof
    *Claim*:  \(\kappa_n(T)\) is smallest \(\kappa\ge\aleph_0\) s.t. \(\not\exists\bara\in\M^n\),
...
** Independence sequence
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \((A_i:i\in I)\) is *independent* over \(B\) if \(A_i\ind_BA_{\neq i}\) where \(A_{\neq i}=\{A_j:j\neq i\}\)
    #+END_definition
    ...
    #+END_examplle
** Bases in strongly minimal theories
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    The *transcendental type* over \(A\) is \(\tp(b/A)\) for any \(b\notin\acl(A)\)
    #+END_definition
    ...
    #+BEGIN_remark
    If \(p\in S_1(\M)\) _transcendental_, if \(b_1,\dots,b_n\in\M\setminus\acl(\emptyset)\), then \((b_1,\dots,b_n)\) is independent
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    If \(M\preceq\M\), a *basis* of \(M\) is a maximal independent set \(I\subseteq M\setminus\acl(\emptyset)\)
    #+END_definition
...
* Cantor-Bendixson rank and Morley rank
** More topology on type spaces
    Fix \(A\subseteq\M\), \(n\),
    ...
    1. If \(\calf\) is a chain
    2. Or if \(\calf\) is filtered (\(\forall X,Y\in\calf\exists Z\in\calf,Z\subseteq X\cap Y\))


    then \(\calf\) has FIP \(\Rightarrow\) \(\bigcap\calf\neq\emptyset\)
    #+END_corollary

    #+ATTR_LATEX: :options [Total separation]
    #+BEGIN_proposition
    label:5.5.9
    ...
    1. \(\exists\) clopen \(U\), \(p\in U\), \(U\cap C=\{p\}\)
    ...
    1. \(p\) is an *isolated point* of \(C\) if  case 1 holds
    2. \(p\) is an *accumulation point* of \(C\) if case 2 holds
    3. The *derived set* \(C'\) of \(C=\{p\in C:p\text{ is an accumulation point}\}\)
    #+END_definition
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(K\subseteq S_n(A)\) is *perfect* if \(K\) is closed and \(K'=K\)

    \(S_n(A)\) is *scattered* if there is no \(K\subseteq S_n(A)\) s.t. \(K\neq\emptyset\), \(K'=K\)
    #+END_definition
** Cantor-Bendixson rank
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(R(p)=\max\{\alpha\mid p\in E_\alpha\}\) (possibly \(\infty\)) *Cantor-Bendixson rank*
    ...
    1. If \(C_1\subseteq C_2\) then \(R(C_1)\le R(C_2)\)
    2. \(R(C_1\cup C_2)=\max(R(C_1),R(C_2))\)
    If \Sigma,\Psi are partial types over \(A\)
    3. [@3] If \(\Sigma\vdash\Psi\), then \(R(\Sigma)\le R(\Psi)\)
    If \(\varphi,\psi\in L(A)\)
    4. [@4] \(R(\varphi\vee\psi)=\max(R(\varphi),R(\psi))\)
    ...
    #+BEGIN_proposition
    label:5.5.4.7
    1. If \(C\subseteq S_n(A)\) closed, then
       \begin{equation*}
       R(C)=\min\{R(U):U\supseteq C,U\text{ clopen}\}
       \end{equation*}
    2. If \(\Sigma(\barx)\subseteq L(A)\), then \(R(\Sigma)=\min\{R(\varphi):\Sigma\vdash\varphi\}\)
    3. If \(C=\bigcap_{i\in I}U_i\) a filtered intersection, then \(R(C)=\min_{i\in I}R(U_i)\)
    4. \(R(\Sigma)=\min_{\Sigma_0\subseteq_f\Sigma}R(\Sigma_0)\)
    #+END_proposition
    ...
    1. \(R(U)\ge\alpha+1\)
    2. there is clopen \(U_1,U_2,\dots\subseteq U\), \(U_i\cap U_j=\emptyset\), \(i\neq j\),  and \(R(U_i)\ge\alpha\)
    3. there is clopen \(U_1,U_2,\dots\subseteq U\) s.t. \(R(U_i\triangle U_j)\ge\alpha\) (idea: modulo the ideal \(\{U:R(U)<\alpha\}\),
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    A family \(\calf\) of definable sets *\(k\)-inconsistent* (\(k\in\omega\)) if \(\forall\)
...
** When is \texorpdfstring{\(S_n(A)\)}{S_n(A)} scattered
    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    ...
    #+BEGIN_lemma
    label:5.5.5.3
    1. \(R(S_n(A))=\infty\) \(\Leftrightarrow\) there is \((U_\sigma:\sigma\in 2^{<\omega})\)
       s.t. \(U_\sigma=U_{\sigma 0}\sqcup U_{\sigma 1}\) \(\forall\sigma\), \(U_\sigma\neq\emptyset\), \(U_\sigma\) clopen
    2. If \(R(S_n(A))=\infty\), then \(\abs{S_n(A)}\ge 2^{\aleph_0}\)
    3. If \(R(S_n(A))<\infty\), then \(\abs{S_n(A)}\le\abs{L(A)}=\abs{L}+\abs{A}\)
    #+END_lemma

    #+BEGIN_proof
    1. If \(R(S_n(A))=\infty\), take \(U=S_n(A)\). By ref:5.5.5.2, \(U=U_0\sqcup U_1\) where \(U_0\), \(U_1\)
    ...
    2. Take a tree \((U_\sigma:\sigma\in 2^{<\omega})\) as in 1. For \(\tau\in 2^\omega\) let \(C_\tau=\bigcap_{n=0}^\infty U_{\tau\uhr n}\).
       Then \(C_\tau\) is nonempty closed set. Take \(p_\tau\in C_\tau\). The \(p_\tau\) is distinct. So we get at
       least \(2^{\aleph_0}\)-many points
    3. Suppose \(R(S_n(A))<\infty\), for each \(p\in S_n(A)\) let \(\alpha_p=R(p)\). Then \(p\) is isolated
       in \(E_{\alpha_p}\), so there is a clopen set \(U_p\ni p\) with \(U_p\cap E_{\alpha_p}=\{p\}\). Then \(p\) is
       the unique type in \(U_p\) of maximal rank. Therefore \(p\mapsto U_p\) is injective.
       But \(\abs{\{U_p:p\in S_n(A)\}}\le\abs{L(A)}\) since each \(U_p\) is defined by an \(L(A)\)-formula
    #+END_proof
    ...
    #+BEGIN_proof
        \(T\) is \lambda-stable for \(\lambda=2^{\abs{L}}=2^{\aleph_0}\). So \(\lambda_0(T)\le 2^{\aleph_0}\).
        Suppose \(\aleph_0<\lambda_0<2^{\aleph_0}\), then \(T\) is not \(\aleph_0\)-stable. Take countable \(A\subseteq\M\)
        with \(\abs{S_1(A)}>\aleph_0\). By Lemma ref:5.5.5.3, either \(\abs{S_1(A)}\le\abs{L(A)}=\aleph_0\)
        or \(\abs{S_1(A)}\ge 2^{\aleph_0}\), then \(\abs{A}\le\aleph_0\le\lambda_0\) but \(\abs{S_1(A)}\ge 2^{\aleph_0}>\lambda_0\), so
        stability fails
    #+END_proof
...
** Morley rank
    ...
    If \(p\in S_n(\M)\),
    1. \(\RM(p)\ge 0\) always
    2. \(\RM(p)\ge\alpha+1\)\(\Leftrightarrow\) \(p\in(\{q\in S_n(\M):\RM(q)\ge\alpha\})'\)
    ...
    *Claim*: if \(\alpha\in\On\), \(D\) is \(M\)-definable, then
    \begin{equation*}
    ...
    #+BEGIN_examplle
    If \(D\) is definable
    1. \(\RM(D)\ge 0\Leftrightarrow D\neq\emptyset\)
    2. \(\RM(D)\ge 1\Leftrightarrow\) there are disjoint nonempty definable subsets \(D_1,D_2,\dots\subseteq D\)
       \(\Leftrightarrow\) \(\abs{D}=\infty\)
    3. \(\RM(D)\ge 2\Leftrightarrow\) there are disjoint infinite definable subsets \(D_1,D_2,\dots\subseteq D\)
    ...
    1. If \(f\) is surjective, then \(\RM(X)\ge\RM(Y)\)
    2. If \(f\) is bijective, then \(\RM(X)=\RM(Y)\)
    3. If \(f\) is injective, then \(\RM(X)\le\RM(Y)\)
    #+END_proposition

    #+BEGIN_proof
    1. *Claim*: if \(\alpha\in\On\), \(f:X\to Y\) definable surjection, \(\RM(Y)\ge\alpha\), then \(\RM(X)\ge\alpha\)
    ...
    2. \(f^{-1}\) and \(f\) are surjection
    3. if \(f:X\to Y\) injective, \(f:X\to f(X)\) is bijection
    #+END_proof
    ...
    label:5.7.6.9
    Fix \(B\subseteq\M\)
    1. If \(\bara\in\M^n\), then \(\RM(\bara/B)=\min\{\RM(X):X\subseteq\M^n,X\text{ is $B$-definable},\bara\in X\}\)
    2. If \(X\) is \(B\)-definable, then \(\RM(X)=\max_{\bara\in X}\RM(\bara/B)\)
    #+END_proposition

    #+BEGIN_proof
    1. Proposition ref:5.5.4.7

       \begin{align*}
       &\RM(\bara/B)=\RM(\tp(\bara/B))=\min_{\Sigma_0\subseteq_f\tp(\bara/B)}\RM(\Sigma_0)\\
       &=\min_{\varphi\in\tp(\bara/B)}\RM(\varphi)
       \end{align*}
    2. If \(\bara\in X\), \([\tp(\bara/B)]\subseteq[X]\subseteq S_n(M)\), if \(X=\varphi(\M)\), \(\varphi\in L(B)\), then
    ...
    1. \(\RM(\pi_1(D))=\RM(\bara/C)\)
    2. \(\RM(\pi_2(D))=\RM(\barb/C)\)
    3. \(D\to\pi_1(D)\) has finite fibers because \(\barb\in\acl(C\bara)\)


    Idea: By Proposition ref:5.7.6.9, there is \(C\)-definable \(D_1\) s.t. \(\RM(D_1)=\RM(\bara/C)\),
    replace \(D\) with \(D\cap\pi^{-1}(D_1)\), now \(\pi_1(D)\subseteq D_1\), so \(\RM(\pi_1(D))\le\RM(D_1)=\RM(\bara/C)\).
    But as \((\bara,\barb)\in D\), \(\bara\in\pi_1(D)\), therefore \(\RM(\pi_1(D))\ge\RM(\bara/C)\).

    As \(\barb\in\acl(C\bara)\), there
    is \(\varphi(\barx,\bary)\in L(C)\), \((\bara,\barb)\in\varphi(\M)\), \(\abs{\varphi(\bara,\M)}=k\). Shrine \(\varphi\)
    make \(\abs{\varphi(\bara',\M)}\le k\) for all \(\bara'\), take \(D\subseteq\varphi(\M)\), then \(D\to\pi_1(D)\) has finite fibers
    ...
    2. \(\RM(\M^n)=n\)
    3. If \(\bara\vDash p^{\otimes n}\uhr C\), then \(\RM(\bara/C)=n\)
    #+END_theorem
    ...
    1. If \(a_i\notin\acl(Ca_1\dots a_{i-1})\) for all \(i\),
       ...
       then \(\bara\in\acl(Ca_1,\dots,a_{i-1},a_{i+1},\dots,a_n)\). By Lemma ref:5.7.6.10 and induction
       \begin{equation*}
       \RM(\bara/C)\le\RM(a_1,\dots,a_{i-1},a_{i+1},\dots,a_n/C)\le\RM(\M^{n-1})\le n-1
       \end{equation*}
    2. take \(\aleph_0\)-saturated \(M\preceq\M\), so CB-rank=Morley rank. If \(q\in S_n(M)\)
    ...
    3. Proposition ref:5.7.6.9 gives \(n=\RM(\M^n)=\max_{\bara\in\M^n}\RM(\bara/C)\), and part 1 gives the answer
    #+END_proof
    ...
    1. \(\abs{B_2}<\infty\), let \(B_1=\{a_1,\dots,a_n\}\), \(B_2=\{b_1,\dots,b_m\}\). Let \(p\) be the global
       transcendental type. Recall that independent sets are Morley sequences of \(p\) over \(\emptyset\).
       By Theorem ref:5.7.6.11, \(\RM(\bara/\emptyset)=n\) and \(\RM(\barc/\emptyset)=m\). In
       particular, \(\RM(\bara/\emptyset)<\RM(\barc/\emptyset)\). But \(\barc\in\acl(\bara)\), so this contradicts
       Lemma ref:5.7.6.10
    2. \(\abs{B_2}=\kappa\ge\aleph_0\), suppose \(\abs{B_1}<\abs{B_2}\),
...
** Totally transcendental theory
    ...
    Fix \(n<\infty\), TFAE
    1. \(R(S_n(\M))<\infty\)
    2. \(R(S_n(A))<\infty\) for all \(A\subseteq\M\)
    3. \(R(S_n(A))<\infty\) for all \(A\subseteq\M\) with \(\abs{A}\le\aleph_0\)
    4. \(R(S_n^{L_0}(A))<\infty\) for all countable \(A\subseteq\M\) and countable \(L_0\subseteq L\)
    #+END_lemma
    ...
    1. \(\RM(\Sigma(\barx))<\infty\) for any \(\Sigma(x_1,\dots,x_n)\) partial type
    2. \(\RM(x=x)<\infty\)
    3. \(T\) is \omega-stable
    #+END_theorem
    ...
    \(3\to 1\): BY stability, \(S_n(A)\) is countable for countable \(A\). By Lemma
    ref:5.5.5.3, \(S_n(A)\) is scattered for countable \(A\). By Lemma ref:5.7.7.1, \(S_n(\M)\) is
    scattered, which implies \(R(C)<\infty\) for any closed set \(C\subseteq S_n(\M)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    For any \(T\), TFAE
    1. \(\RM(\Sigma(x_1,\dots,x_n))<\infty\)
    2. \(\RM(\M)=\RM(x=x)<\infty\)
    3. \(T\uhr L_0\) is \omega-stable  for any countable \(L_0\subseteq L\)
    #+END_theorem

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(T\) is *totally transcendental* (t.t.) if 1-3 hold
    #+END_definition
...
** Morley rank and forking
    Assume \(T\) is t.t.

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    Suppose \(p\in S_n(A)\)
    1. If \(q\in S_n(\M)\), \(q\supseteq p\), then \(\RM(q)\le\RM(p)\)
    2. there is \(r\in S_n(\M)\), \(r\supseteq p\) with \(\RM(r)=\RM(p)\)
    3. If \(q\in S_n(\M)\), \(q\supseteq p\) and \(\RM(q)=\RM(p)\), then \(q\sqsupseteq p\)
    4. If \(q\in S_n(\M)\), \(q\supseteq p\) and \(q\sqsupseteq p\), then \(\RM(q)=\RM(p)\)
    #+END_lemma

    #+BEGIN_proof
    1 - 2: by definition, \(\RM(p)=\max\{\RM(q):q\in S_n(\M),q\supseteq p\}\)

    3. [@3] By Proposition ref:5.5.4.8, there are finitely many such \(q\).
       Then \(\{\sigma(q):\sigma\in\Aut(\M/A)\}\) is finite, so \(q\) is almost \(A\)-definable and \(q\sqsupseteq p\) by
       Proposition ref:4.21.5.6
    4. Take \(r\) as in 2. By 3, \(r\sqsupseteq p\). Then \(r,q\) are non-forking extensions of \(p\). By
       Proposition ref:4.21.4.5, there is \(\sigma\in\Aut(\M/A)\) with \(\sigma(r)=p\). Then \(\RM(q)=\RM(r)=\RM(p)\)
    #+END_proof
    ...
    1. \(\RM(q)\le\RM(p)\)
    2. \(\RM(q)=\RM(p)\Leftrightarrow q\sqsupseteq p\)
    #+END_proposition

    #+BEGIN_proof
    1. Clear
    2. Take \(r\in S_n(\M)\) , \(r\sqsupseteq q\), \(\RM(r)=\RM(q)\) by Lemma ref:5.7.8.1,
       so \(q\sqsupseteq p\Leftrightarrow r\sqsupseteq p\)
    #+END_proof

    Note the parallel between \(\RM(p)\) and the bound \(\tbd(p)\)
    * if \(q\) is a forking extension of \(p\), then \(\RM(q)<\RM(p)\) and \(\tbd(q)<\tbd(p)\)
    * if \(q\) is a non-forking extension of \(p\), then \(\RM(q)=\RM(p)\) and \(\tbd(q)=\tbd(p)\)


    This gives another proof of superstability: if \(\beta_1>\beta_2>\dots\) is a descending chain in the
    fundamental order, we can get a forking chain
    \begin{gather*}
    p_1\subseteq p_2\subseteq p_3\subseteq\dots\\
    p_1\not\sqsubseteq p_2\not\sqsubseteq p_3\not\sqsubseteq\dots\\
    \tbd(p_i)=\beta_i
    \end{gather*}
    But there is no infinite descending chain of ordinals


** Density of isolated types

    \(p\in S_n(A)\) is *isolated* if the following equivalent conditions hold:
    * \(p\) is an isolated point in the closed set \(S_n(A)\)
    * there is a clopen set \(U\) s.t. \(U\cap S_n(A)=\{p\}\)
    * \(\{p\}\) is clopen
    * there is an \(L(A)\)-formula \(\varphi(\barx)\) generating \(p(\barx)\)
    * there is an \(L(A)\)-formula \(\varphi(\barx)\) s.t. for any \(\barb\in\M^n\)
      \begin{equation*}
      \tp(\barb/A)=p\Leftrightarrow\barb\vDash p\Leftrightarrow\M\vDash\varphi(\barb)
      \end{equation*}
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    ...
    #+BEGIN_proof
    \(U\) is not perfect, take \(p\in U\) isolated in \(U\), then \(p\) is an isolated point of \(U\).
    Then a clopen set \(V\) s.t. \(V\cap U=\{p\}\). Then the clopen set \(V\cap U\) isolated \(p\) in \(S_n(A)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    label:5.7.9.2
    If \(T\) is t.t.,\(A\) small, if \(D\subseteq\M^n\) , \(A\)-definable, \(D\neq\emptyset\), then there is \(\barb\in D\)
    s.t. \(\tp(\barb/A)\) is isolated in \(S_n(A)\)
    #+END_theorem

    #+BEGIN_proof
    \(T\) is t.t. \(\Rightarrow\) \(R(S_n(\M))<\infty\), \(R(S_n(A))<\infty\) by Lemma ref:5.7.7.1, \(S_n(A)\) scattered, there
    is \(p\in S_n(A)\) \(p\) is isolated and \(p\in [D]\), take \(\barb\vDash p\), \(\tp(\barb/A)=p\)
    isolated, \(\barb\in D\) because \(p\in [D]\)
    #+END_proof

* Morley's theorem
** Prime models in totally transcendental theories
    \(T\) is complete

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(M\vDash T\) is *prime* if \(\forall N\vDash T\), there is \(f:M\xrightarrow{\preceq}N\).

    \(M\preceq\M\) is prime if \(\forall N\preceq\M\), there is \(\sigma\in\Aut(\M)\), \(\sigma(M)\subseteq N\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_fact
    If \(T\) is totally transcendental, there is a prime model, unique up to isomorphism
    #+END_fact

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(M\preceq\M\), \(M\) is *prime over \(A\subseteq M\)* if for all \(N\preceq\M\), \(N\supseteq A\), there is \(\sigma\in\Aut(\M/A)\)
    s.t. \(\sigma(M)\subseteq N\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_examplle
    If \(\acl(A)\preceq\M\), then \(\acl(A)\) is prime over \(A\). Let \(M=\acl(A)\). If \(A\subseteq N\preceq\M\),
    then \(M=\acl(A)\subseteq N\)

    This holds \(\forall A\subseteq M\vDash\ACF\)
    #+END_examplle

    #+BEGIN_remark
    Suppose \(\abs{L}=\aleph_0\), \(M\vDash T\), then \(\abs{M}=\infty\). If \(M\) is prime over \(A\subseteq M\),
    then \(\abs{M}=\abs{A}+\abs{L}\)

    LST get a \(N\supseteq A\), \(N\preceq\M\), \(\abs{N}\le\abs{A}+\abs{L}=\abs{A}+\aleph_0\)
    #+END_remark

    \(p\in S_n(B)\) is isolated if \(\{p\}\) is clopen

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(B\) is *constructible over \(A\)* if \(B=\{b_\alpha:\alpha<\beta\}\) where \(\tp(b_\alpha/AB_{<\alpha})\) is
    isolated, \(B_{<\alpha}=\{b_\gamma:\gamma<\alpha\}\).

    We call \((b_\alpha:\alpha<\beta)\) a *construction* of \(B\) over \(A\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    label:5.12.1.8
    Suppose \(\tp(c/A)\) is isolated, suppose \(f:A\to B\subseteq M\preceq\M\) partial elementary, then there is \(c'\in M\)
    s.t. \(f\cup\{(c,c')\}\) is a partial elementary map
    #+END_lemma

    #+BEGIN_proof
    \(\exists\sigma\in\Aut(\M)\), \(\sigma\supseteq f\). Replace \(A,c\)  with \(\sigma(A),\sigma(c)\), WMA \(A=B\), \(f=\id_A\).
    Take \(\varphi(x)\in\tp(c/A)\) isolating \(\tp(c/A)\), then \(\M\vDash\exists x\varphi(x)\) so \(M\vDash\exists x\varphi(x)\).
    Take \(c'\in M\), \(M\vDash\varphi(c')\), then \(c'\vDash\tp(c/A)\) and so \(c\equiv_Ac'\),
    therefore \(\id_A\cup\{(c,c')\}\) is a partial elementary map
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    If \(A\subseteq M\preceq\M\) and \(M\) is constructible over \(A\), then \(M\) is prime over \(A\)
    #+END_theorem

    #+BEGIN_proof
    If \(A\subseteq N\preceq\M\), take a construction \((b_\alpha:\alpha<\beta)\) for \(M\) over \(A\). By Lemma ref:5.12.1.8 and
    induction, there is \(c_0,c_1,\dots,c_\alpha,\dots\in N\) s.t. \(\id_A\cup\{(b_0,c_0),\dots,(b_\alpha,c_\alpha)\}\) is partial
    elementary

    Let \(f=\id_A\cup\{(b_\alpha,c_\alpha):\alpha<\beta\}\), then \(f\) is partial elementary map
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \((B_\alpha:\alpha<\beta)\) is a sequence, if \(B_\alpha\) is constructible over \(A\cup\bigcup_{\gamma<\alpha}B_\gamma\) for all \alpha, then
    \(\bigcup_{\alpha<\beta}B_\alpha\) is constructible over \(A\)
    #+END_lemma

    #+BEGIN_proof
    concatenate constructions
    #+END_proof

    #+ATTR_LATEX: :options [ \(T\) is t.t.]
    #+BEGIN_fact
    If \(\emptyset\neq D\subseteq\M^n\) and \(D\) is \(A\)-definable, then there is \(\barb\in D\) s.t. \(\tp(\barb/A)\) is isolated
    #+END_fact
    ...
    #+BEGIN_lemma
    label:5.12.1.11
    Given \(A\subseteq\M\), there is \(B\subseteq\M\) s.t.
    - \(B\) is constructible over \(A\)
    - If \(D\subseteq\M\), \(D\) \(A\)-definable, \(D\neq\emptyset\), then \(B\cap D\neq\emptyset\)
    #+END_lemma

    #+BEGIN_proof
    Let \((D_\alpha:\alpha<\kappa)\) be all nonempty \(A\)-definable sets. By Fact ref:5.12.1.10 and recursion,
    choose \(b_\alpha\in D_\alpha\) s.t. \(\tp(b_\alpha/A\cup\{b_\gamma:\gamma<\alpha\})\) is isolated

    Let \(B=\{b_\alpha:\alpha<\kappa\}\), then \(B\) is constructible over \(A\)
    #+END_proof

    #+BEGIN_remark
    In Lemma ref:5.12.1.11, \(B\supseteq A\) because if \(a\in A\), \(\{a\}\)
    is \(A\)-definable, \(\{a\}\cap B\neq\emptyset\), \(a\in B\)
    #+END_remark

    #+ATTR_LATEX: :options [ \(T\) is t.t.]
    #+BEGIN_theorem
    label:5.12.1.12
    If \(A\subseteq\M\) then there is \(A\subseteq M\preceq\M\), \(M\) is constructible over \(A\), \(M\) is prime over \(A\)
    #+END_theorem

    #+BEGIN_proof
    By Lemma ref:5.12.1.11, there is \(B_0,B_1,B_2,\dots\) s.t. \(B_i\) is constructible
    over \(AB_0\dots B_{i-1}\) and if \(\emptyset\neq D\subseteq\M\), \(D\) definable over \(AB_0\dots B_{i-1}\),
    then \(D\cap B_i\neq\emptyset\). By remark, \(A\subseteq B_0\subseteq B_1\subseteq\dots\). Let \(M=\bigcup_{i<\omega}B_i\). Then \(A\subseteq M\) and \(M\) is
    constructible over \(A\). \(M\preceq\M\) by Tarski-Vaught test: if \(\emptyset\neq D\subseteq\M\) and \(D\)
    is \(M\)-definable, want \(D\cap M\neq\emptyset\). \(D\) is \(B_i\)-definable for sufficiently large \(B_i\)
    #+END_proof
** Vaught pairs
    \(M\prec N\) means \(M\) is a proper elementary substructure

    #+BEGIN_remark
    If \(\varphi\in L(M)\), \(\varphi(M)\) is finite, \(N\succeq M\), then \(\varphi(N)=\varphi(M)\)
    #+END_remark

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    A *vaught pair* is \(M\prec N\) and \(\varphi(x)\in L(M)\) s.t. \(\varphi(M)=\varphi(N)\) and \(\abs{\varphi(M)}=\infty\)
    #+END_definition

    #+BEGIN_remark
    label:5.12.2.3
    If \(\varphi(x,\bary)\in L\), there is \(T_\varphi\)
    s.t. \((M,P,\barb)\vDash T_\varphi\Leftrightarrow M\vDash T, P\prec M,\barb\in P,\varphi(M,\barb)=\varphi(P,\barb)\) except the infinity things
    #+END_remark

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    If \(T\) has a Vaught pair, then there is Vaught pair \(M\prec N\), \varphi where \(\abs{N}\le\abs{L}\)
    #+END_corollary

    #+BEGIN_proof
    Downward LST to \(T_\varphi\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    Suppose \(\forall n<\omega\), there is \((M,P,\barb)\vDash T_\varphi\) with \(\abs{\varphi(M,\barb)}\ge n\). Then by
    compactness, there is \((M,P,\barb)\vDash T_\varphi\) with \(\abs{\varphi(M,\barb)}=\infty\)

    Suppose \(\varphi(x,\bary)\in L\), \(\forall n<\omega\), there is \(M\vDash T\), \(\barb\in M\), \(n\le\abs{\varphi(M,\barb)}<\infty\),
    then take \(N\succ M\), so we get \((N,M,\barb)\vDash T_\varphi\) with \(\abs{\varphi(N,\barb)}\ge n\), then we get a
    Vaught pair
    #+END_corollary

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    If \(\forall n<\omega\) there is \(M\vDash T\), \(\barb\vDash T\), \(\barb\in M\), \(n\le\abs{\varphi(M,\barb)}<\infty\), then \(T\)
    has a Vaught pair
    #+END_corollary
    ...
    #+BEGIN_lemma
    label:5.12.2.5
    If \(T\) doesn't have a Vaught pair, then \(\forall\varphi\exists n_\varphi\forall M\vDash T\forall\barb\in M\)
    either \(\abs{\varphi(M,\barb)}<n_\varphi\) or \(\abs{\varphi(M,\barb)}=\infty\),

    therefore \(T\) eliminates \(\exists^\infty\), \(\exists^\infty x\varphi(x,\bary)\Leftrightarrow\exists^{>n_\varphi}x\varphi(x,\bary)\)
    #+END_lemma

    #+ATTR_LATEX: :options [Morley,Baldwin,Lachlan]
    #+BEGIN_theorem
    label:5.12.2.4
    Let \(T\) be a complete \(L\)-theory, \(L\) countable, models of \(T\) are infinite. TFAE
    1. for some uncountable \kappa, \(T\) is \kappa-categorical
    2. \(T\) is t.t. and \(T\) has no Vaught pair
    3. for every countable cardinal \kappa, \(T\) is \kappa-categorical
    #+END_theorem

    #+BEGIN_proof
    \(1\leftrightarrow 3\): morley's theorem, \(3\leftrightarrow 2\): baldwin, lachlan

    \(2\to 3\): Assume \(T\) is t.t., \(T\) has no Vaught pairs. By Lemma ref:5.12.2.5., \(T\)
    eliminates \(\exists^\infty\)
    #+END_proof

    Assume in this section that the language is countable, models are infinite, and \(T\) is totally
    transcendental with no Vaugtian pairs.

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    If \(M\preceq\M\), \(D\) is \(M\)-definable, then
    1. \(D\) is *minimal* if \(\abs{D}=\infty\) and every \(M\)-definable \(D'\subseteq D\), \(\abs{D'}<\infty\)
       or \(\abs{D\setminus D'}<\infty\)
    2. \(D\) is *strongly minimal* if \(\M\)-definable
    #+END_definition
    ...
    label:5.12.2.7
    If \(M\preceq\M\)
    1. there is \(M\)-definable minimal set \(D\subseteq\M\)
    2. If \(D\subseteq\M\) is minimal then \(D\) is strongly minimal
    #+END_lemma

    #+BEGIN_proof
    1. Let \(R(-)\) be the Cantor-Bendixson rank on \(S_1(M)\). \(R(D)>0\Leftrightarrow\abs{D}=\infty\)
       for \(M\)-definable \(D\subseteq\M\)

       Let \(E_\alpha=\{p\in S_1(M):R(p)\ge\alpha\}\), \(R(S_1(M))=R(\M)>0\), so \(E_1\neq\emptyset\).

       If \(E_1=E_2\), then \(E_1\) is perfect, contradicting the fact that \(S_1(M)\) is scattered
       because \(T\) is totally transcendental (ref:5.7.7.1)

       Therefore \(E_1\supsetneq E_2\). Take \(p\in E_1\setminus E_2\), then \(p\) is an isolated point of \(E_1\).
       Take a clopen set \([D]\subseteq S_1(M)\) with \([D]\cap E_1=\{p\}\). Then \(R([D])=1>0\), so \(D\) is
       infinite. For \(M\)-definable \(D'\subseteq D\), we have \([D]=[D']\sqcup[D\setminus D']\), if \(p\in[D']\)
       then \([D\setminus D']\cap E_1=\emptyset\) so \(R(D\setminus D')\le 0\) and \([D\setminus D']<\infty\). Similar for \(p\in[D\setminus D']\)

    2. Suppose \(D\) is not strongly minimal, witnessed by \(\M\)-definable \(D'\subseteq D\), s.t. \(D'\)
       and \(D\setminus D'\) are infinite. Write \(D\) and \(D'\) as \(\varphi(\M;\barb)\) and \(\psi(\M;\barc)\)
       for \(\barb\in M\) and \(\barc\in\M\). Then
       \begin{align*}
       \M\vDash\exists^\infty\barx(\varphi(\barx;\barb)\wedge\psi(\barx;\barc))\\
       \M\vDash\exists^\infty\barx(\varphi(\barx;\barb)\wedge\neg\psi(\barx;\barc))
       \end{align*}
       By Lemma ref:5.12.2.5, the right hand sides are first-order. The fact that \(M\preceq\M\) implies we
       can change \(\barc\)
    #+END_proof

    #+ATTR_LATEX: :options [continued]
    #+BEGIN_proof
    By Theorem ref:5.12.1.12, there is a prime model \(M_0\), \(\abs{M_0}=\aleph_0\). By Lemma
    ref:5.12.2.7 there is an \(M_0\)-definable strongly minimal set \(D\subseteq\M\). Let \(p\in S_1(M_0)\) be
    the transcendental type of \(D\), the type generated by
    \begin{equation*}
    \{x\in D\}\cup\{x\notin D':D'\subseteq D,D'\text{ $M_0$-definable},\abs{D'}<\infty\}
    \end{equation*}
    Let \(\hatp\in S_1(\M)\) be the analogous global transcendental type in \(D\). Then \(\hatp\)
    is \(M_0\)-invariant hence \(M_0\)-definable, and \(\hatp\supseteq p\), implying that \(p\) is
    stationary and \(\hatp\)is its global non-forking extension

    Suppose \(D=\psi(\M)\), \(\varphi(x)\in\hatp(x)\Leftrightarrow\abs{\neg\varphi(\M)\wedge\psi(\M)}<\infty\). But for
    any \(\sigma\in\Aut(\M/M_2)\), \(\sigma(D)=D\), also \(\abs{\sigma(\neg\varphi(\M)\wedge\psi(\M))}=\abs{\neg\varphi(\M)\wedge\psi(\M)}\), therefore \(\sigma(\neg\varphi(x))\in\hatp\)
    #+END_proof

    #+BEGIN_remark
    Let \(I=(b_\alpha:\alpha<\kappa)\) where \(b_\alpha\vDash p\). TFAE
    1. \(I\) is independent over \(M_0\)
    2. \(I\) is a Morley sequence of \(\hatp\) over \(M_0\)
    3. \(b_\alpha\notin\acl(M_0b_{<\alpha})\)
    #+END_remark

    #+BEGIN_proof
    Similar to ref:5.5.2.2
    #+END_proof

    For each \kappa, let \(I_\kappa\) be a Morley sequence of \(\hatp\) over \(M_0\) of length \kappa.
    Let \(M_\kappa\preceq\M\) be prime over \(M_0\cup I_\kappa\). Note \(\abs{M_\kappa}=\kappa+\aleph_0\)

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    \(T\) is \kappa-categorical for every uncountable \kappa
    #+END_corollary

    #+BEGIN_proof
    Because \(M_0\)  is prime, there is an elementary embedding \(M_0\to M\). Moving \(M\) by an
    automorphism, we may assume \(M_0\subseteq M\). Let \(I\) be a maximal independent set of realizations
    of \(p\) in \(M\). Let \(\kappa=\abs{I}\). Let \(f:I\to I_\kappa\) be any bijection. Then \(f\) is a partial
    elementary map over \(M\) by ref:5.5.2.4. So \(f\cup\id_{M_0}:(M_0\cup I)\to(M_0\cup I_\kappa)\) is a partial
    elementary map. Moving \(M\) and \(I\) by an automorphism, we may assume \(I=I_\kappa\).
    As \(M_\kappa\) is prime over \(M_0\cup I_\kappa\), there is an elementary embedding \(M_\kappa\to M\)
    over \(M_0\cup I_\kappa\). Moving \(M_\kappa\) by an automorphism over \(M_0\cup I_\kappa\), we may
    assume \(M_\kappa\subseteq M\). We claim \(M_\kappa=M\). Suppose \(M_\kappa\subsetneq M\).
    Then \(D(M_\kappa)\subsetneq D(M)\), or else \((M_\kappa,M,D)\) is a Vaught pair.
    Take \(b\in D(M)\setminus D(M_\kappa)\), note \(\acl(M_0I_\kappa)\subseteq\acl(M_\kappa)=M_\kappa\), therefore \(b\notin\acl(M_0I_\kappa)\),
    then \(b\vDash\hatp\uhr M_0I_\kappa\) and \(I_\kappa\cup\{b\}\) is a larger independent set of realizations of \(p\)
    in \(M\), contraidicting the maximality of \(I\)
    #+END_proof

    Goal: If \(T\) is \kappa-categorical for some \(\kappa>\aleph_0\), then \(T\) is t.t. and has no Vaught pairs

    \(T\) is \kappa-categorical \(\Rightarrow\) \(T\) is \omega-stable \(\Leftrightarrow\) \(T\) is t.t.

    Goal: \(T\) has no Vaught pairs
** Constructible models are atomic
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(B\) is *atomic* over \(A\) if \(\tp(\barb/A)\) is isolated \(\forall\barb\in B\)
    #+END_definition
    ...
    #+BEGIN_remark
    If \(\tp(b/A)\) is isolated by \(\varphi(x)\in L(A)\), then \(Ab\) is atomic over \(A\)
    #+END_remark

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(A\subseteq B\subseteq C\), if \(B\) is atomic over \(A\), \(C\) is atomic over \(B\), then \(C\) is atomic
    over \(A\)
    #+END_lemma

    #+BEGIN_proof
    If \(\barc\in C\), take \(\varphi(\barx,\barb)\in\tp(\barc/B)\) isolating \(\tp(\barc/B)\),
    take \(\psi(\bary)\in\tp(\barb/A)\) isolating it

    *Claim* \(\theta(\barx)=\exists\bary\varphi((\barx,\bary)\wedge\psi(\bary))\) isolates \(\tp(\barc/A)\)

    \(\barc\vDash\theta(\barx)\), take \(\bary=\barb\)

    Suppose \(\barc_0\vDash\theta\) take \(\barb_0\)
    s.t. \(\varphi(\barc_0,\barb_0)\wedge\psi(\barb_0)\), \(\psi(\barb_0)\Rightarrow\barb_0\equiv_A\barb\). WMA \(\barb_0=\barb\).
    Then \(\varphi(\barc_0,\barb)\Rightarrow\barc_0\equiv_B\barc\Rightarrow\barc_0\equiv_A\barc\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(A\subseteq M\preceq\M\) and \(M\) is constructible over \(A\), then \(M\) is atomic over \(A\)
    #+END_proposition

    #+BEGIN_proof
    Take \((b_\alpha:\alpha<\beta)\) of construction of \(M\) over \(A\).

    Show \(A\cup\{b_\alpha:\alpha<\gamma\}\) is atomic over \(A\) for \(\gamma\le\beta\) by induction on \gamma

    If \(\gamma=0\)

    \gamma is limit, if \(\barc\in A\cup\{b_\alpha:\alpha<\gamma\}\), then there is \(\gamma'<\gamma\) \(\barc\in A\cup\{b_\alpha:\alpha<\alpha'\}\)

    \(\alpha+1\), \(AB_{<\alpha}\) is atomic over \(A\), \(\tp(b_\alpha/AB_{<\alpha})\) is isolated,
    therefore \(AB_{<\alpha+1}\) is atomic over \(AB_{<\alpha}\), then \(AB_{<\alpha+1}\) is atomic over \(A\)
    #+END_proof
** Orthogonality
    Assume \(T\) is t.t.
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    If \(M\vDash T\), \(p\in S_1(M)\), \(\varphi(x)\in L(M)\), then \(p\) is *orthogonal to \(\varphi(x)\)* (\(p\bot\varphi\))
    if \(\exists N\succeq M\), \(p\) is realized in \(N\) but \(\varphi(M)=\varphi(N)\)
    #+END_definition

    #+BEGIN_remark
    If \(b\vDash p\), \(b\in N\), we may assume \(N\) is constructible over \(Mb\)
    #+END_remark

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    label:5.12.4.3
    If \(p\in S_1(M)\), \(\varphi(x)\in L(M)\), \(p\slashed{\bot}\varphi\) iff \(\exists\psi(x,y,\barz)\in L\), \(\exists\barc\in M\) s.t.
    * \(p(y)\vdash\exists x[\varphi(x)\wedge\psi(x,y,\barc)]\)
    * if \(a\in M\), then \(p(y)\vdash\neg\psi(a,y,\barc)\)
    #+END_lemma

    #+BEGIN_proof
    Suppose \psi,\(c\) exist. Suppose \(N\succeq M\), \(b\in N\), \(b\vDash p\), there
    is \(a_0\in N\), \(N\vDash\varphi(a_0)\wedge\psi(a_0,b,c)\)

    \(a_0\in\varphi(N)\), if \(a_0\in M\), then \(\neg\psi(a_0,b_0,c_0)\), so \(a_0\notin M\), \(a_0\in\varphi(N)\setminus\varphi(M)\)

    Conversely, suppose \(p\slashed{\bot}\varphi\), take \(b\vDash p\), take \(N\) constructible
    over \(Mb\). \(\varphi(N)\supsetneq\varphi(M)\), take \(a\in\varphi(N)\setminus\varphi(M)\). \(N\) is atomic
    over \(Mb\), \(\tp(a/Mb)\) is isolated. Take \(\psi(x,b,c)\)
    isolating \(\tp(a/Mb)\), \(\psi\in L\), \(c\in M\), \(N\vDash\varphi(a)\wedge\psi(a,b,c)\), \(N\vDash\exists x(\varphi(x)\wedge\psi(x,b,c))\)

    If \(a_0\in M\) and \(\psi(a_0,b,c)\). Then \(a\equiv_{Mb}a_0\) so \(a\notin M\Rightarrow a_0\notin M\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    label:5.12.4.4
    If \(M\preceq M'\preceq\M\), \(p\in S_1(M)\), \(p'\in S_1(M')\), if \(p'\sqsupseteq p\) and \(\varphi(x)\in L(M)\), then
    \begin{equation*}
    p\bot\varphi\Leftrightarrow p'\bot\varphi
    \end{equation*}
    #+END_lemma

    #+BEGIN_proof
    The conditions of Lemma ref:5.12.4.3 can be translated into first-order properties of \((M,dp)\)

    and \((M,dp)\preceq(M',dp')\) (\(p'\) has same definitions as \(p\)) ref:2.24.15
    #+END_proof

    #+ATTR_LATEX: :options [Stretching Vaught pairs in t.t. theories]
    #+BEGIN_proposition
    label:5.12.4.5
    Suppose \(T\) is t.t., Suppose there is a Vaught
    pair \(M\prec N\), \(\varphi(x)\in L(M)\), \(\varphi(M)=\varphi(N)\), \(\abs{\varphi(M)}=\infty\), then
    1. \(\forall M'\succeq M\) there is \(N'\succ M'\) with \(\varphi(N')=\varphi(M')\). We may take \(\abs{N'}\le\abs{M'}+\abs{L}\)
    2. For \(\kappa\ge\abs{M}+\abs{L}\) there is \(N\succeq M\) with \(\abs{N}=\kappa\), \(\varphi(N)=\varphi(M)\)
    #+END_proposition

    #+BEGIN_proof
    1. Take \(b\in N\setminus M\), \(p=\tp(b/M)\), \(p\bot\varphi\), \(p'\in S_1(M')\) be the heir. By Lemma
       ref:5.12.4.4, \(p'\bot\varphi\), there is \(N'\succeq M'\), \(b'\vDash p'\), \(b'\in N'\), \(\varphi(N')=\varphi(M')\)

       \(p\) not constant \(\Rightarrow\) \(p'\) not constant, so \(b'\notin M'\) and \(N'\succ M'\).

    2. Iterating part 1, build an increasing elementary chain \((M_\alpha:\alpha<\kappa)\) where
       * \(M_0=M\)
       * \(M_{\alpha+1}\succ M_\alpha\) with \(\varphi(M_{\alpha+1})=\varphi(M_\alpha)\) and \(\abs{M_{\alpha+1}}\le\abs{M_\alpha}+\abs{L}\)
       * If \beta is a limit ordinal, then \(M_\beta=\bigcup_{\alpha<\beta}M_\alpha\)
       Take \(N=\bigcup_\alpha M_\alpha\).
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    Suppose \(\abs{L}=\aleph_0\), \(T\) is t.t. , \(T\) has a Vaught pair, \(M\prec N\), \varphi,
    WLOG, \(\abs{M}=\aleph_0\), \(\forall\kappa\ge\aleph_0\) there is \(N\succeq M\), \(\abs{N}=\kappa\), \(\varphi(N)=\varphi(M)\), \(\abs{\varphi(N)}=\aleph_0\)
    #+END_corollary

** The end of the proof
    Say that an elementary extension \(N\succeq M\) *enlarges infinite set* if \(D(N)\supsetneq D(M)\) for
    every infinite \(M\)-definable set \(D\subseteq\M^n\)

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(M\preceq\M\) then there is \(N\succeq M\) s.t. \(\abs{N}\le\abs{M}+\abs{L}\) and the extension \(N\)
    enlarges infinite sets
    #+END_lemma

    #+BEGIN_proof
    Let \(\kappa=\abs{L(M)}=\abs{L}+\abs{M}\). Let \(\{D_\alpha:\alpha<\kappa\}\) enumerate all infinite \(M\)-definable
    sets. For each \alpha, the set \(D_\alpha=D_\alpha(\M)\) is large by saturation (we can add \(x\neq m\) for
    all \(m\in M\), then by induction \(D_\alpha\) is large) . Take \(\barb_\alpha\in D_\alpha(\M)\setminus D_\alpha(M)\). By
    downward Löwenheim–Skolem Theorem there is a small model \(N\) containing \(M\cup\{\barb_\alpha:\alpha<\kappa\}\)
    with \(\abs{N}\le\abs{M}+\kappa=\abs{M}+\abs{L}\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    Suppose models of \(T\) are infinite. If \(\kappa\ge\abs{L}\) then there is a model \(M\preceq\M\)
    s.t. \(\abs{M}=\kappa\) and \(\abs{D(M)}=\kappa\) for every infinite \(M\)-definable set
    #+END_proposition

    #+BEGIN_proof
    Recursively define an increasing elementary chain \((M_\alpha:\alpha<\kappa)\) as follows
    * \(M_0\) is a model of size \(\abs{L}\) which exists by Löwenheim–Skolem Theorem
    * If \beta is a limit ordinal, then \(M_\beta=\bigcup_{\alpha<\beta}M_\alpha\)
    * \(M_{\alpha+1}\succeq M_\alpha\) is an extension which enlarges infinite sets
      with \(\abs{M_{\alpha+1}}\le\abs{L}+\abs{M_\alpha}\)


    Let \(N=\bigcup_{\alpha<\kappa}M_\alpha\), \(\abs{N}\le\kappa\). It remains to show that \(\abs{D(N)}\ge\kappa\) for
    infinite \(N\)-definable sets \(D\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    Let \(T\) be a complete theory with infinite models in a countable language. If \(T\) is
    \kappa-categorical for some \(\kappa>\aleph_0\), then \(T\) is totally transcendental and \(T\) has no Vaught pairs
    #+END_theorem

    #+BEGIN_proof
    \(T\) is \omega-stable by ref:3.24.32. By ref:5.7.7.2 \omega-stable is equivalent to totally
    transcendental since the language is countable

    Assume for the sake of contradiction that \(T\) has a Vaught pair \((M,N,\varphi(\barx;\barb))\).
    Then \((N,M,\barb)\) is a model of the theory \(T_\varphi\) of remark ref:5.12.2.3. Applying downward
    Löwenheim–Skolem Theorem, we may assume \(N,M\) are countable. By Proposition ref:5.12.4.5 (2)
    there is an elementary extension \(N_1\succeq M\) with \(\abs{N_1}=\kappa\) s.t. \(\varphi(N_1;\barb)=\varphi(M;\barb)\).

    By Proposition ref:5.12.5.2, there is also a model \(N_2\) of cardinality \kappa s.t. \(D(N_2)=\kappa\) for
    every infinite \(N_2\)-definable set. Therefore \(N_1\not\cong N_2\), and \kappa-categoricity fails
    #+END_proof
...
** Foundation rank
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(\RF(x)\alpha\) is defined:
    * \(\RF(x)\ge\alpha\) always true
    * \(\RF(x)\ge\alpha+1\Leftrightarrow\exists y<x\RF(y)\ge\alpha\)
    * If \alpha limit, \(\RF(x)\ge\alpha\) iff \(\forall\beta<\alpha\), \(\RF(x)\ge\beta\)


    Then \(\RF(x)=\max\{\alpha\in\On:\RF(x)\ge\alpha\}\) or \(\infty\). \(\RF(x)\in\Ord\cup\{\infty\}\)
    #+END_definition

    #+BEGIN_remark
   \(\RF(x)\ge n\Leftrightarrow\exists x_0<x_1<\dots<x_n=x\) where \(n\in\N\)
    #+END_remark

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    label:5.26.1.2
    If \(\RF(x)=\infty\), then \(\exists y<x\), \(\RF(y)=\infty\)
    #+END_lemma

    #+BEGIN_proof
    If not, then \(\forall y<x\), \(\RF(y)\in\On\). Let \(\alpha=\sup\{\RF(y):y<x\}\), then \(\RF(x)\ge\alpha+2\), so
    there is \(y<x\) s.t. \(\alpha\ge\RF(y)\ge\alpha+1\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    \(\RF(x)=\infty\) \(\Leftrightarrow\) \(\exists x_0,x_1,\dots\), \(x=x_0>x_1>x_2>\dots\)
    #+END_proposition

    #+BEGIN_proof
    \(\Rightarrow\) ref:5.26.1.2

    \(\Leftarrow\): Fix \(x=x_0>x_1>x_2>\dots\)

    *Claim*: \(\forall\alpha\in\On\forall i\in\omega,\RF(x_i)\ge\alpha\)

    Induction on \alpha
    1. 0
    2. limit
    3. \(\alpha+1\): if \(\forall i\in\omega\RF(x_i)\ge\alpha\), then since \(\forall i(x_i>x_{i+1})\), \(\RF(x_i)\ge\alpha+1\)
    #+END_proof

    #+BEGIN_remark
    1. \(x\ge y\) then \(\RF(x)\ge\RF(y)\)
    2. \(x>y\) then \(\RF(x)>\RF(y)\) or \(\RF(x)=\RF(y)=\infty\)

       By 1, \(\RF(x)\ge\RF(y)\). WLOG, \(\RF(x)=\RF(y)=\alpha\in\On\)
    3. If \(P\) has DCC then \(x>y\Rightarrow\RF(x)>\RF(y)\)
    #+END_remark

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(\RF(x)=\alpha<\infty\) and \(\beta\le\alpha\) then \(\exists y\le x(\RF(y)=\beta)\)
    #+END_lemma

    #+BEGIN_proof
    Let \(S=\{y\le x:\RF(y)\ge\beta\}\). \(x\in S\) so \(S\) is not empty. Take \(y\in S\) minimizing \(\RF(y)\)
    since ordinals are well-ordered. If \(\RF(y)=\beta\) done. Otherwise \(\RF(y)\ge\beta+1\) and there
    is \(z<y\) and \(\RF(z)\ge\beta\). Then \(z\in S\), a contradiction
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    \(\{\RF(x):x\in P\}\cap\On\) is an initial segment of \(\On\) / downward closed
    #+END_corollary
** Lascar rank
    Assume \(T\) is stable.
    ...
    #+BEGIN_lemma]
    label:5.26.2.1
    If \(p\in S_n(A)\), \(\tbd(p)=\beta\) and \(\beta'<\beta\) then \(\exists q\in S_n(B)\), \(q\subseteq p\), \(\tbd(q)=\beta'\)
    #+END_lemma

    #+BEGIN_proof
    Take \(M\supseteq A\), \(M\preceq\M\), \(\exists p'\in S_n(M)\), \(p'\supseteq p\), \([p']=\beta\). By ref:4.28.8.1, there
    is \(N\succeq M\), \(q\in S_n(N)\), \(q\supseteq p'\) s.t. \([q]=\beta'\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    If \(p\in S_n(A)\), \(U(p)\) (*Lascar rank*) is \(\RF(\tbd(p))\) in the fundamental order
    #+END_definition
    ...
    If \(p\in S_n(A)\)
    1. \(U(p)\ge 0\)
    2. If \alpha is a limit ordinal, then \(U(p)\ge\alpha\Leftrightarrow\forall\beta<\alpha\), \(U(p)\ge\beta\)
    3. \(U(p)\ge\alpha+1\Leftrightarrow\exists\) forking extension \(q\supseteq p\) s.t. \(U(q)\ge\alpha\)
    #+END_proposition

    #+BEGIN_proof
    3. [@3] If \(U(p)\ge\alpha+1\), let \(\beta=\tbd(p)\), \(\RF(\beta)\ge\alpha+1\). Then there
       is \(\beta'<\beta\), \(\RF(\beta')\ge\alpha\). By ref:5.26.2.1 there is \(q\supseteq p\)
       and \(\tbd(q)=\beta'\), \(q\not\sqsupseteq p\) because \(\tbd(q)<\tbd(p)\).
    #+END_proof

    Assume \(T\) is superstable. Then \(T\) is \lambda-stable for all \(\lambda\gg 0\)

    Then \(U(p)<\infty\) for all \(p\). (analogous to \(T\) is t.t. \(\Leftrightarrow\) \(\RM(p)<\infty\))

    strongly minimal \(\Rightarrow\) t.t.

    uncountably categorical \(\Rightarrow\) t.t.

    t.t. \(\Rightarrow\) superstable

    \(\R\)-vector spaces is strongly minimal, not \(2^{\aleph_0}\)-categorical \(\R^1,\R^2,\dots\)

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    label:5.26.2.4
    If \(p\in S_n(A)\), \(q\in S_n(B)\), \(p\subseteq q\), then
    1. \(U(q)\le U(p)\)
    2. \(U(q)=U(p)\Leftrightarrow q\sqsupseteq p\)
    #+END_proposition

    Proposition ref:5.26.2.4 is analogous of ref:5.7.8.2 for t.t. theories:
    If \(q\supseteq p\), then \(\RM(q)=\RM(p)\Leftrightarrow q\sqsupseteq p\)

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(T\) t.t. then \(U(p)\le\RM(p)\)
    #+END_proposition

    #+BEGIN_proof
    We prove by induction on \alpha
    1. \(\alpha=0\)
    2. limit ordinal
    3. If \(U(p)\ge\alpha+1\), then there is a forking extension \(q\supseteq p\), \(U(q)\ge\alpha\). \(\RM(p)>\RM(q)\),
       then by induction \(\RM(q)\ge\alpha\), therefore \(\RM(p)\ge\alpha+1\)
    #+END_proof

    #+BEGIN_remark
    If \(U(p)=\alpha\) and \(\beta\le\alpha\), then there is \(q\) with \(U(q)=\beta\) by Lemma ref:5.26.1.6
    #+END_remark
    ...
    Proposition ref:5.26.2.4 says
    1. if \(B\subseteq C\), \(U(\bara/B)\ge U(\bara/C)\), and
       \(U(\bara/B)=U(\bara/C)\Leftrightarrow\tp(\bara/B)\sqsubseteq\tp(\bara/C)\Leftrightarrow a\ind_BC\)


    #+BEGIN_remark
    Proposition ref:5.26.2.3 says \(U(\bara/B)\ge\alpha+1\Leftrightarrow\exists\bara',C\;\tp(\bara'/BC)\not\sqsupseteq\tp(\bara/B)\) and \(U(\bara'/BC)\ge\alpha\)

    \(\bara'\equiv_B\bara\) so WLOG \(\bara'=\bara\), so
    \(U(\bara/B)\ge\alpha+1\Leftrightarrow\exists C\tp(\bara/BC)\not\sqsupseteq\tp(\bara/B)\Leftrightarrow\exists C\) \(\bara\not\ind_BC\) and \(U(\bara/BC)\ge\alpha\)
    #+END_remark
** Lascar inequalities
    Assume \(T\) is superstable
    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    \(U(\bara/B)=0\Leftrightarrow\bara\in\acl(B)\)
    #+END_proposition

    #+BEGIN_proof
    Show \(U(\bara/B)\ge 1\Leftrightarrow\bara\notin\acl(B)\)

    \(U(\bara/B)\ge 1\Leftrightarrow\exists C\;\bara\not\ind_BC\). If \(\bara\notin\acl(B)\), then \(\bara\not\ind_B\bara\)
    (ref:4.28.11.2). If \(\bara\in\acl(B)\), then \(\forall C\), \(C\ind_B\acl(B)\), then \(C\ind_B\bara\).
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(U(\bara/\barb C)\ge\alpha\), then \(U(\bara\barb/C)\ge\alpha\), equivalently \(U(\bara/\barb C)\le U(\bara\barb/C)\)
    #+END_lemma

    #+BEGIN_proof
    0, limit is trivial

    Suppose \(U(\bara/\barb C)\ge\alpha+1\). By remark ref:5.26.2.9, there is \(C'\)
    and \(\barb\not\ind_{\barb C}C'\) s.t. \(U(\bara/\barb CC')\ge\alpha\).
    If \(\bara\barb\ind_CC'\) then base monotonicity gives \(\bara\ind_{\barb C}C'\) a
    contradiction. Therefore \(\bara\barb\not\ind_CC'\). By induction, \(U(\bara\barb/CC')\ge\alpha\), so
    \(U(\bara\barb/C)\ge\alpha+1\)
    #+END_proof

    For ordinals, there are two kinds of addition \(+\), \(\oplus\)

    Goal: Lascar inequalities
    \begin{equation*}
    U(\bara/\barb C)+U(\barb/C)\le U(\bara\barb/C)\le U(\bara/\barb C)\oplus U(\barb/C)
    \end{equation*}

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    \(U(\bara/\barb C)+U(\barb/C)\le U(\bara\barb/C)\)
    #+END_proposition

    #+BEGIN_proof
    *Claim*: If \(\alpha,\beta\in\On\), \(\alpha\le U(\bara/\barb C)\), \(\beta\le U(\barb/C)\), then \(\alpha+\beta\le U(\bara\barb/C)\)

    Fix \alpha, induction on \beta
    1. \(\beta=0\) the lemma
    2. limit case is easy
    3. \(\beta+1\), suppose \(\beta\) works. \(U(\barb/C)\ge\beta+1\) implies there is \(C'\)
       s.t. \(\barb\not\ind_CC'\) and \(U(\barb/CC')\ge\beta\). Move \(C'\) by
       automorphism \(\sigma\in\Aut(\M/\barb C)\) get \(C'\ind_{\barb C}\bara\).
       So \(U(\bara/\barb CC')=U(\bara/\barb C)\ge\alpha\)

       by induction: \(U(\bara\barb/CC')\ge\alpha+\beta\)

       If \(\bara\barb\ind_CC'\), then \(\barb\ind_CC'\) by monotonicity(no).
       So \(\bara\barb\not\ind_CC'\), so \(U(\bara\barb/C)\ge\alpha+\beta+1\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    \(U(\bara\barb/C)\le U(\bara/\barb C)\oplus U(\barb/C)\)
    #+END_proposition

    #+BEGIN_proof
    Induction again

    If \(\alpha+1\le U(\bara\barb/C)\), then there is \(C'\), \(\bara\barb\not\ind_CC'\)
    and \(U(\bara\barb/CC')\ge\alpha\)

    Note that
    \begin{equation*}
    \barb\ind_CC'\wedge\bara\ind_{\barb C}C'\Rightarrow\bara\barb\ind_CC'
    \end{equation*}
    So either \(\barb\not\ind_CC'\) or \(\bara\not\ind_{\barb C}C'\).

    \(U(\bara/\barb CC')\le U(\bara/\barb C)\), \(U(\barb/CC')\le U(\barb/C)\), and at least one of
    these inequality is strict. So
    \begin{equation*}
    U(\bara/\barb CC')\oplus U(\barb/CC')<U(\bara/\barb C)\oplus U(\barb/C)
    \end{equation*}

    Also \(U(\bara\barb/CC')\ge\alpha\Rightarrow U(\bara/\barb CC')\oplus U(\barb/CC')\ge\alpha\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    Suppose \(U(\bara/C)\), \(U(\barb/C)\) are finite.
    Then \(U(\bara\barb/C)\le U(\bara/C)+U(\barb/C)\) and equality holds
    iff \(\bara\ind_C\barb\)
    #+END_proposition

    #+BEGIN_proof
    \(U(\bara/\barb)\le U(\bara/C)+U(\barb/C)\Leftrightarrow U(\bara/\barb C)+U(\barb/C)\le U(\bara/C)+U(\barb/C)\Leftrightarrow U(\bara/\barb C)\le U(\bara/C)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(\bara\in\dcl(\bara B)\) or \(\bara'\in\acl(\bara B)\), then \(U(\bara'/B)\le U(\bara/B)\)
    #+END_proposition

    \(\dcl(\barb C)=\dcl(\bara C)\Rightarrow U(\barb/C)=U(\bara/C)\)

    #+BEGIN_proof
    \begin{align*}
    U(\bara'/B)&=0+U(\bara'/B)\le U(\bara/\bara'B)+U(\bara'/B)\\
    &\le U(\bara\bara'/B)=U(\bara'\bara/B)\\
    &\le U(\bara'/\bara B)\oplus U(\bara/B)=0\oplus U(\bara/B)=U(\bara/B)
    \end{align*}
    #+END_proof

    Analogure for RM ref:5.7.6.10
** Lascar rank of sets
    \(T\) is superstable

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(A\) small, \(X\) \(A\)-definable, \(U_A(X)=\sup_{\barb\in X}U(\barb/A)\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(X\) is \(A\)-definable & \(A'\)-definable, then \(U_A(X)=U_A'(X)\)
    #+END_lemma

    #+BEGIN_proof
    \(S=\{U(\barb/A):\barb\in X\}\) and \(S'=\{U(\barb'/A):\barb'\in X\}\)

    First suppose \(A\subseteq A'\).

    *Claim*: If \(\alpha\in S'\) there is \(\beta\in S\), \(\alpha\le\beta\)

    \(\alpha=U(\barb/A')\le U(\barb/A)\)

    *Claim*: If \(\alpha\in S\) there is \(\beta\in S'\), \(\alpha\le\beta\)

    Take \(\barb\in X\), \(U(\barb/A)=\alpha\). Take \(\barb'\equiv_A\barb\), \(\barb'\ind_AA'\), \(X\)
    is \(A\)-definable so \(\barb'\in X\), \(\beta=U(\barb'/A')=U(\barb'/A)=U(\barb/A)=\alpha\)

    General case: \(U_A(X)=U_{AA'}(X)=U_{A'}(X)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(U(X)=U_A(X)\) for any small set \(A\) defining \(X\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(T\) is t.t. and \(X\) is definable, \(U(X)\le\RM(X)\)
    #+END_proposition

    #+BEGIN_proof
    Take small set \(A\) defining \(X\),
    then \(U(X)=\sup_{\barb\in X}U(\barb/A)\le\sup_{\barb\in X}\RM(\barb/A)=\RM(X)\)
    #+END_proof

    \(U(X)\) can be thought of as the "dimension" of \(X\). Here are some of its nice properties

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    If \(X\) is definable, \(U(X)>0\) \(\Leftrightarrow\) \(X\) is infinite
    #+END_theorem

    #+BEGIN_proof
    Take small \(A\) defining \(X\), \(U(X)>0\Leftrightarrow\exists \barb\in X(U(\barb/A)>0)\Leftrightarrow\exists\barb\in X(\barb\notin\acl(A))\)

    If \(X\) is finite, then \(\barb\in X\Rightarrow\barb\in\acl(A)\)

    If \(X\) is infinite, then \(X\) is large (by saturation), but \(\acl(A)\) is small, so we can find
    #+END_proof
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    If \(X,Y\in\M^n\) are definable
    1. \(X\subseteq Y\Rightarrow U(X)\le U(Y)\)
    2. \(U(X\cup Y)=\max(U(X),U(Y))\)
    #+END_theorem

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    If \(f:X\to Y\) is definable
    1. If \(f\) is surjective, then \(U(X)\ge U(Y)\)
    2. If \(f\) is bijective, then \(U(X)=U(Y)\)
    3. If \(f\) is injective, then \(U(X)\le U(Y)\)
    #+END_theorem

    #+BEGIN_proof
    1. Take small \(A\) defining \(X,Y,f\). If \(\barb\in Y\), then there
       is \(\bara\in X\), \(f(\bara)=\barb\),
       So
       \(\barb\in\dcl(A\bara)\).
       \(U(\barb/A)\le U(\bara/A)\le U(X)\). \(U(Y)=\sup_{\barb\in Y}U(\barb/A)\le U(X)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    If \(U(X),U(Y)<\omega\), then \(U(X\times Y)=U(X)+U(Y)\)
    #+END_theorem

    #+BEGIN_proof
    Take small \(C\) defining \(X,Y\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    Let \(f:X\to Y\) be a definable surjection. Suppose for every \(\barb\in Y\), the definable
    set \(f^{-1}(\barb)\) has Lascar rank \(k\). Suppose \(k,U(Y)<\omega\). Then \(U(X)=k+U(Y)\)
    #+END_theorem

** Lascar & Morley rank in strongly minimal theory
    Suppose \(T\) is strongly minimal (then \(T\) is t.t., superstable)

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(a\in\M'\), \(B\subseteq\M\)
    \begin{equation*}
    U(a/B)=
    \begin{cases}
    0&a\in\acl(B)\\
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(\bara\in\M^n\), \(B\subseteq\M\)
    \begin{equation*}
    U(\bara/B)=\sum_{i=1}^nU(a_i/Ba_1\dots a_{i-1})
    \end{equation*}
    #+END_proposition

    let \(p\in S_1(\M)\) be the transcendental type.

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(a_1,\dots,a_n\) is "independent" over \(B\) if
    1. \(\bara\vDash p^{\otimes n}\uhr B\)
    2. \(a_i\notin\acl(B a_1,\dots,a_{i-1})\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(\bara\in\M^n\) is independent over \(B\), then
    \begin{equation*}
    \RM(\bara/B)=U(\bara/B)=n
    \end{equation*}
    #+END_proposition
    ...
    #+BEGIN_proposition
    label:5.30.5.4
    If \(\bara\in\M^n\), \(B\subseteq\M\), \(\RM(\bara/B)\) is \(\max k\) s.t. there is subtuple \(\barc\)
    of \(\bara\), \(\barc\in\M^k\) and \(\barc\) is independent over \(B\)
    #+END_proposition

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(\varphi(x_1,\dots,x_n)\in L(\M)\), \(X=\varphi(\M^n)\), then \(U(X)=n\Leftrightarrow\varphi\in p^{\otimes n}\)
    #+END_lemma

    #+BEGIN_proof
    Take small \(N\subseteq\M\), \(\varphi\in L(N)\), \(\varphi\) is \(N\)-definable, \(U(X)=\max_{\barb\in X}U(\barb/N)\)

    \(U(X)\ge n\Leftrightarrow U(X)=n\Leftrightarrow\exists\barb\in X:U(\barb/N)=n\Leftrightarrow\)

    \(U(\barb/N)=n\Leftrightarrow\barb\vDash p^{\otimes n}\uhr N\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    If \(\varphi(\barx,\bary)\) is an \(L\)-formula, \(\barx=(x_1,\dots,x_n)\),
    \(\{\barb:U(\varphi(\M^n,\barb))=n\}=\{\barb:\varphi(\barx,\barb)\in p^{\otimes n}\}\) is definable because
    \(p^{\otimes n}\) is definable by stability
    #+END_corollary

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(X\subseteq\M^n\) definable, \(k\le n\), then TFAE
    1. \(\exists\) coordinate projection \(\pi:\M^n\to\M^k\) s.t. \(U(\pi(X))=k\)
    2. \(U(X)\ge k\)
    #+END_lemma

    #+BEGIN_proof
    \(1\to 2\): \(U(X)\ge U(\pi(X))=k\)

    \(2\to 1\): take \(B\subseteq\M\) defining \(X\). Take \(\bara\in X\), \(U(\bara/B)=U(X)\). By Proposition
    ref:5.30.5.4, there is subtuple \(\barc_0\subseteq\bara\) s.t. \(\barc_0\) has length \(j\) and is
    independent over \(B\). Take a subtuple \(\barc\subseteq\barc_0\) and has length \(k\), \(\barc\in\M^k\),
    then \(\barc=\pi(\bara)\) for some projection. Therefore \(U(\pi(X))\ge k\), but \(\pi(X)\subseteq\M^k\),
    so \(U(\pi(X))\le k\)
    #+END_proof

    #+ATTR_LATEX: :options [Definability of \(U\), \(\RM\)]
    #+BEGIN_theorem
    If \(\varphi(x_1,\dots,x_n,\bary)\in L\)  and \(D_k=\{\barb:U(\varphi(\M^n,\barb))=k\}\). Then each \(D_k\) is definable
    #+END_theorem

    #+BEGIN_proof
    Let \(D_k'=\{\barb:U(\varphi(\M^n,\barb))\ge k\}\). \(D_k=D_k'\setminus D_{k-1}'\).

    Fix \(k\), let \(N=\binom{n}{k}\), let \(\pi_1,\dots,\pi_N\) be the coordinate projections \(\M^n\to\M^k\).

    \(U(X)\ge k\Leftrightarrow\exists i\in\{1,\dots,N\}\), \(U(\pi_i(X))\ge k\)

    \(D_k'=\bigcup_{i=1}^nD_{k_i}'\), where \(D_{ki}'​=\{\barb:U(\pi_i(\varphi(\M^n,\barb)))=k\}\). Suffice to show
    that \(D_{ki}'\) is definable. Fix \(i\), let \(\pi​\pi_i\), let
    \(\psi(z_1,\dots,z_k,\bary)=\exists x_1,\dots,x_n:\pi(\barx)=\barz\wedge\varphi(\barx,\bary)\).
    Then \(\psi(\M^k,\barb)=\pi(\varphi(\M^n,\barb))\)

    \(D_{ki}'=\{\barb\in\M:U(\psi(\M^k,\barb))=k\}=\{\barb\in\M:\psi(\barz,\barb)\in p^{\otimes k}\}\)
    #+END_proof

** Type-definable and  V-definable sets
    No assumptions on \(T\)

    Let \(A\subseteq\M\) be small
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(X\subseteq\M^n\) is *type-definable* over \(A\) if
    \begin{equation*}
    X=\bigcap_{i\in I}D_i, \quad\text{each }D_i\text{ is }A\text{-definable}
    \end{equation*}
    \(X\subseteq\M^n\) is *\(\vee\)-definable over \(A\)* if
    \begin{equation*}
    X=\bigcup_{i\in I}D_i,\quad\text{each $D_i$ is $A$-definable}
    \end{equation*}
    #+END_definition

    \(X\) is type-definable over \(A\) iff \(X\) is defined by a partial type \(\Sigma\) over \(A\)

    \(X\) is type-definable over \(A\) iff \(\M^n\setminus X\) is \(\vee\)-definable over \(A\)

    #+ATTR_LATEX: :options [Open criterion]
    #+BEGIN_lemma
    label:5.30.6.2
    \(X\subseteq\M^n\) is \(\vee\)-definable over \(A\) \(\Leftrightarrow\) for any \(b\in X\) there is \(A\)-definable \(D\)
    s.t. \(b\in D\subseteq X\)
    #+END_lemma

    lemma ref:5.30.6.2 looks a little like the definition of "open sets" in topology

    \(\vee\)-definable sets over \(A\) \(\Leftrightarrow\) open sets in \(S_n(A)\)

    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(D\subseteq\M^n\) is definable, \(X\subseteq D\), if \(X,D\setminus X\) are \(\vee\)-definable, thne \(X\) is definable
    #+END_proposition

    #+BEGIN_proof
    \(X,D\setminus X\) are type-definable. Suppose \Gamma defines \(X\), \Sigma defines \(D\setminus X\). \(\Gamma(x)\cup\Sigma(X)\vdash\bot\),
    by compactness, thee is \(\Gamma_0\subseteq_f\Gamma\) and \(\Gamma_0\cup\Sigma\vdash\bot\). If \(b\in D\),
    then \(b\in X\Rightarrow b\vDash\Gamma\Rightarrow b\vDash\Gamma_0\Rightarrow b\not\vDash\Sigma\Rightarrow b\notin D\setminus x\Rightarrow b\in X\). Therefore \(b\in X\Leftrightarrow b\vDash\Gamma_0\), \(X\) is definable
    by \(\bigwedge\Gamma_0\)
    #+END_proof

    Fix \(\varphi(x,y)\), \(D_x=\{b:U(\varphi(\M,b))=k\}\), suppose we know \(U(X)<\omega\) for each
    definable \(X\), \(D_k\) is \(\vee\)-definable, then for each \(k<\omega\), \(D_k\)'s complement
    is \(\bigcup_{j\neq k}D_j\) which is \(\vee\)-definable, therefore \(D_k\) is definable

    \(D_k\) is \(\vee\)-definable over \(\emptyset\) means:
    if \(b\in D_k\) then there is \(\emptyset\)-definable \(D\) s.t. \(b\in D\subseteq D_k\). That is, if \(\varphi(\M,b)\) has
    rank \(k\), then there is \(\psi\in L\) s.t. \(b'\in\psi(\M)\) then \(\varphi(\M,b')\) has rank \(k\)

    idea: if \(U(\varphi(\M,b))=k\), it's because of some formula \(\psi(b)\)

** \texorpdfstring{\(\alpha\)}{α}-isolation
    Suppose \(T\) is t.t.

    #+BEGIN_remark
    If \(\varphi(x)\in L(\M)\), \(\alpha\in\On\), \(\exists^\infty p\in[\varphi]\), \(\RM(p)\ge\alpha\), then \(\RM(\varphi)\ge\alpha+1\)
    #+END_remark

    #+BEGIN_proof
    Let \(E_\alpha=\{p\in S_n(\M):\RM(p)\ge\alpha\}\), then \(E_{\alpha+1}=E_\alpha'\). Assumption: \(E_\alpha\cap[\varphi]\) is infinite.
    Then there is an accumulation point \(p\in E_{\alpha+1}\)
    #+END_proof

    #+BEGIN_remark
    If \(\varphi(x)\in L(A)\), if \([\varphi]\subseteq S_n(A)\), if \(\exists^\infty q\in[\varphi]:\RM(q)\ge\alpha\), then \(\RM(\varphi)\ge\alpha\)
    #+END_remark

    #+BEGIN_proof
    Take non-forking extension of each \(q_i\), \(p_i\sqsupseteq q_i\Rightarrow\RM(p_i)=\RM(q_i)\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    If \(\varphi(x)\in L(B)\), \(p\in S_n(B)\), then \varphi *\alpha-isolates* \(p\)
    if \(\{p\}=\{q\in S_n(B):\RM(q)\ge\alpha,\varphi\in q\}\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_examplle
    \varphi 0-isolates \(p\) if \varphi isolates \(p\)
    #+END_examplle

    If \varphi \alpha-isolates \(p\), then \(p\ni\varphi\)

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    label:5.30.7.2
    If \(\alpha=\RM(a/B)\), then \(\exists\varphi\in\tp(a/B)\), \varphi \alpha-isolates \(\tp(a/B)\)
    #+END_lemma

    #+BEGIN_proof
    By ref:5.7.6.9, \(\RM(a/B)=\min_{\varphi\in\tp(a/B)}\RM(\varphi)\), take \(\varphi_0\in\tp(a/B)\) with \(\RM(\varphi_0)=\alpha\).
    Remark above shows \(\{q\in S_n(B):q\ni\varphi_0,\RM(q)\ge\alpha\}\) is finite, i.e., \(\{p,q_1,\dots,q_m\}\).
    Take \(\psi_i\in p\setminus q_i\). Let \(\varphi=\varphi_0\wedge\bigwedge_{i=1}^m\psi_i\). Then \(\{q\in S_n(B):q\ni\varphi\}=\{p\}\)
    #+END_proof

    #+BEGIN_remark
    If \(M\) is \(\aleph_0\)-saturated model, then morley rank equals cantor-bensixon rank.
    If \(\varphi(x)\in L(M)\) \alpha-isolates \(p\in S_n(M)\), then \(\RM(\varphi)=\alpha\)
    #+END_remark

    #+BEGIN_proof
    \(E_\alpha\cap[\varphi]=\{p\}\), so \(p\notin E_{\alpha+1}\)
    #+END_proof

    suppose: if \(U(a/B)=\alpha\) then there is \(\varphi\)

    *Claim*: \(\forall\alpha\in\On\), \(U(a/B)=\alpha\) then \(\RM(a/B)=\alpha\)

    #+BEGIN_proof
    Take \(\aleph_0\)-saturated \(M\supseteq B\). Move it
    to \(M\ind_Ba\), \(\tp(a/M)\sqsupseteq\tp(a/B)\). \(U(a/M)=U(a/B)\), \(U(a/M)=\RM(a/B)\). Replace \(B\)
    with \(M\). WMA \(B=M\) a \(\aleph_0\)-saturated model

    By induction on \alpha (ref:5.26.2.6), \(U\le RM\), so \(\RM(a/M)\ge\alpha\). By assumption, there
    is \(\varphi(x)\in \tp(a/M)\) s.t. \(\{\tp(a/M)\}=\{p\in S_n(M):p\ni\varphi,U(p)\ge\alpha\}\). If \(p\in S_n(M)\), \(p\ni\varphi\) then
    either \(p=\tp(a/M)\) or \(U(p)<\alpha\), by induction, \(\RM(p)<\alpha\)

    So \varphi \alpha-isolates \(\tp(a/M)\), therefore \(\RM(\tp(a/M))=\alpha\)
    #+END_proof

** mm
    From now on \(T\) is uncountably categorical, \(\abs{L}=\aleph_0\)

    \(T\) is t.t. (ref:5.12.5.3), \(T\) has no Vaught pairs.

    \(T\) eliminates \(\exists^\infty\)

    there is a definable strongly minimal set \(D\) defined over prime model \(M_0\).

    _Assume_ \(D\) is \(\emptyset\)-definable (otherwise carry parameters everywhere) for simplicity

    Note \(1\le U(D)\le\RM(D)\le 1\)

    If \(b\in D\), \(C\subseteq\M\), \(U(b/C)\le U(D)=1\) as \(D\) is \(C\)-definable

    So \(U(b/C)=0\) if \(b\in\acl(C)\) and 1 if not

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(M\preceq\M\), \(a\notin M\), then \(\exists b\in D\) s.t. \(a\not\ind_Mb\), \(b\in\acl(Ma)\setminus\acl(M)\)
    #+END_lemma

    #+BEGIN_proof
    Let \(M[a]\) be prime constructible model \(\supseteq Ma\). By no Vaught
    pairs, \(D(M[a])\supsetneq D(M)\). Take \(b\in D(M[a])\setminus D(M)\)

    ref:5.12.3.5 implies \(M[a]\) is atomic over \(Ma\). \(\tp(b/Ma)\) is isolated in \(S_n(Ma)\)
    by \(\varphi(y)\in L(Ma)\). If \(b'\in\varphi(\M)\) then \(b'\equiv_{Ma}b\), so \(b'\in D\), \(b'\notin M\).
    Therefore \(\varphi(\M)\subseteq D\), \(\varphi(\M)\cap D(M)=\emptyset\). \(\varphi(\M)\) is finite or cofinite in \(D\),
    therefore \(\varphi(\M)\) is finite

    \(U(b/Ma)=0\), \(U(b/M)=1\), so \(b\not\ind_Ma\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_corollary
    \(U(a/C)<\omega\) for any \(a,C\)
    #+END_corollary

    #+BEGIN_proof
    Suppose not, \(U(a/C)\ge\omega\). Remark ref:5.26.2.7 "jump-free"
    \(\{U(a/C):a\in\M^n,C\subseteq\M\}\) is an initial segment, so WLOG, \(U(a/C)=\omega\)

    Take \(M\supseteq C\) small model, move by \(\sigma\in\Aut(\M/C)\) so \(M\ind_Ca\), \(U(a/M)=U(a/C)=\omega\)

    \(U(a/M)>0\Rightarrow a\notin M\)

    Lemma ref:5.30.8.1 gives \(b\in D\), \(a\not\ind_Mb\), \(b\ind_Ma\), \(n=U(a/Mb)<U(a/M)=\omega\)

    \(U(a/M)\le U(ab/M)\le U(a/bM)\oplus U(b/M)=n\oplus 1=n+1\)
    #+END_proof

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    In lemma ref:5.30.8.1, \(U(a/bM)=U(a/M)-1\)
    #+END_lemma

    #+BEGIN_proof
    \(U(ab/M)=U(b/aM)+U(a/M)=0+U(a/M)\)

    \(U(ba/M)=U(a/bM)+U(b/M)=U(a/bM)+1\)
    #+END_proof


** wef
    \(T\) is \(\aleph_1\)-categorical, \(D\) is strongly minimal

    Prove by induction on \(k<\omega\)

    Inductive lemma 1: \(\RM(a/B)=k\Leftrightarrow U(a/B)=k\)

    Inductive Lemma 2: \(U(X)=k\Leftrightarrow\RM(X)=k\)

    Inductive Lemma 3: If \(\varphi(x,y)\in L\), \(\{b:\varphi(\M,b)=k\}\) is \(\vee\)-definable over \(\emptyset\)
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    \(\varphi(x,y)\) is *\(n\)-good* if \(\forall b\), \(U(\varphi(\M,b))=n\) or \(\varphi(\M,b)=\emptyset\)
    #+END_definition

    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    If \(j<k\) and \(\varphi(\M,b)\) has rank \(j\), then there is \(j\)-good formula \(\varphi'\)
    s.t. \(\varphi'(\M,b)=\varphi(\M,b)\) and \(\varphi'(x,y)\vdash\varphi(x,y)\)
    #+END_lemma

    #+BEGIN_proof
    Let \(D_j=\{b:U(\varphi(\M,b))=j\}\), ILem 3 implies \(D_j\) is \(\vee\)-definable over \(\emptyset\). "Open
    criterion" gives \(b\in D\subseteq D_j\), \(D\) is \(\emptyset\)-definable, \(D=\psi(\M)\).
    Let \(\varphi'(x,y)=\varphi(x,y)\wedge\psi(y)\). \(\forall b'\)
    \begin{equation*}
    \varphi'(\M,b')=
    \begin{cases}
    \emptyset&b'\notin\psi(\M)\\
    \varphi(\M,b')&b'\in\psi(\M)
    \end{cases}
    \end{equation*}
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    Suppose \(M\preceq\M\), \(X\) is \(M\)-definable, \(a\in X\), \(U(a/M)=k\). Then \(\varphi(x,c)\in\tp(a/M)\) s.t.
    - \(\varphi(x,y)\) is \(k\)-good
    - \(\varphi(x,c)\) \(k\)-isolates \(\tp(a/M)\) (w.r.t. Morley rank)
    - \(\varphi(\M,c)\subseteq X\)
    #+END_lemma
