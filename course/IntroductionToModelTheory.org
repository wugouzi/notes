...
** wefwf
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    If \(p\in S_n(M)\), \(I\) set, \(\calu\) ultrafilter on \(I\), \(M^{\calu}=M^I/\calu\), the ultrapower of \(p\)
    is the type \(p^{\calu}\in S_n(M^{\calu})\) s.t. \((M,dp)^{\calu}=(M^{\calu},dp^{\calu})\)
    ...
    *Claim* \(\calf\) has F.I.P
    ...
    Thus there is \(\calu\supseteq\calf\). From \(M^{\calu}\), \(p^{\calu}\). Let \(g:N\to M^{\calu}\) as follows.
    If \(c\in N\), \(g(c)=[(f(c):f\in I)]\).

    For
    any
    \(\phi(\barx,\bary)\),
    \(\barb\in N\), \(\phi(\barx,\barb)\in q(\barx)\) \(\Rightarrow\) \(S_{\phi,\barb}\in\calf\) \(\Rightarrow\) \(S_{\phi,\barb}\in\calu\) \(\Rightarrow\)
    ...
    \(N\vDash\phi(\barb)\Rightarrow M^{\calu}\vDash\phi(g(\barb))\). WLOG, \(N\preceq M^{\calu}\) and \(g=\id_N\).
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(p\in S_n(M)\) is not definable, then some formula \(\varphi(x_1,\dots,x_n,\bary)\) has the dichotomy property
    #+END_proposition

    #+BEGIN_proof
    There is an \(N\succeq M\), \(q_1,q_2\in S_n(N)\),  \(q_1,q_2\sqsupseteq p\)  and \(q_1\neq q_2\). There
    ...
    *Claim* If \(M'\succeq N\), \(p'\in S_n(M')\), \(p'\sqsupseteq p\), then there is
    ...
    There is \(M^{\calu}\) s.t. \(M\preceq M'\preceq M^{\calu}\), \(p\subseteq p'\subseteq p^{\calu}\).
    Take \(N^{\calu}\), \(q_1^{\calu}\), \(q_2^{\calu}\).
    \(p\sqsubseteq p^{\calu}\sqsubseteq q_1^{\calu}\). If \(\barb'=\) copy of \(\barb\)
    in \(N^{\calu}\succeq N\), \(q_1^{\calu}\vdash\varphi(x,\barb')\), \(q_2^{\calu}\vdash\neg\varphi(\barx,\barb')\)
  ...
  (M0,p0)   (M1,p1)
    ...
    2. no formula \varphi has D.P.
    3. All types over models are definable
    #+END_theorem
