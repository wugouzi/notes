...
** The fundamental order
    ...
    \(q\sqsupseteq p\) iff \([q]=[p]\) in \(L(M)\)-structures
    #+END_remark
...
** The fundamental order in stable theory
    ...
    #+BEGIN_lemma
    label:3.31.8
    Suppose \(M\preceq N\preceq\M\), \(p\in S_n(M)\), \(q_1,q_2\in S_n(N)\), \(q_1,q_2\supseteq p\) and \([q_1]=[p]=[q_2]\). Then \(q_1=q_2\)
    #+END_lemma
...
** bounds
    ...
    *Claim*: \([p]=\max\Ex_M(p)\), so \(\Bd_M(p)=\{[p]\}\)
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_lemma
    Suppose \(M,N\preceq M\), \(M,N\supseteq A\)
    ...
    2. suppose \(\beta\in\Bd_M(p)\). By 1 twice,
    #+END_proof
** Theorem of the bound
    ...
    Then \(q^{\M}\not\sqsupseteq q^{\M}\uhr N\). By Proposition ref:3.31.9, \([q^{\M}\uhr N]>[q^{\M}]=[q]\)
    ...
    #+BEGIN_lemma
    label:3.31.18
    If \(\barb\in M\), \(A\subseteq\M\), then \(\exists M\supseteq A\), \(M\preceq\M\), the global heir of \(\tp(\barb/M)\) is
...
** Non-forking extensions
    ...
    #+BEGIN_proof
    Take \(M\supseteq B\), \(M\preceq\M\), \(r\in S_n(M)\), \(\)
    #+END_proof
