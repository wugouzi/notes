...
** EI in PA and ACF
    #+ATTR_LATEX: :options []
    #+BEGIN_theorem
    If \(T\supseteq\PA\) (e.g., \(T=\Th(\N)\)), then \(T\) has E.I.
    #+END_theorem
    ...
    *Claim*: If \(\sigma\in\Aut(\M)\), \(\sigma(S)=S\Leftrightarrow\sigma\in\Aut(\M/A)\)
    ...
    #+BEGIN_proof
    \(\ACF_0\vdash 0\neq 1\) so uniform EI \(\Leftrightarrow\) EI. Just need EI

    Take \(e\in D/E\), \(e=\ucorner{X}\) of some \(E\)-equivalence class \(X\)

    By Lemma ref:4.21.3, there is \(\bara\in X\), \(\bara\in\acl^{\eq}(e)\).
    Let \(S=\{\sigma(\bara):\sigma\in\Aut(\M/e)\}=\) realizations of \(\tp(\bara/e)\). By Proposition
    ref:4.7.14, \(\abs{S}<\infty\), \(S\) is \(e\)-definable. By the fact, \(\ucorner{S}\in\M^m\).
...
** Stability and \texorpdfstring{\(\M^{\eq}\)}{Meq}
    ...
    then \(e\in\dcl^{\eq}(\bara)\). \(\tp(e/B)\)
    determines \(\tp(e/A)\), \(\abs{S_{\barx}(B)}\supseteq\abs{S_{\barx}(A)}\), suffice to
...
** Theorem of the bound
    ...
    2. If \([q]=\beta\), then \(q\) is \(A\)-definable by Lemma ref:4.21.4.1. Take
       some \(q'\in S_n(\M)\), \(q'\supseteq p\), \([q']=\beta\). Then Lemma ref:4.21.4.1 shows that \(q'\) is \(A\)-definable
    #+END_proof
    ...
    #+BEGIN_remark
    \(\stp(\barb/A):=\tp(\barb/\acl^{\eq}(A))\), the *strong type* of \(\barb\) over \(A\)
...
** Forking
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_definition
    If \(q\supseteq p\), \(q\) is a *nonforking extension* if \(\tbd(q)=\tbd(p)\).
    #+END_definition
    ...
    #+ATTR_LATEX: :options []
    #+BEGIN_proposition
    If \(p\in S_n(A)\), \(q\in S_n(\M)\), \(q\supseteq p\), then \(q\sqsupseteq p\Leftrightarrow q\) is \(\acl^{\eq}(A)\)-definable
    #+END_proposition

    #+BEGIN_proof
    \(\tbd(q)=\tbd(p)\Leftrightarrow[q]=[p]\Leftrightarrow\) \(q\) is almost \(A\)-definable by Proposition ref:
    #+END_proof
    ...
    If \(r(\barx)\in S_n(\M)\), then \(r\supseteq\Sigma_A\Leftrightarrow r\) is \(\acl^{\eq}(A)\)-invariant \(\Leftrightarrow\) \(r\) is \(\acl^{\eq}(A)\)-definable.
    \(r(\barx)\vdash\varphi(\barx,\barb)\leftrightarrow\varphi(\barx,\barc)\Leftrightarrow\)
    (\(r(\barx)\vdash\varphi(\barx,\barb)\Leftrightarrow r(\barx)\vdash\varphi(\barx,\barc)\))
