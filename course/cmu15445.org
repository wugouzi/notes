...
* Multi Versioning
    The DBMS maintains multiple *physical* versions of a _single_ logical object in the database:
    ...
    * When a txn reads an object, it reads the newest version that existed when the txn started.
    ...
    Read-only txns can read a consistent snapshot without acquiring locks. Easily support
    time-travel queries.
